 ...
 *
 * Corrections v3 :
 *  - URL Edge Function hardcodée (fix 400 "Unknown action")
 *  - callEdge() robuste (apikey explicite)
 *  - Flux retrait conditionnel : KYC + followers + balance
 */
...
(async function initTiks() {
  ...
  /* ── Fetch éligibilité retrait (balance + followers + KYC) ── */
  async function fetchWithdrawEligibility(userId) {
    const fallback = { eligible: false, kycVerified: false, kycExists: false, followerCount: 0, balance: 0 };
    ...
    try {
      const [balRes, profileRes, kycRes] = await Promise.allSettled([
        ...
        supabase.from('user_profiles').select('follower_count').eq('id', userId).maybeSingle(),
      ...
      const followerCount = Number(profileRes.status === 'fulfilled' ? (profileRes.value?.data?.follower_count ?? 0) : 0);
      ...
      return { eligible, kycVerified, kycExists, followerCount, balance };
    } catch (e) {
  ...
  function buildWithdrawSection(eligibility) {
    const { eligible, kycVerified, followerCount, balance } = eligibility;
