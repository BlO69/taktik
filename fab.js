...
(function () {
  // === DEBUG ===
  const dbg = false; // mettre true pour traces détaillées
  ...
  // références d'intervalles (pour teardown)
  ...
  function getSeriesState() {
    ...
    try {
      ...
      // 2) window.gameState (maingame.js)
  ...
  // --- helper to ensure live.js module is loaded if not present ---
  ...
  function build() {
    ...
    // visibility checks: take en compte maingame.js et diverses conventions
    const seriesRelatedEvents = [
      'taktik:joined', 'maingame:joined', 'taktik:series:updated', 'series:updated', 'maingame:series:updated',
      'maingame:display:grille', 'maingame:display:minimap', 'maingame:display:commentaires', 'display:grille'
    ];
    seriesRelatedEvents.forEach(ev => document.addEventListener(ev, updateDiffuseVisibility));

    // périodique de secours au cas où l'état change sans émettre d'event
    _updateInterval = setInterval(updateDiffuseVisibility, 3000);
    dbgLog('updateDiffuseVisibility interval set (3s)');

    // initial try
    updateDiffuseVisibility();

    // sync with livekit publish state (light poll)
    _syncInterval = setInterval(() => {
      try {
        if (!diffuseBtn) { clearInterval(_syncInterval); _syncInterval = null; return; }
  ...
  function teardown() {
    ...
    try {
      ...
      if (diffuseBtn) {
        ...
        if (diffuseBtn._fabSyncInterval) clearInterval(diffuseBtn._fabSyncInterval);
      }
      ...
      if (diffuseBtn && diffuseBtn.parentNode) diffuseBtn.parentNode.removeChild(diffuseBtn);
  ...
  // - la série correspondante a status === 'ongoing' (insensible à la casse)
  async function updateDiffuseVisibility() {
    ...
    try {
      ...
      // fallback to window.gameState (maingame.js sets it)
      try {
        if ((!owner || !opponent) && window.gameState && typeof window.gameState === 'object') {
          const gs = window.gameState;
          owner = owner || gs.owner_id || gs.ownerId || gs.owner || null;
          opponent = opponent || gs.opponent_id || gs.opponentId || gs.opponent || null;
          dbgLog('owner/opponent from gameState', { owner, opponent });
        }
      } catch (e) { dbgLog('gameState read error', e); }
  ...
  window.addEventListener('beforeunload', () => {
    try { if (window.fab && typeof window.fab.destroy === 'function') window.fab.destroy(); } catch (_) {}
  });
