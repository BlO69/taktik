...
export default async function initCreateGame(supabaseClient) {
  ...
  // ---------------------------
  // join/redirect helper
  // ---------------------------
  async function joinGameOrRedirect(gameId) {
  try {
  ...
  return;
    ...
    try {
      ...
      if (mod && typeof mod.default === 'function') {
        // Passer supabase si votre init l'attend ; sinon passez null ou supprimez l'argument.
        await mod.default(window.supabase ?? null);
      }

      // Après init, retenter taktikGame.joinGame si présent
      if (window.taktikGame && typeof window.taktikGame.joinGame === 'function') {
        try {
          await window.taktikGame.joinGame(gameId);
          redirectToGame(gameId);
    ...
    // 3) Fallback final : rediriger MAIS en fournissant le game_id (pas de redirection "aveugle")
    ...
    // En dernier recours, rediriger si on a l'id, sinon vers la page générique
    if (gameId) {
        ...
        try {
          if (decision === 'accepted') {
            ...
            if (acceptErr) {
              ...
              // unique tentative de rejoindre / fallback
              if (targetGameId) {
                await joinGameOrRedirect(targetGameId);
              } else {
