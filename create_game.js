...
attachFollowToAvatar(avatar, p.id, { supabase });
    ...
    try {
      ...
      while (Date.now() < deadline) {
        ...
        if (rec.status === 'accepted') {
          ...
          const finalMatchId = rec.match_id ?? null;
          ...
          // Build redirect URL and include match_id when available
          if (finalGameId) {
            let url = `game.html?game_id=${encodeURIComponent(finalGameId)}`;
            if (finalMatchId) url += `&match_id=${encodeURIComponent(finalMatchId)}`;
            window.location.href = url;
          } else {
            // fallback: pass invitation id and match_id if available
            let url = `game.html?invitation_id=${encodeURIComponent(invitationId)}&from_invite=1`;
            if (finalMatchId) url += `&match_id=${encodeURIComponent(finalMatchId)}`;
            window.location.href = url;
          }
          return;
  ...
  // ---------------------------
  function redirectToGameId(gameId, matchId = null) {
    if (gameId) {
      let url = `game.html?game_id=${encodeURIComponent(gameId)}`;
  ...
  async function sendInvitation(inviteeId, message = '') {
    try {
      ...
      // extract IDs defensively (now includes match_id)
      let seriesId = null;
      let gameId = null;
      let matchId = null;
      try {
        if (Array.isArray(rpcRaw) && rpcRaw.length > 0) {
          const row = rpcRaw[0];
          seriesId = row.series_id ?? row.s_id ?? null;
          gameId = row.game_id ?? row.g_id ?? null;
          matchId = row.match_id ?? row.matchid ?? row.m_id ?? null;
        } else if (rpcRaw && typeof rpcRaw === 'object') {
          seriesId = rpcRaw.series_id ?? rpcRaw.s_id ?? null;
          gameId = rpcRaw.game_id ?? rpcRaw.g_id ?? null;
          matchId = rpcRaw.match_id ?? rpcRaw.matchid ?? rpcRaw.m_id ?? null;
        }
      } catch (e) {
        dbgLog('extract rpc result error', e);
      }

      dbgLog('extracted ids from rpc', { seriesId, gameId, matchId });
      ...
      // 2) insert invitation row (include match_id when available)
      ...
      if (matchId) toInsert.match_id = matchId;

      const { data: inv, error: invErr } = await supabase.from('game_invitations').insert(toInsert).select().single();
        ...
        try {
          if (decision === 'accepted') {
            ...
            if (acceptErr) {
              ...
              const latestMatchId = latest?.match_id ?? rec?.match_id ?? null;
              ...
              // If we have a match id from latest / rec, prefer that for redirect
