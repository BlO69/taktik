// create_game.js — version corrigée: n'effectue une redirection vers game.html
// que si game_invitations.status === 'accepted'. Affiche un petit spinner pendant les
// attentes critiques et des alertes DBG indiquant si un game_id a été reçu.
...
export default async function initCreateGame(supabaseClient) {
  ...
  // small spinner for short waits (returns { close })
  function showSmallSpinner(message = 'Patientez...') {
    const overlay = createOverlay();
    const box = document.createElement('div');
    ...
    const spinner = document.createElement('div');
    spinner.innerHTML = '<div style="margin:6px auto;width:36px;height:36px;border-radius:50%;border:4px solid #ddd;border-top-color:#333;animation:spin 1s linear infinite"></div>';
    ...
    async function tick() {
      ...
      try {
        ...
        if (data && data.status && data.status !== 'pending') {
          // call onFinal but let the caller verify status before redirecting
          onFinal(data);
          stop();
          return;
        }
  ...
  // ---------------------------
  // join/redirect helper — *ne redirige que si status === 'accepted'*
  // ---------------------------
  async function joinGameOrRedirect(gameId, sourceStatus = 'accepted') {
    try {
      // only proceed if the invitation status was accepted
      if (sourceStatus !== 'accepted') {
        dbgLog('joinGameOrRedirect: status != accepted, annulation de la redirection', sourceStatus);
        return;
      }
      ...
      if (!gameId) {
        ...
        alert("DBG: Aucun game_id reçu — impossible de rediriger vers la partie.");
        return;
      }
      ...
      // 1) Déléguer à taktikGame.joinGame si disponible
      if (window.taktikGame && typeof window.taktikGame.joinGame === 'function') {
        try {
          dbgLog?.('joinGameOrRedirect: délégation à taktikGame.joinGame', gameId);
          await window.taktikGame.joinGame(gameId);
          // Si join réussit, rediriger en fournissant le game_id
          redirectToGame(gameId);
          return;
        } catch (e) {
          dbgLog?.('taktikGame.joinGame a échoué — fallback', e);
        }
      }

      // 2) Essayer l'import dynamique de game.js (initialisation éventuelle)
      try {
        dbgLog?.('joinGameOrRedirect: import dynamique de game.js', gameId);
        ...
        if (window.taktikGame && typeof window.taktikGame.joinGame === 'function') {
          try {
            ...
            dbgLog?.('taktikGame.joinGame après import a échoué', e);
          }
        }
      } catch (e) {
        dbgLog?.('Import dynamique de game.js impossible', e);
      }

      // 3) Fallback final : rediriger (on sait que status === 'accepted')
      dbgLog?.('joinGameOrRedirect: fallback -> redirection avec game_id', gameId);
      redirectToGame(gameId);
    } catch (e) {
      console.error('joinGameOrRedirect échoué de façon inattendue', e);
      if (gameId) {
        window.location.href = `game.html?game_id=${encodeURIComponent(gameId)}`;
      } else {
        // ne pas rediriger aveuglément si on n'a pas l'id
        alert('DBG: joinGameOrRedirect: échec et absence de game_id.');
      }
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (fresh && fresh.status && fresh.status !== 'pending') {
          ...
          if (fresh.status === 'accepted') {
            ...
            if (targetGameId) {
              alert('DBG: reçu game_id immediate: ' + targetGameId);
              await joinGameOrRedirect(targetGameId, 'accepted');
            } else {
              alert('DBG: pas de game_id reçu immediate pour invitation ' + fresh.id);
              // ne redirigez pas si la status n'est pas 'accepted' — ici on est accepted mais on a pas d'id -> on peut rester ou afficher message
              // on choisit de ne PAS rediriger aveuglément sans game_id
            }
          } else {
      ...
      async function handleOutgoingAccepted(rec) {
        try {
          ...
          // show spinner while waiting a bit if no id
          let spinnerInst = null;
          if (!finalGameId) spinnerInst = showSmallSpinner('Attente mise à jour base...');
          ...
          if (spinnerInst) spinnerInst.close();

          if (finalGameId) {
            alert('DBG: reçu game_id après accept: ' + finalGameId);
            await joinGameOrRedirect(finalGameId, 'accepted');
          } else {
            alert('DBG: aucun game_id reçu après accept pour invitation ' + finalRec.id + ' — pas de redirection.');
            // volontairement on n'effectue PAS window.location.href = 'game.html' si on n'a pas l'id
            dbgLog('handleOutgoingAccepted: pas de game_id trouvé — aucune redirection effectuée');
          }
        } catch (e) {
      ...
      try {
        ...
        if (!subscribed) {
          ...
          const poller = pollInvitationUntilFinal(inv.id, async (finalRec) => {
            try {
              // ensure we only react if the final status is 'accepted'
              if (!finalRec || finalRec.status !== 'accepted') {
                dbgLog('poller: finalRec status != accepted —', finalRec?.status);
                // don't redirect; just notify
                if (finalRec && ['declined','cancelled','expired'].includes(finalRec.status)) {
                  info('Invitation terminée : ' + finalRec.status);
                }
                return;
              }
              ...
              if (finalGameId) {
                alert('DBG: reçu game_id via poller: ' + finalGameId);
                await joinGameOrRedirect(finalGameId, 'accepted');
              } else {
                alert('DBG: aucun game_id trouvé via poller pour invitation ' + finalRec.id + ' — aucune redirection.');
                dbgLog('poller: no game_id after accepted');
              }
            } catch (e) {
        ...
        const poller = pollInvitationUntilFinal(inv.id, async (finalRec) => {
          try {
            if (!finalRec || finalRec.status !== 'accepted') {
              dbgLog('poller(catch): finalRec status != accepted —', finalRec?.status);
              if (finalRec && ['declined','cancelled','expired'].includes(finalRec.status)) info('Invitation terminée : ' + finalRec.status);
              return;
            }
            ...
            if (!finalGameId) finalGameId = await waitForGameId(finalRec.id, 5000, 500);

            if (finalGameId) {
              alert('DBG: reçu game_id via poller(catch): ' + finalGameId);
              await joinGameOrRedirect(finalGameId, 'accepted');
            } else {
              alert('DBG: aucun game_id trouvé via poller(catch) pour invitation ' + finalRec.id + ' — aucune redirection.');
            }
          } catch (e) {
      ...
      const overlayInst = showIncomingOverlay(rec, async (decision) => {
        try {
          if (decision === 'accepted') {
            const spinner = showSmallSpinner('Acceptation...');
            ...
            if (acceptErr) {
              ...
              if (!targetGameId) {
                ...
                const spinner2 = showSmallSpinner('Récupération game_id...');
              ...
              if (targetGameId) {
                alert('DBG: reçu game_id après accept (invitee): ' + targetGameId);
                await joinGameOrRedirect(targetGameId, 'accepted');
              } else {
                alert('DBG: aucun game_id reçu après accept pour invitation ' + rec.id + ' — aucune redirection.');
                dbgLog('showIncoming: pas de game_id trouvé — aucune redirection effectuée');
              }
            }
  ...
  // Re-subscribe logic: when page becomes visible, when auth state changes.
  let visibilityHandler = null;
  function ensureVisibilityAndAuthHandlers() {
    if (!visibilityHandler) {
      visibilityHandler = () => {
        if (document.visibilityState === 'visible') {
          subscribeIncomingInvitations().catch(e => dbgLog('subscribe on visibility error', e));
        }
      };
    ...
    if (typeof supabase.auth.onAuthStateChange === 'function') {
      supabase.auth.onAuthStateChange((event, session) => {
        ...
        // slight delay to give session a chance to settle
        setTimeout(() => subscribeIncomingInvitations().catch(e => dbgLog('subscribe on auth change error', e)), 200);
      });
    }
  }

  ensureVisibilityAndAuthHandlers();
