...
// Fallback: crée un conteneur #allParticipants où chaque participant obtient sa propre zone vidéo
...
function participantToSlotIdentity(participant, gameState) {
  ...
  if (!gameState) return null; // ne force plus 'moderator' si gameState absent
...
function ensureAudioElement(slotEl) {
  // pour audio on attache un <audio> caché au slot (ou au body si pas de slot)
  let audio = slotEl ? slotEl.querySelector('audio') : null;
  if (audio) {
    audio.autoplay = true;
    audio.controls = false;
    audio.style.display = slotEl ? 'none' : 'block';
    // tenter de rendre audible (souvent bloqué par navigateur si pas d'interaction)
    audio.muted = false;
  ...
  audio.controls = false;
  audio.style.display = slotEl ? 'none' : 'block';
  // tenter de lire tout de suite ; si bloqué, l'utilisateur devra interagir pour activer l'audio
  audio.muted = false;
...
function clearSlot(slotEl) {
  ...
  try {
    ...
    // keep label if participant fallback; but safe to clear content
    const labels = slotEl.querySelectorAll('div');
    for (const node of Array.from(slotEl.childNodes)) {
      if (node.tagName && !['DIV','VIDEO','AUDIO','CANVAS'].includes(node.tagName)) node.remove();
    }
    slotEl.innerHTML = '';
  } catch (e) {
    slotEl.innerHTML = '';
 ...
 *   so that anyone with the gameId sees owner/opponent/animateur.
...
function spectatorMaySubscribe(pub, participant) {
  ...
  // Si cette publication appartient aux slots owner/opponent/moderator => autoriser TOUJOURS
  try {
  ...
  // Pour les autres publications (streams non-principaux), appliquer la limite cliente (3).
...
async function handlePublication(pub, participant, role) {
  try {
    ...
    // Si la publication appartient aux slots owner/opponent/moderator,
    // on force la souscription (audio+video) pour *tout le monde*.
    let forceSubscribeForPrimary = false;
    try {
      const gs = getGameState();
      const slot = participantToSlotIdentity(participant, gs);
      if (['owner', 'opponent', 'moderator'].includes(slot)) {
        forceSubscribeForPrimary = true;
      }
    } catch (e) {
      dbgWarn('handlePublication: participantToSlotIdentity failed', e);
    }

    // si on a une publication déjà souscrite, on ne repasse pas
    const isSubscribed = (pub.isSubscribed ?? true);

    // décision de souscription :
    if (forceSubscribeForPrimary) {
      // essayer de (re)mettre la souscription sur true si possible
      try {
        if (typeof pub.setSubscribed === 'function') {
        ...
        dbgWarn('force subscribe primary pub failed', e);
      }
    } else if (role === 'spectateur') {
      // Pour les spectateurs, appliquer la logique existante / limite
      if (isSubscribed) {
      ...
      // rôles non spectateur (player/animateur) : essayer de s'abonner automatiquement aussi
      if (!isSubscribed) {
        try {
          if (typeof pub.setSubscribed === 'function') {
            await pub.setSubscribed(true);
          ...
          dbgWarn('subscribe attempt for non-spectator failed', e);
        }
      }
    }

    // attacher la piste si elle est disponible et souscrite
    const track = pub.track ?? pub;
  ...
  try {
    ...
    participant?.on && participant.on('trackUnsubscribed', (publishedTrackOrPub) => {
      try {
        ...
        markUnsubscribed(pub);
        const slotKey = participantToSlotIdentity(participant, getGameState());
        const slotEl = slotKey ? getSlotEl(slotKey) : document.getElementById(`participant_${sanitizeId(participant?.identity)}`);
        if (slotEl) {
          clearSlot(slotEl);
        }
    ...
    participant?.on && participant.on('disconnected', () => {
      try {
        ...
        const slotEl = slotKey ? getSlotEl(slotKey) : document.getElementById(`participant_${sanitizeId(participant?.identity)}`);
        if (slotEl) {
          try { slotEl.remove(); } catch (_) { clearSlot(slotEl); }
        }
...
async function connectAndJoin(roomName, identity, gameId) {
  ...
  if (typeof LK.connect === 'function') {
    try {
      ...
      room?.on && room.on('participantDisconnected', p => { const slot = participantToSlotIdentity(p, getGameState()); const slotEl = slot ? getSlotEl(slot) : document.getElementById(`participant_${sanitizeId(p?.identity)}`); if (slotEl) try { slotEl.remove(); } catch(e){ clearSlot(slotEl); } });
      return room;
    } catch (e) {
...
async function startPublish() {
  ...
  try {
    ...
    const slotKey = (gs && userIdStr === safeToString(ownerId)) ? 'owner'
                   ...
                   : 'moderator';
    const slotEl = getSlotEl(slotKey) || getOrCreateParticipantContainer({ identity: userIdStr });
