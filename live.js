...
/**
 * ensureAudioElement(slotElOrNull, { visible = true })
 * - si slotEl fourni et visible=true : ajoute un <audio> dans le slot (visuel)
 * - si slotEl fourni et visible=false : ajoute un <audio> masqué dans le slot (aperçu local)
 * - si slotEl null : crée un <audio> dans document.body (visible) — utile pour que les spectateurs entendent l'audio
 */
function ensureAudioElement(slotEl = null, { visible = true } = {}) {
  try {
    ...
    if (audio) {
      ...
      audio.muted = !visible;
      audio.style.display = visible ? 'block' : 'none';
      return audio;
    }
    ...
    audio.controls = false;
    audio.muted = !visible;
    audio.setAttribute('aria-hidden', visible ? 'false' : 'true');
    audio.style.display = visible ? 'block' : 'none';
    // for body-placed audio keep it unobtrusive
    if (!slotEl) {
    ...
    dbgWarn('ensureAudioElement failed', e);
    return null;
  }
}

/** Attach track to slot (handles audio/video tracks).
 *  Pour l'audio we attach par défaut à document.body (visible/non-mute) afin que tous les spectateurs entendent le flux,
 *  sauf pour les aperçus locaux (isLocalPreview) où on attache un audio muté dans le slot pour monitorer sans feedback.
 */
function attachTrackToSlot(track, slotEl, { isLocalPreview = false } = {}) {
  if (!track) return;
  try {
    // track peut être un objet publication ou un vrai Local/RemoteTrack
    ...
    if (kind === 'video') {
      ...
      // pour l'audio, si aperçu local -> attacher un audio muté dans le slot,
      // sinon attacher un audio dans document.body visible (permet aux spectateurs d'entendre)
      const shouldAttachToBody = !isLocalPreview;
      const audioEl = ensureAudioElement(shouldAttachToBody ? null : slotEl, { visible: shouldAttachToBody ? true : false });
      if (typeof track.attach === 'function') {
        const maybeEl = track.attach(audioEl);
        if (maybeEl && maybeEl !== audioEl) {
          try { audioEl.remove(); } catch(_) {}
          if (shouldAttachToBody) document.body.appendChild(maybeEl);
          else if (slotEl) slotEl.appendChild(maybeEl);
        } else {
          if (shouldAttachToBody) {
            if (!document.body.contains(audioEl)) document.body.appendChild(audioEl);
          } else {
            if (slotEl && !slotEl.contains(audioEl)) slotEl.appendChild(audioEl);
          }
        }
      } else {
...
function markSubscribed(pub, participant) {
  ...
  if (!subscribedTracksMap.has(id)) {
    ...
    // count audio et video ensemble (limite globale)
...
/**
 * Decide if un spectateur peut s'abonner à une publication.
 *
 * - Toujours permettre les trois slots principaux (owner/opponent/moderator)
 * - Sinon appliquer limite client-side (3)
 */
...
async function disableSubscriptionOnPub(pub) {
  try {
    ...
    // fallback: detach any attached elements so user n'entende/voit plus le stream
    try {
...
async function handlePublication(pub, participant, role) {
  try {
    const isSubscribed = (typeof pub.isSubscribed === 'boolean') ? pub.isSubscribed : true;
    const kind = getPublicationKind(pub);

    // Role spectateur: appliquer limite mais laisser owner/opponent/moderator
    if (role === 'spectateur') {
      if (isSubscribed) {
        ...
        // tenter de s'abonner (plusieurs variantes d'API)
        try {
          if (typeof pub.setSubscribed === 'function') {
            await pub.setSubscribed(true);
            dbgLog('pub.setSubscribed(true) success');
          } else if (typeof pub.subscribe === 'function') {
            await pub.subscribe();
            dbgLog('pub.subscribe() success');
          } else if (pub.track && pub.track.play) {
            // some SDKs auto-subscribe
            dbgLog('Assuming auto-subscribe present');
          }
        } catch (e) {
    ...
    // après tentative d'abonnement, attacher si possible
    const track = pub.track ?? pub;
    const isNowSubscribed = (typeof pub.isSubscribed === 'boolean') ? pub.isSubscribed : true;
    if (track && isNowSubscribed) {
      // choisir le slot : owner/opponent/moderator si possible
      const gameState = getGameState();
      const slot = participantToSlotIdentity(participant, gameState);
      const slotEl = getSlotEl(slot);
      // Pour les pistes audio distantes, attacher dans document.body pour que tous puissent entendre
...
async function handleParticipant(participant, gameState, role) {
  try {
    ...
    // events dynamiques : trackSubscribed / trackUnsubscribed
    participant?.on && participant.on('trackSubscribed', async (trackPubOrTrack) => {
      try {
        // livekit sometimes sends publication, sometimes track
    ...
    participant?.on && participant.on('trackUnsubscribed', (publishedTrackOrPub) => {
      try {
        ...
        const slot = participantToSlotIdentity(participant, gameState);
        const slotEl = getSlotEl(slot);
        // clear slot (simple) pour éviter éléments orphelins
        clearSlot(slotEl);
        // plus : tenter supprimer tout audio global pour ce participant (balayage du map)
    ...
    participant?.on && participant.on('disconnected', () => {
      try {
        ...
        const slotEl = getSlotEl(slot);
        clearSlot(slotEl);
        // supprimer pubs appartenant à ce participant
        for (const [id, meta] of Array.from(subscribedTracksMap.entries())) {
          if (meta.participantSid && participant && (participant.sid === meta.participantSid || participant.identity === meta.participantSid)) {
            subscribedTracksMap.delete(id);
          }
...
async function connectAndJoin(roomName, identity, gameId) {
  ...
  // --- CONNECT: try plusieurs formes d'API ---
...
async function startPublish() {
  ...
  try {
    ...
    // Attacher la preview vidéo (muted) et la preview audio (muted) dans le slot local
...
async function stopPublish() {
  ...
  try {
    ...
    // retirer aussi tout <audio> global restant (tentative safe)
    try {
      const bodyAudios = Array.from(document.body.querySelectorAll('audio'));
      bodyAudios.forEach(a => {
...
async function disconnectRoom() {
  ...
  // cleanup global audio elements
  try {
    const bodyAudios = Array.from(document.body.querySelectorAll('audio'));
    bodyAudios.forEach(a => { try { a.remove(); } catch (e) {} });
  } catch (e) {}
