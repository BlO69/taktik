...
async function trySubscribePublication(pub, participant) {
  ...
  // 4) If the publication object emits 'subscribed' event, attach a listener to attach later when track arrives
  try {
  ...
  // 5) last resort: if publication has a track already, attempt to attach immediately
  try {
...
async function handlePublication(pubOrTrack, participant, role) {
  try {
    // Normalize: we accept either a Publication-like or a Track-like object.
