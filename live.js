...
/**
 * ensureVideoElement:
 * - If the slot element itself is a <video>, reuse it (preserve its id).
 * - Otherwise create or reuse a child <video>.
 * - Do not remove the slot element itself (so id remains stable).
 */
function ensureVideoElement(slotEl, { muted = false } = {}) {
  if (!slotEl) return null;

  // If slotEl itself is a <video>, use it directly (preserve id)
  if (slotEl.tagName && slotEl.tagName.toLowerCase() === 'video') {
    const video = slotEl;
    try {
      video.autoplay = true;
      ...
      ensureSlotMuteButton(video.parentElement || video);
    } catch (_) {}
    return video;
  }

  // If slot contains a video child, reuse it and preserve its id
  ...
  if (video) {
    try {
      ...
      dbgWarn('existing video element update failed', e);
    }
  }

  // Otherwise create a new <video> child (do not clear slotEl entirely — preserve attributes)
  ...
  // prefer to give the created video a stable predictable id derived from slot id
  try {
    if (slotEl.id) {
      // only set id if no child video existed (to avoid clobbering existing ids)
      video.id = `${slotEl.id}-video`;
    }
  } catch (e) {}

  // remove placeholder elements but keep the slot element itself
  const placeholders = Array.from(slotEl.querySelectorAll('.lk-broadcast-placeholder, audio, canvas'));
  placeholders.forEach(p => { try { p.remove(); } catch (e) {} });

  // append video as first child
  try {
    slotEl.insertBefore(video, slotEl.firstChild);
  } catch (e) {
    try { slotEl.appendChild(video); } catch (e2) { dbgWarn('failed to append video to slot', e2); }
  }
  ensureSlotMuteButton(slotEl);
  return video;
}
...
function ensureAudioElement(slotEl) {
  // If slotEl itself is an <audio>, reuse it
  if (slotEl && slotEl.tagName && slotEl.tagName.toLowerCase() === 'audio') {
    const audio = slotEl;
    audio.autoplay = true;
    audio.controls = false;
    audio.style.display = 'none';
    audio.muted = false;
    ensureSlotMuteButton(slotEl.parentElement || slotEl);
    return audio;
  }
...
function ensureSlotMuteButton(slotEl) {
  if (!slotEl) return;
  // ensure we attach the mute button to a stable element (prefer slotEl if it's a container; otherwise parent)
  let host = slotEl;
  if (slotEl.tagName && (slotEl.tagName.toLowerCase() === 'video' || slotEl.tagName.toLowerCase() === 'audio')) {
    host = slotEl.parentElement || slotEl;
  }
  if (!host) return;
  if (host.querySelector('.lk-mute-btn')) return;
  ...
  host.dataset.lkMuted = 'false';

  btn.addEventListener('click', () => {
    const isMuted = host.dataset.lkMuted === 'true';
    const newMuted = !isMuted;
    const audios = Array.from(host.querySelectorAll('audio'));
    audios.forEach(a => {
      try { a.muted = newMuted; } catch (e) {}
    });
    // also toggle global audio elements attached to body
    const globalAudios = Array.from(document.querySelectorAll('audio[data-livekit-audio]'));
    globalAudios.forEach(a => { try { a.muted = newMuted; } catch (e) {} });
    host.dataset.lkMuted = newMuted ? 'true' : 'false';
...
function attachTrackToSlot(track, slotEl, { isLocalPreview = false } = {}) {
  ...
  try {
    ...
    if (kind === 'video') {
      // If slotEl is a <video> element itself, pass it to attach to preserve the id
      let videoEl;
      if (slotEl && slotEl.tagName && slotEl.tagName.toLowerCase() === 'video') {
        videoEl = slotEl;
      } else {
        videoEl = ensureVideoElement(slotEl, { muted: !!isLocalPreview });
      }

      if (typeof track.attach === 'function') {
        // Many LiveKit SDKs accept an existing element as argument to attach()
        try {
          const maybeEl = track.attach(videoEl);
          // If SDK returns a new element, replace only the contents but preserve a stable id if possible
          if (maybeEl && maybeEl !== videoEl) {
            // try to keep predictable id: prefer previous id on videoEl or derive from slotEl
            try {
              const preservedId = videoEl && videoEl.id ? videoEl.id : (slotEl && slotEl.id ? `${slotEl.id}-video` : null);
              // remove old videoEl from DOM but keep slot container
              if (videoEl.parentElement) {
                videoEl.remove();
              }
              if (preservedId && maybeEl.setAttribute) maybeEl.setAttribute('id', preservedId);
              if (slotEl) slotEl.appendChild(maybeEl);
            } catch (e) {
              // fallback: append returned element
              try { if (slotEl) slotEl.appendChild(maybeEl); } catch (_) {}
            }
          } else {
            // ensure videoEl is in DOM
            if (slotEl && !slotEl.contains(videoEl)) {
              slotEl.innerHTML = '';
              slotEl.appendChild(videoEl);
            }
          }
        } catch (e) {
          dbgWarn('track.attach(videoEl) threw, trying attach() without arg', e);
          try {
            const maybeEl2 = track.attach();
            if (maybeEl2) {
              // set stable id if possible
              try {
                const preservedId = videoEl && videoEl.id ? videoEl.id : (slotEl && slotEl.id ? `${slotEl.id}-video` : null);
                if (preservedId && maybeEl2.setAttribute) maybeEl2.setAttribute('id', preservedId);
                // remove old
                try { if (videoEl && videoEl.parentElement) videoEl.remove(); } catch (_) {}
                if (slotEl) slotEl.appendChild(maybeEl2);
              } catch (e2) { dbgWarn('attach fallback handling failed', e2); }
            }
          } catch (e2) { dbgWarn('track.attach() fallback also failed', e2); }
        }
      } else {
    ...
    // Bookkeeping: if the publication had sid info, mark subscribed
    try {
      markSubscribed(track, { sid: track.participantSid ?? track.sid ?? track.participantId ?? null });
    } catch (_) {}
...
function clearSlot(slotEl) {
  if (!slotEl) return;
  try {
    // If the slot element is itself a <video>, don't remove the element — just stop/clear it
    if (slotEl.tagName && slotEl.tagName.toLowerCase() === 'video') {
      try {
        // pause and clear srcObject if present
        try { slotEl.pause(); } catch (_) {}
        try { if (slotEl.srcObject) slotEl.srcObject = null; } catch (_) {}
        // remove any children (unlikely for video) and reset attributes except id
        // don't alter slotEl.id
      } catch (e) { dbgWarn('clearSlot (video element) error', e); }
      return;
    }
    ...
    // leave slotEl element in place (we don't reset its id)
    // slotEl.innerHTML = ''; // avoid wiping slot attributes/ids used by other scripts
  } catch (e) {
    try { slotEl.innerHTML = ''; } catch (_) {}
  }
}
...
/**
 * Try multiple subscription APIs for a publication and register fallback listeners.
 */
async function trySubscribePublication(pub, participant) {
  if (!pub) return;
  const id = getPublicationId(pub);
  dbgLog('trySubscribePublication for', id);
  ...
  try {
    if (liveRoom && typeof liveRoom.setSubscribed === 'function') {
      try {
        ...
        return true;
      } catch (e) { dbgWarn('liveRoom.setSubscribed(pub,true) threw', e); }
    }
  } catch (e) {}

  // 5) If the publication object emits 'subscribed' event, attach a listener to attach later when track arrives
  try {
  ...
  // 6) last resort: if publication has a track already, attempt to attach immediately
  try {
      ...
      if (pubKind === 'video') {
        ...
        (async () => {
          try {
            ...
            const hasVideoEl = slotEl && (slotEl.tagName && slotEl.tagName.toLowerCase() === 'video' ? slotEl : slotEl.querySelector('video'));
...
window.livekit = window.livekit || {
  ...
  toggleSlotMute: (logicalName) => {
    try {
      ...
      const host = (slotEl.tagName && (slotEl.tagName.toLowerCase() === 'video' || slotEl.tagName.toLowerCase() === 'audio')) ? (slotEl.parentElement || slotEl) : slotEl;
      const btn = host.querySelector('.lk-mute-btn');
      if (!btn) return false;
      btn.click();
      return true;
...
(async function autoConnectOnLoad(){
  try {
    ...
    } catch (e) { dbgWarn('autoConnect: supabase session read failed', e); }

    if (!identity) {
      identity = window.currentUserId || window.userId || null;
    }
    if (!identity) return;
