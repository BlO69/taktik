...
function markSubscribed(pub, participant) {
  ...
  if (!subscribedTracksMap.has(id)) {
    ...
    // count both audio and video (we consider audio/video equal for the 3-stream limit)
...
/** Decide if a spectator may subscribe to a publication (enforces max 3) */
function spectatorMaySubscribe(pub) {
  const kind = getPublicationKind(pub);
  // only count audio/video
  if (!['video', 'audio'].includes(kind)) return false;
  // if we have fewer than 3 subscriptions, allow
...
async function disableSubscriptionOnPub(pub) {
  try {
    ...
    // try participant.unpublish? not appropriate for remote. skip.
...
async function handlePublication(pub, participant, role) {
  try {
    ...
    const kind = getPublicationKind(pub);
    // If spectator, enforce limit
    if (role === 'spectateur') {
      if (isSubscribed) {
        if (!spectatorMaySubscribe(pub)) {
        ...
        // publication not subscribed yet but spectator may subscribe only if below limit
        if (!spectatorMaySubscribe(pub)) {
          dbgLog('Spectator: skip subscribe (limit reached) for', getPublicationId(pub));
          return;
        }
      }
      // allowed: subscribe (we assume if pub has setSubscribed we can call it)
      try {
        if (typeof pub.setSubscribed === 'function') {
    ...
    // If subscribed (or after we attempted to subscribe), attach if allowed
    const track = pub.track ?? pub;
...
async function connectAndJoin(roomName, identity, gameId) {
  ...
  const autoSubscribe = (role !== 'spectateur'); // spectators: we'll enforce client-side limit
