...
function spectatorMaySubscribe(pub, participant) {
  const kind = getPublicationKind(pub);
  // n'autorise que audio/video
  if (!['video', 'audio'].includes(kind)) return false;

  // On autorise toujours l'audio et la vidéo pour tous les clients.
  // (Si tu veux plus tard réintroduire une limite, remplace 'true' par la logique souhaitée.)
  return true;
}
...
// --- remplacement: handlePublication (force subscribe pour tous) ---
async function handlePublication(pub, participant, role) {
  try {
    ...
    // --- Toujours tenter de s'abonner (pour spectateurs et non-spectateurs) ---
    try {
      if (!pub?.isSubscribed) {
        if (typeof pub.setSubscribed === 'function') {
          ...
          // fallback participant-level API
          try {
    ...
    // Sinon on attend l'événement trackSubscribed sur le participant
    try {
...
(async function autoConnectOnLoad(){
  try {
    // Evite de tenter plusieurs connexions
    if (liveRoom) return;

    // court délai pour laisser les libs et supabase s'initialiser
    await waitForLivekit(4000);

    const gs = getGameState();
    if (!gs || !gs.id) return;

    // résoudre identity depuis Supabase session si disponible
    let identity = null;
    try {
      if (window.supabase && window.supabase.auth && typeof window.supabase.auth.getSession === 'function') {
        const sess = await window.supabase.auth.getSession();
    ...
    if (!identity) {
      // si ton app set window.currentUserId ou autre, on peut fallback (ex: window.currentUserId)
