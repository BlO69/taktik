...
// Dynamic loader for LiveKit (unchanged)
...
function ensureSlotMuteButton(slotEl) {
  ...
  slotEl.dataset.lkMuted = 'true'; // par défaut muted = true => évite blocage autoplay
 ...
 *  Remarque importante: on FORCE l'abonnement quand possible (pub.setSubscribed(true))
 *  — ça règle le cas où owner/opponent ne voyaient pas la piste distante.
 */
async function handlePublication(pub, participant, role) {
  try {
    ...
    // Si la piste est déjà présente et peut s'attacher, on attache immédiatement dans le slot correct
    const trackObj = pub?.track ?? pub;
    const gs = getGameState();
    const slotName = participantToSlotIdentity(participant, gs);
    // prefer DOM slot, otherwise create per-participant fallback
    let slotEl = slotName ? getSlotEl(slotName) : null;
    if (!slotEl) {
      // fallback: create container that affichera le flux
    ...
    if (trackObj && typeof trackObj.attach === 'function') {
      ...
      // si track pas encore attachable, s'abonner à l'événement trackSubscribed pour attacher plus tard
      try {
    ...
    // si c'est une vidéo, essayer aussi d'attacher ses pistes audio appartenant au même participant dans le même slot
    try {
...
// --- token fetch / connectAndJoin / publish / stop publish ---
...
async function startPublish() {
  ...
  try {
    ...
    // create local tracks (attempt, but tolerate failures)
    try {
      dbgLog('Creating local video track (will request camera permission if needed)...');
      localVideoTrack = (typeof createLocalVideoTrack === 'function') ? await createLocalVideoTrack() : null;
      dbgLog('Local video track created?', !!localVideoTrack);
    } catch (e) {
      dbgWarn('Failed to create local video track (camera permission?)', e);
      localVideoTrack = null;
    }

    try {
      dbgLog('Creating local audio track (will request microphone permission if needed)...');
      localAudioTrack = (typeof createLocalAudioTrack === 'function') ? await createLocalAudioTrack() : null;
      dbgLog('Local audio track created?', !!localAudioTrack);
    } catch (e) {
    ...
    // preview attach into correct slot (fallback container if DOM slot absent)
...
async function stopPublish() {
  // Allow stopping even if currentlyPublishing flag is inconsistent: attempt best-effort cleanup.
  ...
  try {
    ...
    // if we have a liveRoom and localParticipant, try to unpublish via all available APIs
    if (liveRoom && liveRoom.localParticipant) {
      const pubs = liveRoom.localParticipant.tracks ?? liveRoom.localParticipant.publications ?? null;
      if (pubs) {
        // handle Map / array / forEach shapes
        try {
          if (typeof pubs.forEach === 'function') {
            pubs.forEach(pub => {
              try {
                if (pub.unpublish) pub.unpublish();
                if (pub.unpublishTrack) pub.unpublishTrack();
        ...
        } catch (e) { dbgWarn('Error while iterating/unpublishing pubs', e); }
      }

      // try explicit unpublishTrack if available
      try {
      ...
      } catch (e) { dbgWarn('unpublishTrack failed', e); }

      // older API: localParticipant.unpublish([tracks])
      try {
        if (typeof liveRoom.localParticipant.unpublish === 'function') {
          const toUnpublish = [];
          if (localVideoTrack) toUnpublish.push(localVideoTrack);
          if (localAudioTrack) toUnpublish.push(localAudioTrack);
          if (toUnpublish.length) {
            try { await liveRoom.localParticipant.unpublish(toUnpublish); } catch(e) { dbgWarn('localParticipant.unpublish failed', e); }
          }
        }
      } catch (e) {}
    }

    // stop tracks and detach any attached elements
    ...
    const slotKey = (gs && userIdStr === safeToString(ownerId)) ? 'owner'
                   : (gs && userIdStr === safeToString(opponentId)) ? 'opponent'
                   : 'moderator';
    const slotEl = getSlotEl(slotKey) || document.getElementById(`participant_${sanitizeId(userIdStr)}`);
    clearSlot(slotEl);
    ...
    // clear subscription bookkeeping (client-side)
    subscribedTracksMap.clear();
    subscribedVideoCount = 0;

    dbgLog('Stopped publishing (best-effort)');
...
window.livekit = window.livekit || {
  ...
  toggleSlotMute: (logicalName) => {
    try {
      const slotEl = getSlotEl(logicalName) || document.getElementById(`participant_${sanitizeId(logicalName)}`);
