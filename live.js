...
function spectatorMaySubscribe(pub, participant) {
  ...
  try {
    ...
    // if we can map slot and it's a primary one, always allow
    if (slot && ['owner', 'opponent', 'moderator'].includes(slot)) {
      return true;
    }
  } catch (e) {
  ...
  // If slot not mappable, be permissive: allow initial subscriptions while under limit
...
async function handlePublication(pub, participant, role) {
  try {
    ...
    // After subscribe attempt, attach if available
    const track = pub.track ?? pub;
    ...
    if (track && isNowSubscribed) {
      ...
      // prefer explicit slot elements when mapping available
      let slotEl = (slot ? getSlotEl(slot) : null);

      // if no designated slot DOM element, use per-participant fallback container
      if (!slotEl) {
