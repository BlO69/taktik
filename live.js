...
// S'assure aussi que les flux audio sont attachés et lisibles par tous les participants du même gameId.
...
/** Determine which logical slot a participant corresponds to (owner/opponent/moderator) */
...
/** Ensure there's a video element inside the slot and return it */
...
/**
 * Ensure an <audio> element exists to play audio.
 * Note: autoplay for audio may be blocked by browsers until a user gesture; we call play() and catch errors.
 */
function ensureAudioElement(slotEl) {
  // We'll create an audio element and keep it (hidden in the slot if provided, otherwise in body)
  let audio = null;
  try {
    audio = slotEl ? slotEl.querySelector('audio.live-audio') : document.querySelector('audio.live-audio-global');
    if (audio) {
      audio.autoplay = true;
  ...
  audio.className = slotEl ? 'live-audio' : 'live-audio-global';
  audio.autoplay = true;
  audio.controls = false;
  audio.muted = false; // unmute so audio is audible (browser may block autoplay)
  audio.setAttribute('playsinline', 'true');
  audio.style.display = 'none';
...
/** Attach a given LiveKit track to the DOM slot (handles both video and audio) */
function attachTrackToSlot(track, slotEl, { isLocalPreview = false } = {}) {
  if (!track) return;
  try {
    // LiveKit track shapes vary across versions. Try to detect kind.
    const kind = track.kind ?? (track.track && track.track.kind) ?? (track.mediaStreamTrack && track.mediaStreamTrack.kind) ?? 'unknown';
    ...
    if (kind === 'video') {
      ...
      if (typeof track.attach === 'function') {
        try {
          ...
          // track.attach sometimes returns the element used
          ...
          dbgWarn('track.attach(video) threw', e);
        }
      } else {
        // Fallback: if track has a MediaStream, set srcObject
        try {
          const ms = track.mediaStream ?? track.track?.mediaStream ?? (track.track && new MediaStream([track.track]));
          if (ms) {
            videoEl.srcObject = ms;
            videoEl.play().catch(() => {});
          } else {
            dbgWarn('No attach() and no mediaStream for video track');
          }
        } catch (e) {
          dbgWarn('fallback attach video failed', e);
        }
      }
    } else if (kind === 'audio') {
      // attach audio to a hidden audio element and attempt to play
      ...
      if (typeof track.attach === 'function') {
        try {
          ...
          if (maybeEl && maybeEl !== audioEl) {
            ...
            // try to play
            try { maybeEl.play && maybeEl.play().catch(()=>{}); } catch(_) {}
          } else {
            ...
            audioEl.play().catch((err) => {
              dbgWarn('Audio play() blocked (autoplay policy). Will remain paused until user gesture.', err);
            });
          }
        } catch (e) {
          dbgWarn('track.attach(audio) threw', e);
        }
      } else {
        ...
        try {
          ...
          if (ms) {
            ...
            audioEl.play().catch((err) => {
              dbgWarn('Audio play() blocked (autoplay policy).', err);
            });
          } else {
            dbgWarn('No attach() and no mediaStream for audio track');
          }
        } catch (e) {
          dbgWarn('fallback attach audio failed', e);
        }
      }
    } else {
      // unknown kind: try generic attach
      if (typeof track.attach === 'function') {
        try {
...
/** Remove all media elements from a slot */
function clearSlot(slotEl) {
  if (!slotEl) return;
  try {
    const els = Array.from(slotEl.querySelectorAll('video,canvas,audio,iframe'));
    els.forEach(v => { try { v.remove(); } catch (e) {} });
    slotEl.innerHTML = '';
  } catch (e) {
    slotEl.innerHTML = '';
  }
 ...
 * - Always allow subscriptions to the three primary slots (owner/opponent/moderator), for both audio and video.
 * - For other publications, enforce a client-side limit (3) to avoid overloading.
 */
function spectatorMaySubscribe(pub, participant) {
  ...
  // otherwise enforce the 3-subscription limit (audio/video combined)
...
async function disableSubscriptionOnPub(pub) {
  try {
    ...
    // fallback: detach elements so no audio/video remains playing
    try {
...
/** Handle a single publication according to role (enforce spectator rules but allow main slots) */
async function handlePublication(pub, participant, role) {
  try {
    // Some LiveKit shapes: pub may be a Publication object with isSubscribed, or a plain track
    ...
    if (role === 'spectateur') {
      if (isSubscribed) {
        ...
        // allowed: attempt to subscribe using available APIs
        try {
          if (typeof pub.setSubscribed === 'function') {
            await pub.setSubscribed(true);
          } else if (typeof pub.subscribe === 'function') {
            await pub.subscribe();
          } // else rely on autoSubscribe
        } catch (e) {
...
async function handleParticipant(participant, gameState, role) {
  try {
    ...
    // dynamic events: trackSubscribed / trackUnsubscribed
    // LiveKit event names may differ by version; handle common variants
    try {
      if (participant?.on) {
        ...
        participant.on('trackUnsubscribed', (trackOrPub) => {
          try {
            const pub = trackOrPub?.publication ? trackOrPub.publication : trackOrPub;
            markUnsubscribed(pub);
            const slot = participantToSlotIdentity(participant, gameState);
            const slotEl = getSlotEl(slot);
            // clear slot to avoid stale UI; the UI could be more granular if needed
            clearSlot(slotEl);
        ...
        participant.on('disconnected', () => {
          try {
            ...
            const slotEl = getSlotEl(slot);
            clearSlot(slotEl);
            // remove pubs belonging to this participant
      ...
      dbgWarn('subscribe to participant events failed', e);
    }
...
/** Walk existing participants in the room and handle each */
...
async function connectAndJoin(roomName, identity, gameId) {
  ...
  // Attempt multiple connection shapes for compatibility
...
async function startPublish() {
  ...
  try {
    ...
    // Attach local preview video and audio
