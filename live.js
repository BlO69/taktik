...
function participantToSlotIdentity(participant, gameState) {
  ...
  if (!gameState) return null; // <-- ne plus forcer 'moderator'
...
function spectatorMaySubscribe(pub, participant) {
  ...
  // Allow audio always, allow video always here (we force-subscribe), keeping hook for future limits.
  return true;
}
...
async function handlePublication(pub, participant, role) {
  try {
    ...
    // For all roles we try to ensure subscription — this prevents owner/opponent from missing each other's streams
    try {
      if (!pub?.isSubscribed) {
        if (typeof pub.setSubscribed === 'function') {
    ...
    if (track && isNowSubscribed) {
      ...
      // attach the publication's track
      attachTrackToSlot(track, slotEl, { isLocalPreview: false });
      markSubscribed(pub, participant);

      // if this is a video pub, also attach participant's audio pubs to same slot
      try {
...
async function startPublish() {
  ...
  try {
    ...
    // publish with fallbacks for different LiveKit versions
    if (typeof liveRoom.localParticipant.publishTrack === 'function') {
      try {
        ...
        try {
          if (liveRoom.localParticipant.publishTracks && localVideoTrack) {
            await liveRoom.localParticipant.publishTracks([localVideoTrack]);
          }
        } catch (e) { dbgWarn('publishTracks fallback failed', e); }
      }
      try {
        if (localAudioTrack) await liveRoom.localParticipant.publishTrack(localAudioTrack);
      } catch (err) {
        dbgWarn('publishTrack audio failed, trying publishTracks fallback', err);
        try {
          if (liveRoom.localParticipant.publishTracks && localAudioTrack) {
            await liveRoom.localParticipant.publishTracks([localAudioTrack]);
          }
        } catch (e) { dbgWarn('publishTracks audio fallback failed', e); }
      }
    } else if (typeof liveRoom.localParticipant.publishTracks === 'function') {
      try {
        const arr = [];
        if (localVideoTrack) arr.push(localVideoTrack);
        if (localAudioTrack) arr.push(localAudioTrack);
        if (arr.length) await liveRoom.localParticipant.publishTracks(arr);
      } catch (e) { dbgWarn('publishTracks failed', e); }
    } else {
      dbgWarn('No publish API found on localParticipant');
    }

    // attach local preview to own slot so local user sees their own preview
    try {
      const gs = getGameState();
      const mySlot = participantToSlotIdentity(liveRoom.localParticipant, gs) || 'owner';
      const slotEl = getSlotEl(mySlot) || getOrCreateParticipantContainer(liveRoom.localParticipant);
      if (localVideoTrack) attachTrackToSlot(localVideoTrack, slotEl, { isLocalPreview: true });
      if (localAudioTrack) attachTrackToSlot(localAudioTrack, slotEl, { isLocalPreview: true });
    } catch (e) { dbgWarn('local preview attach failed', e); }

    currentlyPublishing = true;
    dbgLog('startPublish complete');
    return true;
  } catch (e) {
    dbgError('startPublish unexpected error', e);
    throw e;
  }
}
...
async function stopPublish() {
  try {
    ...
    if (liveRoom && liveRoom.localParticipant) {
      ...
      } catch (e) { dbgWarn('unpublishTrack video failed', e); }
      try {
      ...
      } catch (e) { dbgWarn('unpublishTrack audio failed', e); }
    }
    try { localVideoTrack && localVideoTrack.stop && localVideoTrack.stop(); } catch (e) {}
    try { localAudioTrack && localAudioTrack.stop && localAudioTrack.stop(); } catch (e) {}
...
async function disconnectRoom() {
  try {
    ...
    } catch (e) { dbgWarn('room disconnect/leave failed', e); }
    liveRoom = null;
    subscribedTracksMap.clear();
    subscribedVideoCount = 0;
    dbgLog('Disconnected from LiveKit room');
  } catch (e) {
    dbgWarn('disconnectRoom failed', e);
  }
}

// expose some utilities on window for other scripts if needed
window.livekit = window.livekit || {};
window.livekit.waitForReady = waitForLivekit;
...
window.livekit.disconnectRoom = disconnectRoom;

dbgLog('live.js loaded (corrected version)');
