...
function participantToSlotIdentity(participant, gameState) {
  ...
  // If gameState not yet available, return null so we use fallback container instead of forcing 'moderator'
...
function attachTrackToSlot(track, slotEl, { isLocalPreview = false } = {}) {
  ...
  try {
    ...
    if (kind === 'video') {
      // slotEl must be non-null when called; otherwise caller should have provided fallback container
      const videoEl = ensureVideoElement(slotEl, { muted: !!isLocalPreview });
      if (!videoEl) {
        dbgWarn('attachTrackToSlot: videoEl missing, skipping attach');
        return;
      }
      ...
      if (!audioEl) {
        dbgWarn('attachTrackToSlot: audioEl missing, skipping attach');
        return;
      }
      if (typeof track.attach === 'function') {
...
function spectatorMaySubscribe(pub, participant) {
  ...
  try {
    // --- PATCH: allow local owner/opponent to always subscribe ---
    try {
      ...
      if (localIdent && (localIdent === normOwner || localIdent === normOpponent)) {
        // if the local user is owner or opponent, always allow subscribe (they must see each other)
        return true;
      }
    } catch (e) { /* ignore local-ident resolution errors */ }
  ...
  if (kind === 'audio') return true; // always allow audio
  if (subscribedVideoCount < 3) return true; // still apply spectator video limit
  return false;
}
...
async function handlePublication(pub, participant, role) {
  try {
    ...
    if (track && isNowSubscribed) {
      ...
      // prefer DOM named slot, fallback to per-participant container so everyone can see
      let slotEl = getSlotEl(slot);
              ...
              tks.forEach((pubCandidate) => {
                try {
                  if (getPublicationKind(pubCandidate) === 'audio') {
                    ...
                    // attach audio into same slotEl so the mute button controls it
  ...
  try {
    ...
    participant?.on && participant.on('trackUnsubscribed', (publishedTrackOrPub) => {
      try {
        ...
        markUnsubscribed(pub);
        const slot = participantToSlotIdentity(participant, gameState);
        const slotEl = getSlotEl(slot) || document.getElementById(`participant_${sanitizeId(participant?.identity)}`);
        clearSlot(slotEl);
    ...
    participant?.on && participant.on('disconnected', () => {
      try {
        ...
        const slotEl = getSlotEl(slot) || document.getElementById(`participant_${sanitizeId(participant?.identity)}`);
        clearSlot(slotEl);
        // simple sweep: remove pubs of this participant
