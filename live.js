 ...
 * IMPORTANT: retourne null si gameState absent pour permettre le fallback visuel.
 */
function participantToSlotIdentity(participant, gameState) {
  ...
  if (!gameState) return null; // ne force plus 'moderator' quand gameState absent
 ...
 *
 * - Audio subscriptions are always allowed for everyone (so everyone can hear streams).
...
async function handlePublication(pub, participant, role) {
  try {
    ...
    if (track && isNowSubscribed) {
      ...
      // prefer dedicated slot element, otherwise fallback container
  ...
  try {
    ...
    participant?.on && participant.on('trackUnsubscribed', (publishedTrackOrPub) => {
      try {
        ...
        markUnsubscribed(pub);
        const slot = participantToSlotIdentity(participant, gameState);
        const slotEl = slot ? getSlotEl(slot) : document.getElementById(`participant_${sanitizeId(participant?.identity)}`);
