...
// 2) try dynamic import from jsdelivr
...
async function dynamicImportLivekit() {
  ...
  const candidates = [
    'https://cdn.jsdelivr.net/npm/livekit-client@1.4.6/dist/livekit-client.esm.mjs',
    'https://unpkg.com/livekit-client@1.4.6/dist/livekit-client.es.js'
  ];
  ...
  //    This sometimes behaves slightly differently with CORS/redirects.
  try {
    dbgLog('Attempting module-injection fallback to load LivekitClient');
    await new Promise((resolve, reject) => {
      const script = document.createElement('script');
      script.type = 'module';
      // prefer jsdelivr in the injected script
      script.textContent = `
