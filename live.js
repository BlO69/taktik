...
function ensureVideoElement(slotEl, { muted = false } = {}) {
  ...
  // styling that works with Tailwind layout
  video.style.width = '100%';
  video.style.height = '100%';
...
function attachTrackToSlot(track, slotEl, { isLocalPreview = false } = {}) {
  ...
  try {
    ...
    if (typeof track.attach === 'function') {
      try {
        // some implementations accept an element to attach into
        const maybeEl = track.attach(videoEl);
        // if attach returned an element different from videoEl, ensure it's in slot
...
function clearSlot(slotEl) {
  ...
  try {
    ...
    vids.forEach(v => {
      try {
        // LiveKit track.detach may have been used elsewhere; safe remove
        v.remove();
      } catch (e) {}
    });
    slotEl.innerHTML = '';
...
async function handleParticipant(participant, gameState) {
  try {
    iterateParticipantTracks(participant, (pub) => {
      try {
        if (!pub) return;
        // publication object may be { track, isSubscribed }
        if (pub.track && pub.isSubscribed) {
          const slot = participantToSlotIdentity(participant, gameState);
          const slotEl = getSlotEl(slot);
          attachTrackToSlot(pub.track, slotEl, { isLocalPreview: false });
        }
      } catch (_e) {}
    });
...
async function requestLivekitTokenForGame(gameId) {
  ...
  const endpoint = window.LIVEKIT_TOKEN_ENDPOINT || '/functions/v1/get_livekit_token';
...
async function connectAndJoin(roomName, identity, gameId) {
  ...
  if (gameId) {
    ...
    // fallback: attempt standard endpoint with room & identity (but your Edge fn expects game_id)
    const endpoint = window.LIVEKIT_TOKEN_ENDPOINT || '/functions/v1/get_livekit_token';
    const url = new URL(endpoint, window.location.origin);
...
async function startPublish() {
  ...
  try {
    ...
    const slotKey = (gs && userId === (gs.owner_id || gs.ownerId || gs.owner)) ? 'owner'
                   : (gs && userId === (gs.opponent_id || gs.opponentId || gs.opponent)) ? 'opponent'
                   : 'moderator';
          ...
          pubs.forEach(pub => {
            try {
              ...
              if (pub.track && typeof pub.track.detach === 'function') {
                ...
                if (els && els.forEach) els.forEach(el => el.remove());
              }
            } catch(_) {}
          });
