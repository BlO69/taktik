...
async function handlePublication(pub, participant, role) {
  try {
    ...
    if (role === 'spectateur') {
      ...
      // NON-SPECTATORS: ensure we attempt to subscribe if not already (fix owner/opponent not seeing each other)
      if (!isSubscribed) {
        try {
          if (typeof pub.setSubscribed === 'function') {
            await pub.setSubscribed(true);
            dbgLog('Non-spectator: requested setSubscribed(true) for', getPublicationId(pub));
          } else if (typeof pub.subscribe === 'function') {
            await pub.subscribe();
            dbgLog('Non-spectator: requested subscribe() for', getPublicationId(pub));
          }
        } catch (e) {
          dbgWarn('Non-spectator subscribe attempt failed', e);
        }
      }
    }

    // attach if track available and subscription state permits
    const track = pub.track ?? pub;
    ...
    if (track && isNowSubscribed) {
      ...
      // if this was a video pub, attach audio pubs from same participant into same slot
      try {
