...
// --- remplacer spectatorMaySubscribe ---
function spectatorMaySubscribe(pub, participant) {
  ...
  try {
    // résolution safe identité locale
    ...
    // déterminer slot du publisher
    let pubSlot = null;
    try { pubSlot = participantToSlotIdentity(participant, gs); } catch (e) {}
    // fallback via metadata / participantSid / publication owner info
    if (!pubSlot && pub) {
      const pubOwnerIdent = normalizeIdentity(pub.participantIdentity ?? pub.participantSid ?? pub.owner ?? pub.publisher ?? '');
      if (pubOwnerIdent) {
    ...
    if (!localSlot && liveRoom?.localParticipant) {
      try { localSlot = participantToSlotIdentity(liveRoom.localParticipant, gs); } catch (e) {}
    }

    // règle : si local et publisher sont des rôles "privilegiés", allow
    const privileged = ['owner', 'opponent', 'moderator'];
    if (privileged.includes(localSlot) && privileged.includes(pubSlot)) return true;

    // fallback permissif : si la publication est explicitement dans owner/opponent/moderator, allow
...
// --- remplacer handlePublication (version renforcée pour forcer la subscribe chez non-spectateurs) ---
// ---- Remplacer handlePublication par ce bloc ----
async function handlePublication(pub, participant, role) {
  try {
    // debug quick dump to help diagnose shapes
    if (dbg) {
      try {
    ...
    // If this client is a spectator, keep existing limit logic
    if (isLocalSpectator) {
      // reuse your spectator gating function
      if (!(await spectatorMaySubscribe(pub, participant))) {
        dbgLog('Spectator: not allowed to subscribe to', getPublicationId(pub));
        // ensure we disable subscription if already on
        await disableSubscriptionOnPub(pub);
        markUnsubscribed(pub);
        return;
      }
    } else {
      // NON-SPECTATOR: try to ensure subscription is requested for this publication
      try {
        if (!pub?.isSubscribed) {
          if (typeof pub.setSubscribed === 'function') {
            ...
            // last-resort: some clients expose participant-level API
            try {
    ...
    // If pub.track exists and has attach, attach now. Otherwise wait for trackSubscribed event.
    ...
    if (trackObj && typeof trackObj.attach === 'function') {
      ...
      dbgLog('Attached track immediately for', getPublicationId(pub));
      return;
    }

    // If we reach here, pub.track not present (yet). Ensure we rely on trackSubscribed event.
    // Add a one-time listener to this publication / participant to attach when available.
    try {
      // participant-level event (most common)
      if (participant && typeof participant.on === 'function') {
        ...
        try {
          if (participant.on) participant.on('trackSubscribed', onceHandler);
