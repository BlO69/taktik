/* Colle ici le contenu complet corrigé de tournament.js */
/* (Le fichier est long — je fournis la version modifiée en bloc) */
export default async function initTournament(supabaseClient, containerEl) {
  ...
  if (!document.getElementById('trn-styles')) {
    ...
    s.textContent = `
      ...
      .trn-spin{display:inline-block;width:18px;height:18px;border:2px solid #cbd5e1;border-top-color:#6366f1;border-radius:50%;animation:trn-spin .7s linear infinite;flex-shrink:0}
      @keyframes trn-pulse{0%,100%{opacity:1}50%{opacity:.4}}
      .trn-pulse{animation:trn-pulse 1.8s ease-in-out infinite}
      .trn-btn-primary{padding:10px 22px;border:none;border-radius:8px;background:linear-gradient(135deg,#6366f1,#0ea5e9);color:#fff;font-weight:700;font-size:14px;cursor:pointer;transition:opacity .15s}
      .trn-btn-primary:hover{opacity:.88}
      .trn-btn-primary:disabled{opacity:.45;cursor:default}
      .trn-btn-secondary{padding:8px 18px;border:1px solid #e2e8f0;border-radius:8px;background:#fff;color:#475569;font-weight:600;font-size:13px;cursor:pointer;transition:background .15s}
      ...
      .trn-slot{display:inline-flex;align-items:center;gap:7px;padding:6px 12px;border-radius:8px;background:#f8fafc;border:1px solid #e2e8f0;font-size:13px;font-weight:600;color:#64748b;min-width:0}
      ...
      .trn-slot.vs{background:#fefce8;border-color:#fde047;color:#92400e;font-size:11px;font-weight:800;text-transform:uppercase;justify-content:center;min-width:30px;padding:6px 7px}
      .trn-bracket-row{display:flex;align-items:center;gap:5px;flex-wrap:wrap;margin-bottom:8px}
      .trn-round-lbl{font-size:11px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.05em;min-width:48px;flex-shrink:0}
      .trn-match-btn{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border:2px solid #e2e8f0;border-radius:10px;background:#fff;cursor:pointer;transition:border-color .15s,background .15s;width:100%;text-align:left;margin-bottom:8px}
  ...
  function spinner(msg = 'Chargement…') {
    return `<div style="display:flex;align-items:center;gap:10px;padding:24px 0;color:#94a3b8;"><span class="trn-spin"></span><span>${msg}</span></div>`;
  }
  function errBox(msg) {
    return `<div style="padding:16px;background:#fef2f2;border:1px solid #fca5a5;border-radius:10px;color:#dc2626;font-size:13px;">❌ ${msg}</div>`;
  }
  function okBox(msg) {
    return `<div style="padding:12px;background:#ecfdf5;border:1px solid #bbf7d0;border-radius:8px;color:#065f46;font-size:13px;">✅ ${msg}</div>`;
  }
  function fmt(n, d = 0) {
  ...
  /* ========== Helpers: tournament balance ========== */
  async function getTournamentBalance(tournamentId, entry_fee) {
    try {
      // nombre d'inscrits
      const { count: reg_count, error: regErr } = await supabase
        .from('tournament_registrations')
        .select('id', { head: true, count: 'exact' })
        .eq('tournament_id', tournamentId);
      if (regErr) { dbg('getTournamentBalance reg count error', regErr); }

      // somme des prizes déjà enregistrés (déjà distribués)
      const { data: prizes, error: prizeErr } = await supabase
        .from('tournament_prizes')
        .select('amount')
        .eq('tournament_id', tournamentId);
      if (prizeErr) { dbg('getTournamentBalance prizes error', prizeErr); }

      const total_collected = (Number(entry_fee) || 0) * (reg_count ?? 0);
      const total_prizes = (prizes ?? []).reduce((s, r) => s + Number(r.amount || 0), 0);
      const balance = Number((total_collected - total_prizes) ?? 0);
      return { total_collected, total_prizes, balance };
    } catch (e) {
      dbg('getTournamentBalance exception', e);
      return { total_collected: 0, total_prizes: 0, balance: 0 };
    }
  }
  ...
  async function showOverview() {
    ...
    } catch (e) { dbg('getSession error', e); }

    if (!supabase) { render(errBox('Connexion Supabase non disponible.')); return; }

    const myProfile = uid ? await getMyProfile(uid) : null;
    const isAdmin = !!(myProfile && (['administrator','admin','super_admin','superuser'].includes((myProfile.role ?? '').toLowerCase()) || myProfile.is_admin === true || myProfile.can_create_tournament === true));
    ...
    // for each tournament, compute counts and balances (parallel)
    const counts = {};
    ...
    let html = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
      ...
      <div class="trn-small">Format KO · Frais par tournoi affichés</div></div>
    ...
    for (const t of tournaments) {
      ...
      // compute balance for display
      const balInfo = await getTournamentBalance(t.id, t.entry_fee);
      html += `<div class="trn-card" data-tid="${t.id}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <div>
            <div style="font-weight:800;">Tournoi · ${t.max_players} joueurs</div>
            <div class="trn-small">Démarré: ${t.started_at ? new Date(t.started_at).toLocaleString() : '—'}</div>
          </div>
          <div style="text-align:right;">
            <div class="trn-small">Frais ${fmt(t.entry_fee)} TIKS</div>
            <div class="trn-small">🥇 ${fmt(t.prize_first)} · 🥈 ${fmt(t.prize_second)}</div>
            <div class="trn-small" style="margin-top:6px;">Balance: <strong>${fmt(balInfo.balance)}</strong> TIKS</div>
            ${isAdmin && t.status === 'finished' ? `<div style="margin-top:6px;"><button class="trn-btn-primary trn-withdraw-btn" data-tid="${t.id}">Retirer</button></div>` : ''}
          </div>
        </div>
    ...
    root.querySelectorAll('.trn-view-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // bracket uses polling by default
    ...
    // withdraw listeners (admin)
    root.querySelectorAll('.trn-withdraw-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          const { data, error } = await supabase.rpc('rpc_withdraw_tournament_funds', { p_tournament_id: tid, p_admin_fee_percent: 2.5 });
          if (error) throw error;
          alert('Retrait effectué : ' + JSON.stringify(data));
          // rerender overview to update balances
          await showOverview();
        } catch (err) {
    ...
    try {
      ...
      if (error) {
        ...
        if (msg.toLowerCase().includes('insuffisant') || msg.toLowerCase().includes('insufficient')) {
          render(`${errBox(msg)}<button class="trn-btn-secondary" style="margin-top:12px;" id="trn-back-btn">← Retour</button>`);
      ...
      render(`${errBox(e?.message ?? String(e))}<button class="trn-btn-secondary" style="margin-top:12px;" id="trn-back-btn">← Retour</button>`);
      root.querySelector('#trn-back-btn')?.addEventListener('click', showOverview);
    }
  }

  async function createAndJoin() { await joinTournament(null, 20); }
    ...
    const html = `
      <div>
        ...
        <div class="trn-card" style="margin-bottom:14px;">
          <div style="font-size:12px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:10px;">Bracket en cours de formation</div>
          ${bracketHtml}
        ...
        <div style="padding:14px 16px;background:#eff6ff;border:1px solid #93c5fd;border-radius:10px;text-align:center;">
          ...
          <div style="font-size:12px;color:#3b82f6;margin-top:6px;">Le tournoi démarrera automatiquement dès que ${t.max_players} joueurs seront prêts.</div>
        </div>
      </div>`;
  ...
  async function showBracket(tid, subscribe = false) {
    ...
    // Use polling for bracket refresh (every 5s). We keep realtime for matches
    if (subscribe) {
  ...
  async function renderBracketView(tid, uid) {
    ...
    const { data: matches } = await supabase.from('tournament_matches').select('*').eq('tournament_id', tid).order('round_number').order('match_index');
    const { data: regs } = await supabase.from('tournament_registrations').select('user_id,slot').eq('tournament_id', tid);

    const allMatches = matches ?? [];
    const allRegs    = regs ?? [];

    // profiles
    const allUids = [...new Set([ ...allRegs.map(r=>r.user_id), ...allMatches.flatMap(m => [m.player1_id, m.player2_id, m.winner_id].filter(Boolean)), ])];
    const profiles = await getProfiles(allUids);
    const pseudo = id => profiles.find(p => p.id === id)?.pseudo ?? (id ? id.slice(0,8) + '…' : '?');

    const myActiveMatch = allMatches.find(m => m.status === 'active' && (m.player1_id === uid || m.player2_id === uid));
    const isFinished = t?.status === 'finished';
    const statusBadge = isFinished ? `<span class="trn-badge" style="background:#dcfce7;color:#16a34a;">Terminé 🏁</span>` : `<span class="trn-badge" style="background:#dcfce7;color:#16a34a;">En cours ⚔️</span>`;
        ...
        <button class="trn-match-btn ${cls}" ${m.status !== 'active' || !isMe ? 'disabled style="cursor:default"' : ''} data-tmid="${m.id}">
          <div>
            <div style="font-size:13px;font-weight:700;color:#1e293b;margin-bottom:3px;">
              ${p1 ?? '?'} <span style="color:#94a3b8;font-weight:400;">vs</span> ${p2 ?? '?'} ${winnerLabel}
            </div>
    ...
    const alertHtml = myActiveMatch && !isFinished ? `<div style="padding:14px 16px;background:#f0fdf4;border:2px solid #22c55e;border-radius:10px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:16px;">
      <div><div style="font-size:14px;font-weight:700;color:#15803d;">⚔️ Ton match t'attend !</div><div style="font-size:12px;color:#16a34a;margin-top:2px;">Tu affrontes <strong>${pseudo(myActiveMatch.player1_id === uid ? myActiveMatch.player2_id : myActiveMatch.player1_id)}</strong></div></div>
      <button class="trn-btn-primary trn-play-btn" data-tmid="${myActiveMatch.id}" style="padding:10px 20px;">▶ Jouer maintenant</button>
    </div>` : '';

    const resultHtml = isFinished && t?.winner_id ? `<div style="padding:16px;background:#fefce8;border:1px solid #fde68a;border-radius:10px;text-align:center;margin-bottom:16px;">
      <div style="font-size:22px;margin-bottom:6px;">🎉</div><div style="font-size:15px;font-weight:800;color:#713f12;">Tournoi terminé !</div>
      <div style="font-size:13px;color:#92400e;margin-top:4px;">🥇 <strong>${pseudo(t.winner_id)}</strong> ${t.runner_up_id ? ` · 🥈 <strong>${pseudo(t.runner_up_id)}</strong>` : ''}</div>
    </div>` : '';
    ...
    const withdrawBtnHtml = isAdmin && t?.status === 'finished' ? `<button id="trn-withdraw-btn" class="trn-btn-primary">Retirer (${fmt(balInfo.balance)} TIKS)</button>` : '';
    ...
    const html = `
      <div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap;">
          ...
          <div style="margin-left:auto;font-size:12px;color:#64748b;">Balance: <strong>${fmt(balInfo.balance)} TIKS</strong></div>
          ${withdrawBtnHtml}
        </div>
        ...
        <div class="trn-card"><div style="font-size:12px;font-weight:700;color:#94a34b;text-transform:uppercase;margin-bottom:12px;">Demi-finales / Premiers matches</div>${semis.map(m=>matchBlock(m)).join('') || '<div style="color:#94a3b8;font-size:13px;">En attente des matchs…</div>'}</div>

        <div class="trn-card" style="margin-top:12px;"><div style="font-size:12px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:12px;">Reste du tableau</div>${matchBlock(finale)}</div>

        <div class="trn-card" style="margin-top:12px;"><div style="font-size:12px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:10px;">Participants</div>
          <div style="display:flex;flex-wrap:wrap;gap:8px;">${allRegs.sort((a,b)=>a.slot-b.slot).map(r=>`<span class="trn-slot ${r.user_id===uid?'me':'filled'}">${r.user_id===uid?'👤':'🎮'} ${pseudo(r.user_id)} <span style="opacity:.5;font-size:10px;">#${r.slot}</span></span>`).join('')}</div>
        </div>
      </div>`;
    ...
    root.querySelector('#trn-withdraw-btn')?.addEventListener('click', async (e) => {
      ...
      e.target.textContent = 'Retrait en cours…';
      try {
        const { data, error } = await supabase.rpc('rpc_withdraw_tournament_funds', { p_tournament_id: tid, p_admin_fee_percent: 2.5 });
        if (error) throw error;
        alert('Retrait effectué : ' + JSON.stringify(data));
        await renderBracketView(tid, uid); // refresh view
      } catch (err) {
        dbg('withdraw error', err);
        alert('Erreur retrait : ' + (err?.message ?? String(err)));
        e.target.disabled = false;
        e.target.textContent = `Retirer (${fmt(balInfo.balance)} TIKS)`;
      }
    });
  ...
  async function launchGame(tournamentMatchId, uid, tid) {
    ...
    render(`<div style="padding:32px;text-align:center;"><span class="trn-spin" style="width:28px;height:28px;border-width:3px;margin-bottom:14px;display:inline-block;"></span><div style="font-size:15px;font-weight:700;color:#1e293b;margin-bottom:6px;">Préparation du match…</div><div style="font-size:13px;color:#64748b;">Connexion au jeu en cours.</div></div>`);
    try {
      const { data: tmatch } = await supabase.from('tournament_matches').select('*').eq('id', tournamentMatchId).maybeSingle();
      if (!tmatch) throw new Error('Match introuvable.');
      if (tmatch.status === 'finished') {
        render(`${errBox('Ce match est déjà terminé.')}<button class="trn-btn-secondary" style="margin-top:12px;" id="trn-back-btn">← Retour au bracket</button>`);
      ...
      if (!matchId) {
        if (tmatch.player1_id === uid) {
          ...
          const { data: mRow, error: mErr } = await supabase.from('matches')
            .insert({ owner_id: tmatch.player1_id, opponent_id: tmatch.player2_id, status: 'ongoing', metadata: { tournament_id: tmatch.tournament_id, tournament_match_id: tmatch.id } })
            .select('id').single();
          if (mErr || !mRow) { dbg('create match error', mErr); throw new Error('Erreur création match : ' + (mErr?.message ?? 'inconnu')); }
          matchId = mRow.id;
          ...
          const { data: gRow, error: gErr } = await supabase.from('games')
            ...
            .select('id').single();
          if (gErr || !gRow) { dbg('create game error', gErr); throw new Error('Erreur création game : ' + (gErr?.message ?? 'inconnu')); }
          gameId = gRow.id;
          ...
          while (!matchId && Date.now() < deadline) {
            ...
            const { data: latest } = await supabase.from('tournament_matches').select('match_id').eq('id', tmatch.id).maybeSingle();
      ...
      render(`${errBox(e?.message ?? String(e))}<button class="trn-btn-secondary" style="margin-top:12px;" id="trn-back-btn">← Retour au bracket</button>`);
      root.querySelector('#trn-back-btn')?.addEventListener('click', () => showBracket(tid, true));
    }
  }
  ...
  async function init() {
    ...
    if (uid) {
      try {
        const { data: myRegs } = await supabase.from('tournament_registrations').select('tournament_id, tournaments!inner(status)').eq('user_id', uid).in('tournaments.status', ['waiting','active']).limit(1).maybeSingle();
        if (myRegs) {
        ...
        try {
          const { data: myRegs2 } = await supabase.from('tournament_registrations').select('tournament_id, tournaments!inner(status)').eq('user_id', uid).in('tournaments.status', ['active']).limit(1).maybeSingle();
          if (myRegs2) {
...
export async function renderHomePreview(previewEl, supabaseClient) {
  ...
  try {
    ...
    const t = data ?? [];
    if (!t.length) { previewEl.textContent = 'Aucune compétition pour l\'instant'; return; }
    previewEl.innerHTML = t.map(x => `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f1f5f9;"><div style="font-weight:600;">Tournoi · ${x.max_players} joueurs</div><div style="color:#64748b;">Frais ${Number(x.entry_fee).toLocaleString('fr-FR')}</div></div>`).join('');
  } catch (e) {
