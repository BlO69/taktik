...
export default async function initTournament(supabaseClient, containerEl) {
  ...
  // fallback si tout se bloque : afficher "Aucune compétition" après 7s
  let timedOut = false;
  const fallbackTimeout = setTimeout(() => {
    timedOut = true;
    dbg('showOverview: fallback timeout triggered');
    render(`<div class="trn-card" style="text-align:center;color:#94a3b8;padding:32px;">
      <div style="font-size:32px;margin-bottom:8px;">🎮</div>
      <div style="font-weight:600;margin-bottom:6px;">Aucune compétition en cours</div>
      <div style="font-size:12px;color:#64748b;">Reviens bientôt</div>
    </div>`);
  }, 7000);
  ...
  try {
    ...
    console.error('[tournament] getSession failed', e);
    dbg('getSession error', e);
    // continue même si la session a échoué (on considère utilisateur non connecté)
    uid = null;
  }
  ...
  let myProfile = null;
  try {
    myProfile = uid ? await getMyProfile(uid) : null;
  } catch (e) {
    console.error('[tournament] getMyProfile failed', e);
    dbg('getMyProfile error', e);
    myProfile = null;
  }
  ...
  try {
    ...
    console.error('[tournament] tournaments query failed (first try)', e);
    ...
    try {
      ...
      console.error('[tournament] tournaments query failed (second try)', e2);
  ...
  // Si on est arrivé ici, tout va bien : on peut annuler le fallback
  clearTimeout(fallbackTimeout);
  if (timedOut) {
    // le fallback a déjà rendu quelque chose ; continuer quand même (le prochain render remplacera)
    dbg('showOverview: timedOut was true but continuing to render final view');
  }
