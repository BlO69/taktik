...
export default async function initTournament(supabaseClient, containerEl) {
  ...
  async function getMyProfile(uid) {
    if (!uid) return null;
    const { data, error } = await supabase.from('user_profiles').select('id,pseudo,role').eq('id', uid).maybeSingle();
    if (error) {
  ...
  async function showOverview() {
    ...
    const isAdmin = myProfile?.role === 'administrator';
    ...
    /* Rendu */
    // En-tête : afficher le bouton "Créer un tournoi" pour les admins (même si des tournois existent)
    let html = `
  ...
  async function createTournament() {
    ...
    try {
      ...
      if (error || !data) {
        throw new Error(error?.message ?? 'Erreur création tournoi');
      }
      // Afficher la salle d'attente du tournoi créé
      await showCheckin(data.id);
    } catch (e) {
