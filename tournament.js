...
export default async function initTournament(supabaseClient, containerEl) {
  ...
  function computePrizesFromTemplate(tmpl) {
    ...
    const distributable = pool - adminShare;
    const first  = Math.floor((distributable * (Number(tmpl.prize_pct_first)  || 0) / 100) / 10) * 10;
    const second = Math.floor((distributable * (Number(tmpl.prize_pct_second) || 0) / 100) / 10) * 10;
    const third  = distributable - first - second;
    return { pool, adminShare, prize_first: first, prize_second: second, prize_third: Math.max(third, 0) };
  }
