/**
 * tournament.js — Module compétitions Taktik (version corrigée)
 *
 * Modifications / objectifs demandés :
 *  - showBracket utilise désormais polling (setInterval) au lieu de la subscription realtime
 *  - getTournamentBalance() ajouté (calcul balance côté client)
 *  - affichage Balance compétition dans overview + bracket
 *  - bouton Retirer visible aux admins pour tournoi finished ; appelle rpc_withdraw_tournament_funds
 *  - les checks d’inscription se basent sur entry_fee (et non sur une valeur hardcodée)
 *
 * Le reste du comportement original est conservé.
 */
...
export default async function initTournament(supabaseClient, containerEl) {
  ...
  /* ===== Nouveau helper : calculer la "balance" d'un tournoi côté client =====
     balance = total_collected (entry_fee * registrations) - sum(prizes_amount)
     Renvoie { total_collected, total_prizes, balance }
  */
  async function getTournamentBalance(tournamentId, entry_fee = 0) {
    try {
      ...
      const balance = Number((totalCollected - totalPrizes) ?? 0);
      return { totalCollected, totalPrizes, balance };
    } catch (e) {
  ...
  async function showOverview() {
    ...
    try {
      const { data, error } = await supabase
        .from('tournaments')
        .select('id,max_players,status,entry_fee,prize_first,prize_second,prize_third,fund_withdrawn,created_at,started_at')
    ...
    // Compter inscriptions pour chaque tournoi (simple) and compute balance
    const counts = {};
    ...
    // Render each tournament and include balance (calculated)
    for (const t of tournaments) {
      const n = counts[t.id] ?? 0;
      const pct = Math.round(n / (t.max_players || 4) * 100);
      // compute balance for display
      let balInfo = { totalCollected: 0, totalPrizes: 0, balance: 0 };
      try { balInfo = await getTournamentBalance(t.id, t.entry_fee); } catch (e) { dbg('balance calc error', e); }
      const withdrawnLabel = t.fund_withdrawn ? `<div style="font-size:12px;color:#64748b;margin-top:6px;">Fonds retirés</div>` : '';
      ...
      html += `<div class="trn-card" data-tid="${t.id}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          ...
          <div style="text-align:right;">
            ...
            <div style="margin-top:6px;font-size:13px;color:#64748b;">Balance tournoi: <strong>${fmt(balInfo.balance)}</strong> TIKS</div>
            ${withdrawnLabel}
            ${isAdmin && t.status === 'finished' && !t.fund_withdrawn ? `<div style="margin-top:8px;"><button class="trn-btn-primary trn-withdraw-btn" data-tid="${t.id}">Retirer (${fmt(balInfo.balance)} TIKS)</button></div>` : ''}
          </div>
        </div>
    ...
    root.querySelectorAll('.trn-withdraw-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          // rerender overview to update balances / buttons
          await showOverview();
        } catch (err) {
  ...
  /* ══════════════════════════════════════════════════════════════════
     ...
     (on laisse la logique d'origine, mais les checks UX se basent sur entry_fee)
  ══════════════════════════════════════════════════════════════════ */
  ...
  async function showCheckin(tid) {
    ...
    /* Realtime (utile pour inscriptions) */
    try {
  ...
  /* ══════════════════════════════════════════════════════════════════
     VUE 3 — BRACKET / COMPÉTITION (POLLING pour le rafraîchissement)
  ══════════════════════════════════════════════════════════════════ */

  let _bracketChannel = null; // conservé pour compat mais nous n'utilisons pas channel pour bracket refresh
  ...
  async function showBracket(tid, subscribe = false) {
    ...
    // Utiliser polling pour rafraîchir le bracket (toutes les 5s)
    if (subscribe) {
  ...
  async function renderBracketView(tid, uid) {
    ...
    // balance info (calcul côté client)
    ...
    const withdrawBtnHtml = isAdmin && t?.status === 'finished' && !t?.fund_withdrawn
      ? `<button id="trn-withdraw-btn" class="trn-btn-primary">Retirer (${fmt(balInfo.balance)} TIKS)</button>`
    ...
    const html = `
      <div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap;">
          ...
          <div style="margin-left:auto;font-size:12px;color:#64748b;">Balance: <strong>${fmt(balInfo.balance)}</strong> TIKS</div>
          <div>${withdrawBtnHtml}</div>
        </div>
    ...
    root.querySelector('#trn-withdraw-btn')?.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (error) throw error;
        alert('Retrait effectué : ' + JSON.stringify(data));
        // refresh bracket (balance & button)
        await renderBracketView(tid, uid);
      } catch (err) {
        dbg('withdraw error', err);
        alert('Erreur retrait : ' + (err?.message ?? String(err)));
