...
function applyVideoToggle(id, enabled, logicalName) {
  ...
  try {
    // prefer window.livekit API (live.js exposes window.livekit)
    if (window.livekit && typeof window.livekit.toggleVideo === 'function') {
      ...
      // backward compatibility if something exposes liveManager
...
// -----------------------------
// Toggle both wrapper (card) and the actual slot that live.js uses.
// This ensures a) the video slot is visible/hidden and b) live.js attachments match visibility.
document.addEventListener('display:video1', (e) => {
  const enabled = !!(e && e.detail && e.detail.enabled);
  applyVideoToggle('videoCardPlayer1', enabled, 'video1'); // wrapper
  applyVideoToggle('player1Video', enabled, 'video1');     // actual slot used by live.js
});
...
document.addEventListener('display:video3', (e) => {
  ...
  applyVideoToggle('moderatorVideo', enabled, 'video3'); // slot id in HTML
});
