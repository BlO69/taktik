...
// Efface les flags localStorage pour une série terminée afin que la modale puisse
// s'afficher à nouveau lors d'une prochaine série (même series_id ou nouveau).
function clearSeriesModalMarkers(seriesId) {
  try {
    const sid = (seriesId == null || seriesId === '') ? '__none__' : String(seriesId);
    try {
      localStorage.removeItem(`taktik_modal_shown_seriesFinishedShownForSeriesId_${sid}`);
      localStorage.removeItem(`taktik_modal_dismissed_seriesFinishedShownForSeriesId_${sid}`);
    } catch(e) { dbgLog('clearSeriesModalMarkers localStorage failed', e); }
    state.seriesFinishedShownForSeriesId = null;
    state.seriesFinishedShownForSeriesIdDismissed = null;
    dbgLog('clearSeriesModalMarkers done', sid);
  } catch(e) { dbgLog('clearSeriesModalMarkers failed', e); }
}
...
if (!tryMarkModalShown('newRoundShownForGameId', g.id) && !amLocalWinner) {
  ...
  dbgLog('auto-join + showNewRoundModal block failed', e);
    ...
    // Si la série est terminée, laisser showSeriesModal gérer l'idempotence (gagnant toujours affiché)
    if (state.series_winner) {
      ...
      } catch(e) { dbgLog('showSeriesModal (handleGlobalNewGameCreated) failed', e); }
    }
        ...
        if (state.outcomeShownForGameId !== gameId) {
          ...
          try {
            ...
            if (state.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
             ...
             // showSeriesModal gère l'idempotence en interne
try {
  showSeriesModal({
    series_id: state.series_id,
    series_owner_wins: state.series_owner_wins,
...
} catch(e) { dbgLog('showSeriesModal (pollGameOnce outcome path) failed', e); }
...
clearSeriesModalMarkers(state.series_id);
  ...
  dbgLog('enqueue new-round modal failed', e);
    ...
    // Si la série est terminée, afficher la modale série (gagnant : toujours, autres : idempotent)
if (finalizeRow.series_winner) {
  try {
    showSeriesModal({
      ...
      series_winner_pseudo: (finalizeRow.series_winner && (String(finalizeRow.series_winner) === String(state.ownerId) ? state.ownerProfile?.pseudo : state.opponentProfile?.pseudo)) || null,
  ...
  } catch(e){ dbgLog('showSeriesModal (finalize) failed', e); }
...
const markDismissed = () => {
  try {
   ...
   // Gagnant de la série : on affiche TOUJOURS la modale (pas de blocage localStorage).
// Non-gagnant : idempotence standard via tryMarkModalShown.
...
if (amUserSeriesWinner) {
  // Pour le gagnant : effacer les flags localStorage résiduels (série précédente avec même id)
  // puis forcer le marqueur en mémoire pour que l'affichage ne soit jamais bloqué.
  try { clearSeriesModalMarkers(intendedSeriesId); } catch(e) { dbgLog('clearSeriesModalMarkers in showSeriesModal failed', e); }
...
// --- CLEAR: effacer les flags localStorage de l'ancienne série pour que la modale
// s'affiche correctement pour la prochaine série (y compris si même series_id réutilisé).
try {
  clearSeriesModalMarkers(parentSeriesId);
} catch(e) { 
      ...
      if (state.series_id) {
        ...
        schan.on('postgres_changes', {
          ...
          try {
            ...
            updatePlayerUI();
            // Afficher la modale série si un gagnant est déclaré
            // showSeriesModal gère l'idempotence en interne (gagnant : toujours, autres : une seule fois)
            if (state.series_winner) {
              try {
                showSeriesModal({
                  series_id: state.series_id,
              ...
              } catch(e){ dbgLog('showSeriesModal (subscription) failed', e); }
            } // end if (state.series_winner)
...
if (state._pendingSeriesModal && state._pendingSeriesModal.length) {
  for (const item of state._pendingSeriesModal) {
    try {
  // showSeriesModal gère l'idempotence en interne (gagnant : toujours, autres : une seule fois)
  try { showSeriesModal(item.info); } catch(e){ dbgLog('showSeriesModal (pending) failed', e); }
