...
// Fix : gagnant d'une manche → modale "Nouvelle manche" directe (plus de double-modale) ;
//       gagnant de la série (3 victoires, best-of-5) → modale "Série terminée" directe ;
//       perdant → modale defeat + defeat.wav (une seule fois), jamais la modale série.
...
async function fetchAndApplyPartyAndSeries(partyId, seriesId) {
  ...
  try {
    ...
    if (prevGameId) {
      // Depuis le fast-path gagnant dans showOutcomeModal, le gagnant a déjà finalisé
      // et son outcomeShownForGameId est positionné. On marque seulement si pas encore fait
      // ou si l'utilisateur n'est pas le gagnant local.
try {
  ...
  if (state.outcomeShownForGameId || !amLocalWinner) {
    ...
    dbgLog('skip pre-mark outcome for prevGame (local winner already auto-finalized)', prevGameId);
  }
} catch (e) {
...
async function pollPartyNewGameOnce() {
  ...
  try {
    ...
    if (newGameId) {
      ...
      const prevGameId = gameId;
      if (prevGameId) {
        // Le gagnant a déjà finalisé via le fast-path dans showOutcomeModal.
        // outcomeShownForGameId est donc déjà positionné. On marque quand même
        // pour les clients non-gagnants.
try {
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  if (state.outcomeShownForGameId || !amLocalWinner) {
    state.outcomeShownForGameId = String(prevGameId);
...
const markDismissed = () => {
  try {
    ...
    // --- PERDANT : ne pas afficher la modale "Série terminée" --- 
    // Le perdant voit déjà la modale defeat (showOutcomeModal). Seul le gagnant de la série
    // et les spectateurs voient la modale "Série terminée".
    const _isPlayer = state.userId && (String(state.userId) === String(state.ownerId) || String(state.userId) === String(state.opponentId));
    if (_isPlayer && info.series_winner && String(state.userId) !== String(info.series_winner)) {
      dbgLog('suppress series modal for loser (they see defeat modal instead)', state.userId);
      // marquer comme vu quand même pour éviter qu'un autre handler re-essaie de l'afficher
...
const prevGameId = gameId;
    if (prevGameId) {
      ...
      // Le gagnant a déjà finalisé via fast-path ; outcomeShownForGameId est déjà positionné.
try {
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  if (state.outcomeShownForGameId || !amLocalWinner) {
    state.outcomeShownForGameId = String(prevGameId);
...
if (prev) prev.remove();
    ...
    // --- GAGNANT DE MANCHE : fast-path ---
    // Le gagnant ne voit PAS la modale outcome : on finalise automatiquement en arrière-plan,
    // puis on affiche directement "Nouvelle manche" (manche non-série) ou "Série terminée".
    const amWinnerNow = amPlayer && actualWinnerId && state.userId && String(state.userId) === String(actualWinnerId);
    if (amWinnerNow) {
      // Jouer le son de victoire immédiatement
      try { playSound(VICTORY_SOUND); } catch(e) { dbgLog('victory sound failed', e); }
      // Finaliser la manche côté serveur, puis afficher la bonne modale au gagnant
      handleFinalizeRoundAndFollowup(String(targetGameId), { starterChar: null })
        .then(finalizeRow => {
          try {
            if (finalizeRow && finalizeRow.series_winner) {
              // La série est terminée : showSeriesModal a déjà été appelé depuis handleFinalizeRoundAndFollowup
              dbgLog('winner fast-path: series finished, series modal handled inside finalize');
            } else {
              // Manche non-décisive : afficher directement la modale "Nouvelle manche" au gagnant
              showNewRoundModal({
                party_id: state.party_id,
                party_owner_wins: state.party_owner_wins,
                party_opponent_wins: state.party_opponent_wins,
          ...
          } catch(e) { dbgLog('winner post-finalize modal failed', e); }
        })
        .catch(e => {
          dbgLog('winner auto-finalize failed in showOutcomeModal', e);
          try { pollGameOnce(); } catch(_) {}
        });
      return; // ne pas afficher la modale outcome pour le gagnant
    }
...
try {
  ...
  state._newRoundShownAt = null;
    ...
    // --- Son defeat pour le perdant (joué une seule fois, immédiatement à l'affichage de la modale) ---
    if (amLoser) {
      try { playSound(DEFEAT_SOUND); } catch(e) { dbgLog('defeat sound failed', e); }
    }
