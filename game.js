...
if (!state.lastPartyKnownGameId) {
  // propagate match_id from the polled game (useful when arriving by polling)
  if (g.match_id) {
    // only overwrite if we don't already have one (optional); remove the condition if you want always-to-sync
    if (!state.match_id) {
      state.match_id = String(g.match_id);
    } else {
      // keep the existing state.match_id but ensure it's a string
      state.match_id = String(state.match_id);
    }
    exposeGameStateToWindow();
  }

  // use the canonical id returned by the poll (g.id). fallback to outer gameId only if missing.
  state.lastPartyKnownGameId = String(g.id ?? gameId ?? '');
}
