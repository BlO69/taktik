...
// Si on est en train de joindre/traiter localement ce même newGameId (gagnant qui vient de créer/join),
// ignorer l'événement global pour éviter double-reset / double-update du board / scores.
if (state._joiningGameId && String(state._joiningGameId) === String(g.id)) {
  dbgLog('handleGlobalNewGameCreated: skipping because local join/create in progress for', g.id);
  return;
}
...
try {
  ...
  async function createNewSeriesFromSeries(parentSeriesId, lastRoundWinner = null) {
    try {
      ...
      // (insérer ligne juste après `const newGameId = resp.new_game_id;`)
state._joiningGameId = String(newGameId); // marque qu'on rejoint/créé activement ce gameId (évite les races)
