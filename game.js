...
//
// === PATCHES appliqués (2024) ===
// PATCH 1 : showOutcomeModal snapshot-safe — accepte opts complet (winnerId, winnerPseudo,
//           ownerId, opponentId, ownerPseudo, opponentPseudo, forGameId) ; state.* en fallback
// PATCH 2 : Tous les appels à showOutcomeModal passent désormais explicitement owner/opponent
//           ids + pseudos + winnerId/winnerPseudo + forGameId
// PATCH 3 : handleFinalizeRoundAndFollowup — si series_winner présent dans finalizeRow,
//           appelle showSeriesModal avec snapshot complet (try/catch non-bloquant)
// PATCH 4 : fetchAndApplyPartyAndSeries — après avoir appliqué seriesRow, si winner_id existe
//           et correspond à l'utilisateur local, propose showSeriesModal (snapshot, idempotent)
// PATCH 5 : tryMarkModalShown / wasModalDismissed — ne pré-marque jamais la série si
//           l'utilisateur est le gagnant (il doit voir la modale et cliquer)
// PATCH 6 : dbgLog contextualisés autour des paths critiques (showOutcomeModal, showSeriesModal,
//           marquages)
// PATCH 7 : compatibilité préservée — polling/realtime et queues _pendingNewRoundModal /
//           _pendingSeriesModal intacts
...
// Retourne true si on vient de marquer, false si déjà marqué.
// PATCH 5 : pour la clé 'seriesFinishedShownForSeriesId', ne jamais marquer si
//           l'utilisateur courant est le gagnant de la série (il doit voir la modale).
function tryMarkModalShown(stateKey, id, { skipIfLocalWinner = false, seriesWinnerId = null } = {}) {
  try {
    const sid = id == null ? '' : String(id);
    if (state[stateKey] && String(state[stateKey]) === sid) return false;

    // PATCH 5 : protection gagnant local pour la série
    if (skipIfLocalWinner && seriesWinnerId && state.userId &&
        String(state.userId) === String(seriesWinnerId)) {
      dbgLog('[PATCH5] tryMarkModalShown: skip pre-mark for series winner (local winner must see modal)', stateKey, sid);
      return false;
    }

    state[stateKey] = sid;
    // si on marque la NEW-ROUND, mémoriser l'instant pour éviter que ce flag ancien
    // bloque ultérieurement l'affichage d'un outcome légitime
    if (stateKey === 'newRoundShownForGameId') state._newRoundShownAt = Date.now();
    dbgLog('[PATCH6] tryMarkModalShown marked', stateKey, '=', sid);
    return true;
  } catch (e) {
  ...
  async function placeMoveRPC(pos, row, col, playerChar) {
    ...
    try {
      ...
      if (gameFinished) {
        ...
        // PATCH 2 : passer snapshot complet
        if (state.outcomeShownForGameId !== gameId) {
          showOutcomeModal({
            winnerId:      state.winnerId,
            winnerPseudo:  state.winnerPseudo,
            ownerId:       state.ownerId,
            opponentId:    state.opponentId,
            ownerPseudo:   state.ownerProfile?.pseudo ?? state.ownerPseudo ?? null,
            opponentPseudo: state.opponentProfile?.pseudo ?? state.opponentPseudo ?? null,
            forGameId:     gameId
          });
          state.outcomeShownForGameId = gameId;
          dbgLog('[PATCH 2] showOutcomeModal from placeMoveRPC', gameId);
        }
        ...
        (async () => {
          ...
          if (rec.status === 'finished') {
            ...
            if (state.outcomeShownForGameId !== gameId) {
              ...
              state.outcomeShownForGameId = gameId;
              dbgLog('[PATCH 2] showOutcomeModal from realtime games UPDATE', rec.id);
            }
          } else {
            updateTurnUI();
          }
  ...
  try {
    ...
    if (sid) {
      ...
      if (!serr && seriesRow) {
        ...
        // PATCH 4 : si la série est terminée et l'utilisateur local est le gagnant,
        // afficher showSeriesModal (snapshot) si la modal n'a pas encore été montrée.
        // PATCH 5 : NE PAS pré-marquer si l'utilisateur local est le gagnant — il doit voir et cliquer.
        if (seriesRow.winner_id && state.userId) {
          const amLocalWinnerHere = String(state.userId) === String(seriesRow.winner_id);
          const alreadyShown = state.seriesFinishedShownForSeriesId
            && String(state.seriesFinishedShownForSeriesId) === String(seriesRow.id);
          dbgLog('[PATCH 4] seriesRow winner check', { seriesId: seriesRow.id, winner: seriesRow.winner_id, amLocalWinnerHere, alreadyShown });
          if (!alreadyShown) {
            if (amLocalWinnerHere) {
              // PATCH 5 : gagnant local — ne pas pré-marquer, montrer directement
              dbgLog('[PATCH 4/5] local winner — showing series modal without pre-marking', seriesRow.id);
              try {
                const seriesSnap4 = {
                  series_id:           seriesRow.id,
                  series_owner_wins:   state.series_owner_wins,
                  series_opponent_wins: state.series_opponent_wins,
                  series_winner:       seriesRow.winner_id,
                  series_winner_pseudo: (String(seriesRow.winner_id) === String(state.ownerId)
                    ? (state.ownerProfile?.pseudo ?? state.ownerPseudo ?? null)
                    : (state.opponentProfile?.pseudo ?? state.opponentPseudo ?? null)),
                  owner_id:            state.ownerId,
                  opponent_id:         state.opponentId,
                  owner_pseudo:        state.ownerProfile?.pseudo ?? state.ownerPseudo ?? null,
                  opponent_pseudo:     state.opponentProfile?.pseudo ?? state.opponentPseudo ?? null,
                ...
                showSeriesModal(seriesSnap4);
              } catch (e) {
                dbgLog('[PATCH 4] showSeriesModal for local winner failed', e);
              }
            }
            // Pour les non-gagnants, showSeriesModal est déclenché par les autres paths (finalize/realtime)
          }
        }
      }
    }
...
if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
  ...
  // ensure global marker to avoid races
    ...
    try {
      ...
      if (g.status === 'finished') {
        ...
        if (state.outcomeShownForGameId !== gameId) {
          ...
          state.outcomeShownForGameId = gameId;
          dbgLog('[PATCH 2] showOutcomeModal from pollGameOnce', g.id);
...
// PATCH 3 : snapshot complet passé à showSeriesModal ; try/catch non-bloquant
// PATCH 5 : NE PAS pré-marquer si l'utilisateur local est le gagnant (il doit voir la modal et cliquer)
if (finalizeRow.series_winner) {
  const intendedSeriesId = String(finalizeRow.series_id ?? state.series_id ?? '');
  const amLocalSeriesWinner = state.userId && (String(state.userId) === String(finalizeRow.series_winner));

  dbgLog('[PATCH 3] series_winner detected in finalize', { intendedSeriesId, amLocalSeriesWinner, seriesWinner: finalizeRow.series_winner });

  // PATCH 5 : si l'utilisateur est le gagnant de la série, NE PAS pré-marquer — il doit voir la modal
  if (amLocalSeriesWinner) {
    dbgLog('[PATCH 5] local user is series winner — skip pre-mark so they see the series modal');
    // On tente quand même de montrer la modal immédiatement
    try {
      // Snapshot complet des valeurs (PATCH 3)
      const seriesSnap = {
      ...
      dbgLog('[PATCH 3] calling showSeriesModal (winner path) with snapshot', seriesSnap);
      showSeriesModal(seriesSnap);
    } catch (e) {
      dbgLog('[PATCH 3] showSeriesModal (winner path) from finalize failed', e);
    }
  } else {
    // si un autre handler a déjà marqué cette série, on skip
    if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
      dbgLog('[PATCH 5/6] series already marked in finalize handler, skipping show', intendedSeriesId);
    } else {
      // marquage local pour éviter toute course plus tard
      state.seriesFinishedShownForSeriesId = intendedSeriesId;
      dbgLog('[PATCH 5/6] series marked in finalize', intendedSeriesId);
      try {
        // Snapshot complet (PATCH 3)
        const seriesSnap = {
          series_id:           finalizeRow.series_id ?? state.series_id,
          series_owner_wins:   Number(finalizeRow.series_owner_wins   ?? state.series_owner_wins   ?? 0),
          series_opponent_wins:Number(finalizeRow.series_opponent_wins?? state.series_opponent_wins?? 0),
        ...
        dbgLog('[PATCH 3] calling showSeriesModal (non-winner path) with snapshot', seriesSnap);
        showSeriesModal(seriesSnap);
      } catch (e) {
        dbgLog('[PATCH 3] showSeriesModal from finalize failed', e);
      }
    }
  }
}
    ...
    if (newGameId) {
      ...
      // Ensure local client has authoritative party/series counters & score subscriptions after join
try {
  await fetchAndApplyPartyAndSeries(state.party_id, state.series_id);
} catch (e) { dbgLog('fetchAndApply after finalize failed', e); }

try {
  await setupScoreSubscriptions();
} catch (e) { dbgLog('setupScoreSubscriptions after finalize failed', e); }
    } else {
 ...
 async function showNewRoundModal(info) {
  try {
    ...
    if (info && info.new_game_id) {
      if (!tryMarkModalShown('newRoundShownForGameId', info.new_game_id)) {
        dbgLog('newRound modal already handled (early mark)', info.new_game_id);
        return;
      }
    }
...
joinBtn.onclick = async () => {
  ...
  try {
    ...
    // PATCH 5/6 : NE PAS pré-marquer si l'utilisateur local est le gagnant de la série
    // (il doit pouvoir voir la modal et cliquer "Nouvelle série")
    const amLocalSeriesWinnerModal = state.userId && info.series_winner
      && (String(state.userId) === String(info.series_winner));

    // try to mark now; if another handler already did, skip
    // PATCH 5 : si le local user est le gagnant, on n'utilise pas tryMarkModalShown pour bloquer
    if (!amLocalSeriesWinnerModal) {
      if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
        dbgLog('[PATCH 5/6] series modal already marked by another handler (non-winner path)', intendedSeriesId);
        return;
      }
      dbgLog('[PATCH 6] series modal marked (non-winner)', intendedSeriesId);
    } else {
      dbgLog('[PATCH 5/6] local winner — bypassing tryMarkModalShown guard in showSeriesModal', intendedSeriesId);
    }

    // Marquer **après** l'ajout DOM pour éviter les races (PATCH 5 : post-append marking)
    // Pour le gagnant local : on ne marque ici que si le marquage n'est pas déjà fait ailleurs
    if (!amLocalSeriesWinnerModal) {
      // déjà marqué par tryMarkModalShown ci-dessus
    } else {
      // gagnant local : on marque maintenant que la modal est dans le DOM
      state.seriesFinishedShownForSeriesId = intendedSeriesId;
      dbgLog('[PATCH 5/6] post-append mark for local series winner', intendedSeriesId);
    }
...
joinBtn.onclick = async () => {
  ...
  try {
    ...
    await pollGameOnce();
      ...
      // PATCH 5 : marquage définitif post-append (pour tout le monde, y compris gagnant local)
      state.seriesFinishedShownForSeriesId = intendedSeriesId;
      dbgLog('[PATCH 5/6] showSeriesModal DOM appended + final mark done', intendedSeriesId);
...
try {
  ...
  // PATCH 1 : showOutcomeModal snapshot-safe.
  // Accepte un objet opts avec TOUTES les valeurs nécessaires :
  //   winnerId, winnerPseudo, ownerId, opponentId, ownerPseudo, opponentPseudo, forGameId
  // state.* est utilisé UNIQUEMENT comme fallback si une valeur n'est pas fournie dans opts.
  // Tous les messages et la logique son utilisent les valeurs snapshotées.
  function showOutcomeModal(opts = {}) {
  try {
    // --- PATCH 1 : snapshot de toutes les valeurs dès l'entrée ---
    const winnerId       = opts.winnerId       ?? state.winnerId      ?? null;
    const winnerPseudo   = opts.winnerPseudo   ?? state.winnerPseudo  ?? null;
    const snapOwnerId    = opts.ownerId        ?? state.ownerId       ?? null;
    const snapOpponentId = opts.opponentId     ?? state.opponentId    ?? null;
    const snapOwnerPseudo    = opts.ownerPseudo    ?? state.ownerProfile?.pseudo    ?? state.ownerPseudo    ?? (snapOwnerId    ? String(snapOwnerId).slice(0,8)    : 'Joueur 1');
    const snapOpponentPseudo = opts.opponentPseudo ?? state.opponentProfile?.pseudo ?? state.opponentPseudo ?? (snapOpponentId ? String(snapOpponentId).slice(0,8) : 'Joueur 2');
    const snapUserId     = opts.userId         ?? state.userId        ?? null;
    const forGameId      = opts.forGameId      ?? null;

    const targetGameId = forGameId ?? gameId;
    dbgLog('[PATCH6][showOutcomeModal] called', { targetGameId, winnerId, snapOwnerId, snapOpponentId });
    ...
    // suppress outcome for non-winners if a NEW-ROUND modal for a different game was shown very recently
    if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(targetGameId)) {
      ...
      if (Date.now() - when < 15000) {
        const amLocalWinner = snapUserId && (String(snapUserId) === String(winnerId || state.lastRoundWinnerId));
        if (!amLocalWinner) {
          dbgLog('[PATCH6] suppress outcome modal for non-winner (new-round shown recently)', state.newRoundShownForGameId, 'target', targetGameId);
          return;
        } else {
          dbgLog('[PATCH6] allow outcome modal for local winner despite recent new-round modal', targetGameId);
        }
      } else {
        state.newRoundShownForGameId = null;
        state._newRoundShownAt = null;
    ...
    if (!state.initialSyncDone) {
      dbgLog('[PATCH6] suppress outcome modal until initial sync for', targetGameId);
    ...
    if (String(state.outcomeShownForGameId) === String(targetGameId)) {
      dbgLog('[PATCH6] outcome already shown for', targetGameId);
      return;
    }

    // MARQUAGE IMMÉDIAT pour éviter races
    state.outcomeShownForGameId = String(targetGameId);
    dbgLog('[PATCH6] outcome marked as shown for', targetGameId);
    ...
    function launchEmojis(list = ['🎉', '✨', '🎊']) {
      ...
      } catch (ex) { dbgLog('launchEmojis failed', ex); }
    }

    // --- PATCH 1 : rôles et résolution depuis les valeurs snapshotées uniquement ---
    const amOwner    = !!(snapUserId && snapOwnerId    && String(snapUserId) === String(snapOwnerId));
    const amOpponent = !!(snapUserId && snapOpponentId && String(snapUserId) === String(snapOpponentId));
    const amSpectator = !amOwner && !amOpponent;
    const amPlayer = amOwner || amOpponent;

    const actualWinnerId = winnerId;
    let actualWinnerPseudo = winnerPseudo;
    if (!actualWinnerPseudo && actualWinnerId) {
      if (String(actualWinnerId) === String(snapOwnerId)) actualWinnerPseudo = snapOwnerPseudo;
      else if (String(actualWinnerId) === String(snapOpponentId)) actualWinnerPseudo = snapOpponentPseudo;
    }
    const actualLoserPseudo = (actualWinnerId && String(actualWinnerId) === String(snapOwnerId))
      ? snapOpponentPseudo : snapOwnerPseudo;
    const amLoser = amPlayer && actualWinnerId && snapUserId && String(snapUserId) !== String(actualWinnerId);

    // --- header / messages — PATCH 1 : valeurs snapshotées ---
    const title = document.createElement('h3');
    ...
    if (amPlayer) {
      if (amOwner && actualWinnerId && String(actualWinnerId) === String(snapOwnerId)) {
        msg.textContent = `Félicitations, ${snapOwnerPseudo} — vous avez gagné !`;
      } else if (amOwner) {
        msg.textContent = `Dommage, ${snapOwnerPseudo} — vous avez perdu cette manche.`;
      } else if (amOpponent && actualWinnerId && String(actualWinnerId) === String(snapOpponentId)) {
        msg.textContent = `Félicitations, ${snapOpponentPseudo} — vous avez gagné !`;
      } else if (amOpponent) {
        msg.textContent = `Dommage, ${snapOpponentPseudo} — vous avez perdu cette manche.`;
      }
    } else {
      if (actualWinnerId && String(actualWinnerId) === String(snapOwnerId)) {
        msg.textContent = `${snapOwnerPseudo} a gagné cette manche.`;
      } else if (actualWinnerId && String(actualWinnerId) === String(snapOpponentId)) {
        msg.textContent = `${snapOpponentPseudo} a gagné cette manche.`;
      } else {
    ...
    // PATCH 1+2 : comparaison uniquement sur snapUserId vs actualWinnerId
    if (snapUserId && actualWinnerId && String(snapUserId) === String(actualWinnerId)) {
    ...
    document.body.appendChild(overlay);
    dbgLog('[PATCH6] showOutcomeModal DOM appended for', targetGameId);

    // --- sons — PATCH 1 : logique son depuis valeurs snapshotées ---
    try {
      const viewerRole = amOwner ? 'owner' : (amOpponent ? 'opponent' : 'spectator');
      const ownerWon    = actualWinnerId && snapOwnerId    && (String(actualWinnerId) === String(snapOwnerId));
      const opponentWon = actualWinnerId && snapOpponentId && (String(actualWinnerId) === String(snapOpponentId));
    ...
    } catch (e) { dbgLog('outcome sound logic failed', e); }
...
if (state._pendingSeriesModal && state._pendingSeriesModal.length) {
  for (const item of state._pendingSeriesModal) {
    try { showSeriesModal(item.info); } catch(e){ dbgLog('showSeriesModal from pending failed', e); }
  }
