...
async function fetchAndApplyPartyAndSeries(partyId, seriesId) {
  ...
  try {
    ...
    // ── EXIT IMMÉDIAT si le gagnant a déjà rejoint ce jeu via le bouton de la modale ──────
    // joinNewGame a déjà tout géré (board, scores, subscriptions).
    // Si on continue ici on :
    //   1) écrase state.board = {} APRÈS que joinNewGame l'a correctement rendu
    //   2) marque outcomeShownForGameId = gameId (= nouveau jeu) → bloque showOutcomeModal
    //      pour toute la nouvelle série (bug "modale absente en nouvelle série")
    //   3) re-fetch les anciennes données et appelle updatePlayerUI → flickering scores
    if (String(gameId) === String(g.id)) {
      dbgLog('handleGlobalNewGameCreated: gagnant déjà rejoint via modale, exit immédiat', g.id);
