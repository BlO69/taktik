  ...
  async function pollGameOnce() {
    ...
    try {
      ...
      state.party_id = g.party_id ?? state.party_id;
            // immediately fetch canonical party/series counters so UI shows true scores on refresh
try {
  if (state.party_id) {
    // best-effort sync of party & series scores for immediate UI consistency
    await fetchAndApplyPartyAndSeries(state.party_id, null);
    dbgLog('Fetched party/series counters after pollGameOnce to avoid score reset on refresh', state.party_id);
  } else if (state.series_id) {
    await fetchAndApplyPartyAndSeries(null, state.series_id);
  }
} catch (e) { dbgLog('fetchAndApplyPartyAndSeries after pollGameOnce failed', e); }
...
state.lastPartyKnownGameId = String(newGameId);
      ...
      renderBoard(state.board);
      updateTurnUI();
      // If we are the local winner (we called finalize), show the new-round modal locally as well
try {
  if (newGameId) {
    // ensure authoritative scores are present before showing the modal
    try { await fetchAndApplyPartyAndSeries(state.party_id ?? null, state.series_id ?? null); } catch(e){ dbgLog('pre-showNewRound fetch failed', e); }

    // mark & show the new-round modal for the newGameId (idempotent)
    if (!tryMarkModalShown('newRoundShownForGameId', newGameId)) {
      dbgLog('newRound already marked (finalize flow), skipping show', newGameId);
    } else {
      showNewRoundModal({
        party_id: state.party_id,
        party_owner_wins: state.party_owner_wins,
...
} catch(e) { dbgLog('showNewRoundModal after finalize failed', e); }
...
try {
 ...
 async function showNewRoundModal(info) {
  try {
    ...
    // --- guard: if local winner still needs to finalize, defer showing new-round modal ---
if (info && info.new_game_id) {
  // If local winner still has to click "Continuer" (i.e. lastRoundWinnerId === userId)
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  // If outcome hasn't been shown yet for the previous game, defer new-round modal to avoid race
  if (amLocalWinner && (!state.outcomeShownForGameId || String(state.outcomeShownForGameId) === String(gameId))) {
    dbgLog('Deferring new-round modal because local winner must finalize first', info.new_game_id);
    state._pendingNewRoundModal = state._pendingNewRoundModal || [];
    if (!state._pendingNewRoundModal.find(it => String(it.info?.new_game_id) === String(info.new_game_id))) {
      state._pendingNewRoundModal.push({ info });
    }
    return;
  }

  // Otherwise try to mark idempotently (as before)
