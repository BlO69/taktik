...
try {
  ...
  dbgLog('enqueue new-round modal failed', e);
    ...
    try {
     // (PATCH) Preserve outcome modal for loser; show loading overlay instead of removing everything
const om = document.getElementById('taktik-outcome-modal');
if (om) {
  // si on est le gagnant local, on peut enlever la modale (il/elle doit cliquer "Continuer")
  const amLocalWinner = state.lastRoundWinnerId && state.userId &&
                        (String(state.lastRoundWinnerId) === String(state.userId));
  if (amLocalWinner) {
    dbgLog('local winner — remove outcome modal to allow interaction');
    om.remove();
  } else {
    // pour le perdant/spectateur : NE PAS retirer la modal de victoire,
    // afficher à la place un overlay "chargement..." pour éviter le flicker.
    dbgLog('preserve outcome modal for loser; show loading overlay instead of flicker');
    showLoadingOverlay();
    // on ne supprime pas om ici — il restera visible une seule fois (outcomeShownForGameId protège la répétition)
  }
}
 ...
 async function showNewRoundModal(info) {
  ...
  tryMarkModalShown('newRoundShownForGameId', info.new_game_id);
    ...
    if (prev) prev.remove();
      ...
      if (info.new_game_id) {
  // si l'utilisateur local est le gagnant récent (dernière manche) OU le gagnant de la série,
  // NE PAS afficher le bouton "Rejoindre la nouvelle partie".
  const amLocalWinner = state.userId && (String(state.userId) === String(info.last_round_winner ?? info.series_winner));
  if (amLocalWinner) {
    dbgLog('skip showing join button to local winner for new game', info.new_game_id);
  } else {
    ...
    joinBtn.onclick = async () => {
      try {
        // nettoyer marqueurs avant join pour éviter replays de modales
        state.newRoundShownForGameId = null;
        state.outcomeShownForGameId = null;
        state.seriesFinishedShownForSeriesId = null;
        state.seriesFinishedShownForSeriesIdDismissed = null;
        ...
        if (info.last_round_winner) {
          if (String(info.last_round_winner) === String(state.ownerId)) {
            starterChar = 'X';
          } else if (String(info.last_round_winner) === String(state.opponentId)) {
            starterChar = 'O';
          }
        } else if (state.lastRoundWinnerId) {
          if (String(state.lastRoundWinnerId) === String(state.ownerId)) {
            starterChar = 'X';
          } else if (String(state.lastRoundWinnerId) === String(state.opponentId)) {
            starterChar = 'O';
          }
        }

        await joinNewGame(info.new_game_id, starterChar);
        // resync complet après join
        await pollGameOnce();
        ...
        dbgLog('join button click failed', e);
...
// Guard idempotent: si la modal de série a été dismissée ou déjà montrée, on skippe.
// Evite que le gagnant voie deux fois la modale.
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
  dbgLog('series modal previously dismissed, skipping', intendedSeriesId);
  return;
}
if (state.seriesFinishedShownForSeriesId && String(state.seriesFinishedShownForSeriesId) === intendedSeriesId) {
  dbgLog('series modal already shown in-memory, skipping', intendedSeriesId);
  return;
}
// Mark early to avoid racing callers from also showing it
...
if (!amUserSeriesWinner) {
  ...
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
    ...
    overlay.style.position = 'fixed';
      ...
      closeBtn.style.borderRadius = '6px';
      closeBtn.style.border = '1px solid #ccc';
      closeBtn.onclick = () => { markDismissed(); overlay.remove(); };
      btns.appendChild(closeBtn);
      ...
      box.appendChild(btns);
      document.body.appendChild(overlay);
    // allow new series/new games to present their own new-round modals afresh
state.newRoundShownForGameId = null;
