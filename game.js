...
try {
  ...
  // runtime: si déjà marqué dans l'état, sortir
  if (state.seriesFinishedShownForSeriesId && String(state.seriesFinishedShownForSeriesId) === sid) {
    dbgLog('showSeriesModal: already shown for', sid);
    return;
  }

  // persistant: si localStorage dit que la modal a déjà été montrée, on skip
  const shownKey = `taktik_modal_shown_seriesFinishedShownForSeriesId_${sid}`;
  if (typeof localStorage !== 'undefined' && localStorage.getItem && localStorage.getItem(shownKey)) {
