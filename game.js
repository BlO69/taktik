...
btns.appendChild(closeBtn);     
  ...
  try {
    ...
    if (info.new_game_id) {
      ...
      // L'ancien code faisait state._joiningGameId = String(info.new_game_id) pour
      // bloquer handleGlobalNewGameCreated concurrent, mais cela déclenchait la garde
      // anti-doublon de joinNewGame dès son entrée → return immédiat, board jamais
      // réinitialisé, nouveau jeu jamais rejoint.
      ...
      if (String(info.new_game_id) === String(gameId)) {
        // Déjà dans cette game (handleGlobalNewGameCreated l'a rejoint en avance pour le gagnant).
        // joinNewGame serait court-circuité → on synchronise seulement l'état/UI sans re-join.
        dbgLog('nouvelle série: déjà dans la nouvelle game (background join), sync only', info.new_game_id);
        try { await pollGameOnce(); } catch (_pe) { dbgLog('pollGameOnce sync-only failed', _pe); }
        try { await fetchAndApplyPartyAndSeries(state.party_id, state.series_id); } catch (_fe) { dbgLog('fetchAndApply sync-only failed', _fe); }
        updatePlayerUI();
        try { await setupScoreSubscriptions(); } catch (_se) {}
        // Redémarrer le polling/realtime si nécessaire
        try { await setupRealtime(); } catch (_re) {}
        try { startPolling(); } catch (_spe) {}
      } else {
        // rpc_finalize_round a déjà créé la nouvelle série/game → rejoindre directement
        // joinNewGame fait déjà fetchAndApplyPartyAndSeries + pollGameOnce en interne
...
// Quand la série est déjà close (series_winner présent), on NE montre PAS la
// modale victoire/défaite — la modale "Série terminée" gère l'affichage.
// On marque quand même outcomeShownForGameId pour bloquer les appels répétés.
if (state.series_winner) {
  if (String(state.outcomeShownForGameId) !== String(targetGameId)) {
    state.outcomeShownForGameId = String(targetGameId);
    dbgLog('showOutcomeModal: série terminée — affichage supprimé, marquage seul', targetGameId);
  }
  return;
}
// ────────────────────────────────────────────────────────────────────────────
