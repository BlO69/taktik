...
// --- POST-JOIN: forcer l'affichage des modales pour le gagnant (après joinNewGame)
// Idempotent : showNewRoundModal / showSeriesModal gèrent les marquages internes.
try {
  // construire info autoritaire (préférer finalizeRow si disponible)
  const modalInfo = {
    party_id: state.party_id,
    party_owner_wins: Number(finalizeRow.party_owner_wins ?? state.party_owner_wins ?? 0),
    party_opponent_wins: Number(finalizeRow.party_opponent_wins ?? state.party_opponent_wins ?? 0),
    party_winner: finalizeRow.party_winner ?? state.party_winner ?? null,
    series_id: finalizeRow.series_id ?? state.series_id,
    series_owner_wins: Number(finalizeRow.series_owner_wins ?? state.series_owner_wins ?? 0),
    series_opponent_wins: Number(finalizeRow.series_opponent_wins ?? state.series_opponent_wins ?? 0),
    series_winner: finalizeRow.series_winner ?? state.series_winner ?? null,
    new_game_id: newGameId,
    last_round_winner: finalizeRow.last_round_winner ?? state.lastRoundWinnerId ?? null
  };

  // afficher la modale "Nouvelle manche" pour le gagnant (après le join)
  showNewRoundModal(modalInfo);

  // si la série est finie, afficher aussi la modale série *après* le join
  if (finalizeRow && finalizeRow.series_winner) {
    showSeriesModal(modalInfo);
  }
} catch (e) {
  dbgLog('post-join show modals failed', e);
}
...
try {
  ...
  async function showOutcomeModal({ winnerId = null, winnerPseudo = null, forGameId = null } = {}) {
    try {
...
if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(targetGameId)) {
  ...
  if (Date.now() - when < 15000) { // 15s window
    ...
    if (!state.initialSyncDone) {
      ...
      // Avant d'afficher la modale, s'assurer que les compteurs party/series sont à jour
  try {
    await fetchAndApplyPartyAndSeries(state.party_id, state.series_id);
  } catch (e) {
    dbgLog('fetchAndApplyPartyAndSeries in showOutcomeModal failed', e);
  }
...
if (prev) prev.remove();
  ...
  // Afficher les scores (manche & série) dans la modale d'issue
  try {
    const scoresDiv = document.createElement('div');
    scoresDiv.style.fontSize = '14px';
    scoresDiv.style.color = '#333';
    scoresDiv.style.marginBottom = '10px';
    scoresDiv.innerHTML = `
      <div>Score manche — Joueur 1 : <strong>${state.party_owner_wins ?? 0}</strong> — Joueur 2 : <strong>${state.party_opponent_wins ?? 0}</strong></div>
      <div style="margin-top:6px">Score série — Joueur 1 : <strong>${state.series_owner_wins ?? 0}</strong> — Joueur 2 : <strong>${state.series_opponent_wins ?? 0}</strong></div>
    `;
    box.appendChild(scoresDiv);
  } catch (e) {
    dbgLog('showOutcomeModal: append scores failed', e);
  }
  ...
  // Si le client est le perdant, proposer un bouton pour rejoindre la nouvelle partie
  try {
    if (amLoser) {
      const joinBtn = document.createElement('button');
      joinBtn.textContent = 'Rejoindre la nouvelle partie';
      ...
      joinBtn.onclick = async () => {
        ...
        // tentative 1: utiliser le dernier game connu pour la party
        let newGameIdCandidate = (state.lastPartyKnownGameId && String(state.lastPartyKnownGameId) !== String(gameId)) ? state.lastPartyKnownGameId : null;

        // tentative 2: si absent, demander la dernière game en base pour cette party
        if (!newGameIdCandidate) {
          try {
            if (state.party_id) {
              const { data: rows, error } = await supabase
                .from('games')
                ...
                .order('created_at', { ascending: false })
                .limit(1);
              if (!error && Array.isArray(rows) && rows.length) newGameIdCandidate = rows[0].id;
            }
          } catch (fetchErr) {
            dbgLog('outcome modal join: fetch latest game failed', fetchErr);
          }
        }

        if (newGameIdCandidate) {
          try {
            await joinNewGame(newGameIdCandidate, null);
            // resync après join pour garantir grille & scores corrects
            await pollGameOnce();
            try { await setupScoreSubscriptions(); } catch(e){ dbgLog('post-join setupScoreSubscriptions failed', e); }
            updatePlayerUI();
          } catch (e) {
            dbgLog('joinNewGame from outcome modal failed', e);
            await pollGameOnce();
          }
        } else {
          // court feedback si aucune nouvelle partie trouvée
          try {
            const toast = document.createElement('div');
            toast.textContent = 'Aucune nouvelle partie trouvée pour le moment.';
            toast.style.position = 'fixed';
            toast.style.bottom = '18px';
            toast.style.left = '50%';
            toast.style.transform = 'translateX(-50%)';
    ...
    dbgLog('showOutcomeModal: add join button failed', e);
  }
