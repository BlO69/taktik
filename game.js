...
function joinMatchRPC(matchId) {
  ...
  function updatePlayerUI() {
    ...
    // 'X' is owner, 'O' is opponent
        // Update scores on page (only show a value if known; otherwise leave blank to avoid flicker/reset)
    setText('scoreX', (state.party_owner_wins != null) ? state.party_owner_wins : '');
    setText('scoreO', (state.party_opponent_wins != null) ? state.party_opponent_wins : '');
    setText('seriesX', (state.series_owner_wins != null) ? state.series_owner_wins : '');
    setText('seriesO', (state.series_opponent_wins != null) ? state.series_opponent_wins : '');
...
async function fetchAndApplyPartyAndSeries(partyId, seriesId) {
    const prevSeriesIdLocal = state.series_id;
  try {
    // Party
    if (partyId) {
    ...
    if (sid) {
      ...
      if (!serr && seriesRow) {
        ...
        state.series_winner = seriesRow.winner_id ?? state.series_winner;
            ...
            // --- PATCH: if series_id changed we should clear the "series finished shown" sentinel
      // so the next series completion can show its modal again. Also if a new series started
      // and this client was the *loser* of the previous series, show a single "join new series" modal.
      try {
        const fetchedSeriesId = seriesRow?.id ? String(seriesRow.id) : (seriesId ? String(seriesId) : null);
        const prevSeriesId = prevSeriesIdLocal;
        if (fetchedSeriesId && String(fetchedSeriesId) !== String(prevSeriesId)) {
          dbgLog('fetchAndApplyPartyAndSeries detected series_id change', prevSeriesId, '→', fetchedSeriesId);
          // clear previous sentinel so future series can show their modal normally
          state.seriesFinishedShownForSeriesId = null;

          // If the previous series was just finished and this client was the loser,
          // show a single "join new series" modal (queued/shown once).
          try {
            const prevSeriesWinner = state.series_winner;
            if (prevSeriesWinner && state.userId) {
              const loserId = String(prevSeriesWinner) === String(state.ownerId) ? String(state.opponentId) : String(state.ownerId);
              if (String(state.userId) === String(loserId)) {
                state._pendingSeriesJoinModal = state._pendingSeriesJoinModal || [];
                if (!state._pendingSeriesJoinModal.find(it => String(it.info?.new_series_id) === String(fetchedSeriesId))) {
                  state._pendingSeriesJoinModal.push({ info: { new_series_id: fetchedSeriesId, prev_series_id: prevSeriesId } });
                  dbgLog('enqueued join-series modal for loser for new series', fetchedSeriesId);
                }
                if (state.initialSyncDone) {
                  try { showJoinSeriesModal({ new_series_id: fetchedSeriesId, prev_series_id: prevSeriesId }); } catch(e){ dbgLog('showJoinSeriesModal failed', e); }
                }
              }
            }
          } catch(e){ dbgLog('post-series-change loser-join handling failed', e); }
        }
      } catch (e) {
        dbgLog('fetchAndApplyPartyAndSeries post-series-change patch failed', e);
      }
    }
  ...
  try {
    ...
    // --- TOLERANT GUARD ---
// Si on a déjà traité cette nouvelle partie, sortir — mais seulement si le marqueur est récent.
// Si le marqueur est ancien (stale), on le nettoie et on continue.
if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) === String(g.id)) {
  const when = state._newRoundShownAt || 0;
  // fenêtre de tolérance 30s (ajustable)
  if (Date.now() - when < 30000) {
    dbgLog('new game already processed recently, skipping', g.id);
    return;
  } else {
    dbgLog('found stale newRoundShownForGameId for', g.id, ' — clearing sentinel and continuing');
...
try {
  ...
  gameId = g.id;
    ...
    // join the new game first for winner/spectators.
  // If this client is the *loser* of the last round, DO NOT auto-join:
  // show/queue the NewRound modal so the loser must click "Rejoindre la nouvelle partie".
  try {
    // determine loserId (best-effort) using lastRoundWinnerId or g.owner/opponent
    let isLoserClient = false;
    try {
      const winnerId = state.lastRoundWinnerId ?? g.winner ?? null;
      let loserId = null;
      if (winnerId) {
        // prefer explicit g.owner_id/opponent_id if present, otherwise state
        const ownerId = g.owner_id ?? state.ownerId;
        const opponentId = g.opponent_id ?? state.opponentId;
        if (String(winnerId) === String(ownerId)) loserId = opponentId;
        else loserId = ownerId;
      }
      if (state.userId && loserId && String(state.userId) === String(loserId)) isLoserClient = true;
    } catch (e) { dbgLog('determine loser failed', e); }

    if (isLoserClient) {
      dbgLog('Client is loser — do not auto-join; show new-round modal for manual join', g.id);

      // enqueue/ensure modal will be shown (best-effort)
      state._pendingNewRoundModal = state._pendingNewRoundModal || [];
      if (!state._pendingNewRoundModal.find(it => String(it.info?.new_game_id) === String(g.id))) {
      ...
      // ensure UI reflects latest state before showing modal
      try { await pollGameOnce(); } catch(_) {}
      try { await fetchAndApplyPartyAndSeries(g.party_id ?? state.party_id, g.series_id ?? state.series_id); } catch(e){ dbgLog('fetch scores before new-round modal failed', e); }
      // show modal immediately (idempotent — showNewRoundModal already protects against duplicates)
      try { showNewRoundModal({ new_game_id: g.id, last_round_winner: state.lastRoundWinnerId }); } catch(e){ dbgLog('showNewRoundModal for loser failed', e); }
    } else {
      // winner/spectator: preserve old behavior (auto-join)
      try {
    ...
    dbgLog('conditional auto-join block failed', e);
  }
...
try {
  ...
  dbgLog('auto-join + showNewRoundModal block failed', e);
        ...
        // If the series has been closed (series_winner present) show the series modal once (guarded).
    if (state.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
      try {
        // compute a stable series id string
        const seriesIdToShow = String(state.series_id ?? (g.series_id ?? ''));
        // mark & show only if not already marked (race-safe)
        if (tryMarkModalShown('seriesFinishedShownForSeriesId', seriesIdToShow)) {
          showSeriesModal({
            series_id: seriesIdToShow,
            series_owner_wins: state.series_owner_wins,
            series_opponent_wins: state.series_opponent_wins,
            series_winner: state.series_winner,
          ...
          dbgLog('series modal already marked, skipping', seriesIdToShow);
        }
      } catch (e) {
        dbgLog('series modal guarded show failed', e);
      }
    }
    ...
    try {
      ...
      gameId = g.id;
            ...
            // then join — but do NOT auto-join if this client is the loser; show modal instead.
      try {
        let isLoserClient = false;
        try {
          const winnerId = state.lastRoundWinnerId ?? g.winner ?? null;
        ...
        if (isLoserClient) {
          dbgLog('Client is loser — skipping auto-join here; presenting new-round modal', g.id);
          // ensure modal is queued/shown
          state._pendingNewRoundModal = state._pendingNewRoundModal || [];
          if (!state._pendingNewRoundModal.find(it => String(it.info?.new_game_id) === String(g.id))) {
            state._pendingNewRoundModal.push({
              info: {
        ...
        dbgLog('conditional join/error handling failed', e);
      }
    } catch (e) {
                ...
                if (state.party_id) {
                  ...
                  if (!rgerr && Array.isArray(recentGames) && recentGames.length) {
                    ...
                    if (String(latest.id) !== String(gameId)) newGameIdFromServer = latest.id;
                          ...
                          // showSeriesModal: guard with tryMarkModalShown so we only display it once per series
            try {
              const sId = String(state.series_id ?? '');
              if (tryMarkModalShown('seriesFinishedShownForSeriesId', sId)) {
                showSeriesModal({
                  series_id: sId,
                  series_owner_wins: state.series_owner_wins,
                  series_opponent_wins: state.series_opponent_wins,
                ...
                dbgLog('pollGameOnce: series modal already marked, skipping', sId);
              }
            } catch (e) {
              dbgLog('pollGameOnce: guarded showSeriesModal failed', e);
            }
            }
          } catch (e) {
  ...
  dbgLog('enqueue new-round modal failed', e);
    ...
    if (newGameId) {
      if (!state.initialSyncDone) {
        ...
        state.newRoundShownForGameId = String(newGameId);
      }
    }
...
if (finalizeRow.series_winner) {
  try {
        try {
      const sid = String(finalizeRow.series_id ?? state.series_id ?? '');
      if (tryMarkModalShown('seriesFinishedShownForSeriesId', sid)) {
        showSeriesModal({
        ...
        dbgLog('finalize: series modal already marked, skipping', sid);
      }
    } catch (e) { dbgLog('showSeriesModal from finalize failed', e); }
  } catch (e) {
    ...
    if (newGameId) {
      ...
      // After finalizing the round and before joining/polling, clear any stale new-round marker
state.newRoundShownForGameId = null;
state._newRoundShownAt = null;
dbgLog('Cleared newRoundShownForGameId after finalizeRound', p_game_id);
...
try {
  await setupScoreSubscriptions();
  // === Ensure series modal is shown for the caller (winner) after auto-join ===
// (Guarded to avoid duplicates if already shown)
try {
  if (finalizeRow && finalizeRow.series_winner) {
    ...
    if (tryMarkModalShown('seriesFinishedShownForSeriesId', sid)) {
      ...
      dbgLog('post-join: series modal already marked, skipping', sid);
    }
  }
} catch (e) {
  dbgLog('post-join forced showSeriesModal failed', e);
...
joinBtn.onclick = async () => {
  ...
  try {
    ...
    // If already shown for this series, skip
    if (String(state.seriesFinishedShownForSeriesId) === intendedSeriesId) {
      dbgLog('series modal already shown for', intendedSeriesId);
      return;
    }
...
joinBtn.onclick = async () => {
  ...
  try {
    ...
    await pollGameOnce();
      ...
      box.appendChild(btns);
      document.body.appendChild(overlay);
    // after we actually inserted the series modal into the DOM, mark it as shown
try {
  const intendedSeriesId = info?.series_id ?? state.series_id ?? null;
  if (intendedSeriesId && String(state.seriesFinishedShownForSeriesId) !== String(intendedSeriesId)) {
    state.seriesFinishedShownForSeriesId = intendedSeriesId;
    state._seriesShownAt = Date.now();
    dbgLog('Marked seriesFinishedShownForSeriesId after DOM insert', state.seriesFinishedShownForSeriesId);
  }
} catch (e) {
  dbgLog('marking seriesShown failed', e);
}
    // clear new-round sentinel so next series / party can show fresh modals
state.newRoundShownForGameId = null;
state._newRoundShownAt = null;
dbgLog('Cleared newRoundShownForGameId after showing series modal', state.series_id);
  ...
  async function showJoinSeriesModal(info = {}) {
  try {
    if (!info || !info.new_series_id) return;
    const sid = String(info.new_series_id);
    // avoid duplicates
    state._pendingSeriesJoinModal = state._pendingSeriesJoinModal || [];
    if (state._pendingSeriesJoinModal._shownFor && String(state._pendingSeriesJoinModal._shownFor) === sid) return;
    // mark shown
    state._pendingSeriesJoinModal._shownFor = sid;

    const prev = document.getElementById('taktik-series-join-modal');
    if (prev) prev.remove();

    const overlay = document.createElement('div');
    overlay.id = 'taktik-series-join-modal';
    overlay.style.position = 'fixed';
    overlay.style.inset = '0';
    overlay.style.background = 'rgba(0,0,0,0.65)';
    overlay.style.display = 'flex';
    ...
    const title = document.createElement('h3');
    title.textContent = 'Nouvelle série disponible';
    box.appendChild(title);

    const p = document.createElement('p');
    p.textContent = 'Tu as perdu la série précédente. Veux-tu rejoindre la nouvelle série maintenant ?';
    ...
    const joinBtn = document.createElement('button');
    joinBtn.textContent = 'Rejoindre la nouvelle série';
    joinBtn.style.padding = '8px 12px';
    joinBtn.style.borderRadius = '8px';
    joinBtn.onclick = async () => {
      try {
        await pollGameOnce();
        await fetchAndApplyPartyAndSeries(null, sid);
        overlay.remove();
      } catch (e) {
        dbgLog('join-series button failed', e);
      }
    };
    btns.appendChild(joinBtn);
    ...
    dbgLog('showJoinSeriesModal exception', e);
  }
}
...
// only suppress outcome modal if a NEW-ROUND modal for a different game was shown very recently
if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(targetGameId)) {
  const when = state._newRoundShownAt || 0;
  if (Date.now() - when < 15000) { // 15s window
    dbgLog('suppress outcome modal because a new-round modal was shown recently', state.newRoundShownForGameId, 'target', targetGameId);
