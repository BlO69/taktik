...
try {
  ...
  async function handleFinalizeRoundAndFollowup(p_game_id, options = {}) {
    try {
      // Ensure the game's winner has been persisted server-side before calling finalize.
      // Sometimes rpc_place_move returns quickly while the DB row hasn't been visible to the finalizer yet.
      async function ensureWinnerPersisted(gameId, maxAttempts = 8, delayMs = 250) {
        try {
          for (let attempt = 0; attempt < maxAttempts; attempt++) {
            // fetch authoritative game row's winner column
            const { data: g, error } = await supabase
              .from('games')
              .select('id,winner')
              .eq('id', gameId)
        ...
        } catch (e) { dbgLog('ensureWinnerPersisted exception', e); }
        return false;
      }

      // If we don't have a winner in state yet, try to ensure the DB row is updated first.
      if (!state.winnerId) {
        await ensureWinnerPersisted(p_game_id, 8, 250);
      }

      // call finalize RPC (server will compute party/series and may create new series/game)
      ...
      // (some clients may not have updated it yet via realtime/poll)
      ...
      // If finalize rpc signals series finished, show a dedicated series modal (once)
      if (finalizeRow.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
        // prepare extra info: last_round_winner may be present in finalizeRow
        showSeriesModal({
          series_id: state.series_id,
          series_owner_wins: state.series_owner_wins,
          series_opponent_wins: state.series_opponent_wins,
      ...
      if (state.party_id) {
        ...
        // Note: this client already has the newGameId (will join below). Other clients use polling to detect.
...
function showSeriesModal(info = {}) {
  ...
  function showOutcomeModal({ winnerId = null, winnerPseudo = null } = {}) {
    try {
      ...
      if (isWinner) {
        ...
        // play victory sound on modal open (only once) — only winner hears this
        playSound(VICTORY_SOUND);
      } else {
        // Loser or spectator view: show defeat modal & play defeat sound (ONLY for loser)
        // Distinguish loser vs spectator by checking if user took part in game and is not winner
        const amPlayer = (state.userId && (String(state.userId) === String(state.ownerId) || String(state.userId) === String(state.opponentId)));
        const amLoser = amPlayer && state.winnerId && String(state.userId) !== String(state.winnerId);
  ...
  async function boot() {
    ...
    // start party polling fallback if we have a party
    startPartyPolling();
