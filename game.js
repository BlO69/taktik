...
try { if (_shouldHideLoading) hideLoadingOverlay(); } catch(e){ dbgLog('hideLoadingOverlay failed', e); }
...
if (prev) prev.remove();
    ...
    if (state.userId && actualWinnerId && String(state.userId) === String(actualWinnerId)) {
      ...
      continueBtn.onclick = async () => {
        ...
        try {
          // Call finalize and capture the returned row (may include series/new_game info)
          finalizeRow = await handleFinalizeRoundAndFollowup(String(targetGameId), { starterChar: null });
        } catch (err) {
          dbgLog('winner finalize failed', err);
          // fallback: refresh state
          try { await pollGameOnce(); } catch (_) {}
        } finally {
          // close the outcome overlay (we will open the series modal if appropriate)
          try { overlay.remove(); } catch (e) { /* ignore */ }
        }

        // After finalize, if the series was finished and local user is the series winner,
        // explicitly show the series modal (this ensures the winner sees the 'Nouvelle série' button)
        try {
          const seriesWinner = (finalizeRow && finalizeRow.series_winner) ? finalizeRow.series_winner : state.series_winner;
          const seriesId = (finalizeRow && finalizeRow.series_id) ? finalizeRow.series_id : state.series_id;
          if (seriesWinner && state.userId && String(seriesWinner) === String(state.userId)) {
            try {
              dbgLog('Post-finalize: local series winner detected, showing series modal', seriesId);
              await showSeriesModal({
                series_id: seriesId,
                series_owner_wins: finalizeRow?.series_owner_wins ?? state.series_owner_wins,
                series_opponent_wins: finalizeRow?.series_opponent_wins ?? state.series_opponent_wins,
                series_winner: seriesWinner,
                series_winner_pseudo: (finalizeRow?.series_winner_pseudo ?? (seriesWinner && (String(seriesWinner) === String(state.ownerId) ? state.ownerProfile?.pseudo : state.opponentProfile?.pseudo))) || null,
                party_owner_wins: finalizeRow?.party_owner_wins ?? state.party_owner_wins,
                party_opponent_wins: finalizeRow?.party_opponent_wins ?? state.party_opponent_wins,
                new_game_id: finalizeRow?.new_game_id ?? null,
                last_round_winner: finalizeRow?.last_round_winner ?? state.lastRoundWinnerId
              });
            } catch (e) {
              dbgLog('showSeriesModal after finalize failed', e);
            }
          }
        } catch (e) {
          dbgLog('post-finalize series handling failed', e);
        } finally {
