...
try { await pollGameOnce(); } catch(e){ dbgLog('pollGameOnce after global new-game failed', e); }
      // --- best-effort: fetch series row to immediately reflect series counters for losers/spectators ---
try {
  if (state.series_id) {
  ...
  dbgLog('post-newgame fetch series failed', e);
}
...
joinBtn.onclick = async () => {
  ...
  async function createNewSeriesFromSeries(parentSeriesId, lastRoundWinner = null) {
    try {
      ...
      // --- ensure we won't accidentally consider the old series as already "shown" ---
state.seriesFinishedShownForSeriesId = null;
// clear local series counters until we fetch authoritative values (will be refreshed)
