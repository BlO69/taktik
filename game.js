...
// FIX #2 : Pour le perdant, l'overlay de chargement est déjà visible (montré juste au-dessus).
// On retire TOUTES les modales (y compris la modale série) pour éviter tout flickering.
// Le gagnant de la série conserve sa modale série (il a montré l'overlay lui-même dans son bouton).
const _amLocalWinnerCleanup = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
const om = document.getElementById('taktik-outcome-modal');
if (om) om.remove();
const nm = document.getElementById('taktik-new-round-modal');
if (nm) nm.remove();
if (!_amLocalWinnerCleanup) {
  // perdant/spectateur : retirer aussi la modale série (l'overlay de chargement couvre tout)
  const sm = document.getElementById('taktik-series-modal');
  if (sm) sm.remove();
}
    } catch (e) {
...
// Si le gagnant a déjà vu la modale via handleFinalizeRoundAndFollowup (tryMarkModalShown déjà posé),
// ne pas réafficher même si amLocalWinner est true.
if (!tryMarkModalShown('newRoundShownForGameId', g.id)) {
  dbgLog('newRound already marked, skipping show (handleGlobalNewGameCreated)', g.id);
} else {
  showNewRoundModal({
    party_id: state.party_id,
    party_owner_wins: state.party_owner_wins,
...
state.seriesFinishedShownForSeriesIdDismissed = null;
  ...
  dbgLog('finalize modal cleanup failed', e);
    ...
    // FIX #3 : Afficher la modale "nouvelle manche" directement si le sync initial est terminé,
    // sinon l'empiler dans _pendingNewRoundModal pour traitement au startup.
    // showNewRoundModal supprime elle-même l'affichage pour le perdant/spectateur.
    if (newGameId) {
      const roundModalInfo = {
        party_id: state.party_id,
        party_owner_wins: state.party_owner_wins,
        party_opponent_wins: state.party_opponent_wins,
        party_winner: state.party_winner,
      ...
      if (!state.initialSyncDone) {
        ...
        if (!state._pendingNewRoundModal.find(it => String(it.info?.new_game_id) === String(newGameId))) {
          state._pendingNewRoundModal.push({ info: roundModalInfo });
          dbgLog('enqueued new-round modal for later (not synced yet)', newGameId);
        }
      } else {
        // Synced : appel direct pour que la modale s'affiche à chaque manche dans toute série
        dbgLog('direct showNewRoundModal from handleFinalizeRoundAndFollowup', newGameId);
        try {
          await showNewRoundModal(roundModalInfo);
        } catch(e) { dbgLog('showNewRoundModal direct call from finalize failed', e); }
      }
    }
...
// FIX #1 : une seule modale par série, même pour le gagnant.
// state.seriesFinishedShownForSeriesId est remis à null lors d'une nouvelle série (createNewSeriesFromSeries / joinNewGame).
if (String(state.seriesFinishedShownForSeriesId) === intendedSeriesId) {
  dbgLog('series modal already shown for this series_id, skipping (even for winner)', intendedSeriesId);
  return;
}

// PATCH 4: vérifier si la modale a été explicitement dismissée par l'utilisateur (non-gagnant seulement)
