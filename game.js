    ...
    if (state.userId && actualWinnerId && String(state.userId) === String(actualWinnerId)) {
      ...
      continueBtn.onclick = async () => {
        ...
        try {
          ...
          // CORRECTION: après finalize, vérifier si une série s'est terminée et afficher la modale
          if (state.series_winner && state.userId && String(state.userId) === String(state.series_winner)) {
            const sid = state.series_id ? String(state.series_id) : '';
            // afficher la modale série si le gagnant
            try {
              dbgLog('Series winner continuing → showing series modal', sid);
              await showSeriesModal({
                series_id: state.series_id,
                series_owner_wins: state.series_owner_wins,
                series_opponent_wins: state.series_opponent_wins,
            ...
            } catch(e) { dbgLog('showSeriesModal from continueBtn failed', e); }
          }
        } catch (err) {
