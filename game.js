  ...
  try {
    ...
    // si déjà marqué en localStorage, restituer dans state et indiquer "déjà vu"
      ...
      if (localStorage.getItem(storageKey) === '1') {
        // If the series modal was already marked shown by another client,
        // allow the local series winner to still proceed (race condition fix).
        if (stateKey === 'seriesFinishedShownForSeriesId' &&
            state.userId && state.series_winner &&
            String(state.userId) === String(state.series_winner)) {
          // reflect in runtime state and allow showing for the winner
          state[stateKey] = sid;
          dbgLog('tryMarkModalShown: previously shown but allowing local series winner to proceed', sid);
          return true;
        }

        // otherwise restore runtime state and indicate "already shown"
        state[stateKey] = sid;
        return false;
      }
    } catch (e) {
...
try {
  ...
  dbgLog('overlay show guard failed', e);
    ...
    try {
  // retire seulement les modales outcome / new-round — ne pas supprimer la modale de série
  const om = document.getElementById('taktik-outcome-modal');
  if (om) om.remove();
  const nm = document.getElementById('taktik-new-round-modal');
  if (nm) nm.remove();
  // intentionally DO NOT remove 'taktik-series-modal' here so the series winner can interact.
} catch (e) {
...
// ✅ FIXED: Logique corrigée pour que le gagnant puisse TOUJOURS voir la modale
if (finalizeRow.series_winner) {
  ...
  // ✅ CONDITION 1: La modale a été DISMISSÉE par le gagnant → respecter son choix
  ...
  // ✅ CONDITION 2: C'EST LE GAGNANT → TOUJOURS afficher, peu importe si marquée avant
  else if (amLocalSeriesWinnerFinalize) {
    dbgLog('series winner: showing modal regardless of previous mark', intendedSeriesId);
    try {
      showSeriesModal({
      ...
      dbgLog('showSeriesModal from finalize (winner) failed', e);
    }
  }
  // ✅ CONDITION 3: Ce n'est pas le gagnant ET elle a déjà été marquée → skipper
  ...
  // ✅ CONDITION 4: Premier affichage pour non-gagnant
  else {
    try {
      showSeriesModal({
        series_id: finalizeRow.series_id ?? state.series_id,
...
if (newGameId) {
  ...
  if (amSeriesWinner) {
    ...
    const starterChar = options.starterChar ?? (
      state.lastRoundWinnerId ? (String(state.lastRoundWinnerId) === String(state.ownerId) ? 'X' : 'O') : null
    );
...
try {
  ...
  const nm = document.getElementById('taktik-new-round-modal');
  if (nm) nm.remove();
  // intentionally do NOT remove taktik-series-modal here to allow series-winner interaction.
} catch (e) {
...
try {
  ...
  const intendedSeriesId = state.series_id ? String(state.series_id) : '';
    ...
    String(state.userId) === String(state.series_winner);
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerSub) {
