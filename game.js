...
if (prevGameId) {
  ...
  // courte protection : empêcher que pollers re-pooled affichent l'ancien outcome
  state._modalCooldown = Date.now() + 1500;
}

    // marquer immédiatement la nouvelle partie comme "nouvelle manche affichée"
    state.newRoundShownForGameId = String(g.id);
   ...
   // retire outcome / new-round immédiatement (laisse la série si elle concerne la série courante)
try {
  ...
  // pour la series-modal : ne la supprimer que si elle n'appartient PAS à la serie courante
  const sm = document.getElementById('taktik-series-modal');
if (sm) {
  const modalSeriesId = sm.dataset && sm.dataset.seriesId ? String(sm.dataset.seriesId) : null;
  // remove the series modal if it doesn't belong to the current series or if it has no dataset (legacy)
  if (!modalSeriesId || String(state.series_id || '') !== modalSeriesId) {
    sm.remove();
  }
}
} catch (e) {
...
async function pollPartyNewGameOnce() {
  ...
  if (sm) {
    const modalSeriesId =
      sm.dataset && sm.dataset.seriesId
        ? String(sm.dataset.seriesId)
        : null;
  ...
  dbgLog('modal cleanup failed', e);
    ...
    // ensure we consistently store newRoundShownForGameId as a string (if newGameId present)
    if (newGameId) state.newRoundShownForGameId = String(newGameId);
    ...
    if (finalizeRow.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
      showSeriesModal({
        ...
        series_opponent_wins: state.series_opponent_wins,
        series_winner: state.series_winner,
        series_winner_pseudo:
          (state.series_winner && (String(state.series_winner) === String(state.ownerId)
...
// remplace l'ancien joinBtn.onclick = async () => { ... } par ce bloc
joinBtn.addEventListener('click', async () => {
  try {
    // sécurité : info et new_game_id requis
    if (!info || !info.new_game_id) return;

    // empêcher double-clics
    joinBtn.disabled = true;

    // retirer la modale tout de suite pour éviter les flashes répétés
    try { overlay.remove(); } catch (e) { dbgLog('overlay.remove() failed', e); }

    // Déterminer starterChar: prefer provided last_round_winner, then state.lastRoundWinnerId
    let starterChar = null;
    ...
    // tenter de rejoindre la nouvelle game (joinNewGame doit faire pollGameOnce() et reset du board)
    // si joinNewGame supporte starterChar, on le transmet; sinon passe juste info.new_game_id
    try {
      await joinNewGame(info.new_game_id, starterChar);
    } catch (e) {
      // si joinNewGame échoue (ex: réseau), tenter un fallback
      dbgLog('joinNewGame failed, will attempt fallback poll and rethrow for outer catch', e);
      // tentative de resync
      try { await pollGameOnce(); } catch (err) { dbgLog('fallback pollGameOnce failed', err); }
      throw e; // remonter pour afficher message à l'utilisateur
    }

    // post-join: setup subscriptions et UI update si nécessaire (best-effort)
    try { if (typeof setupScoreSubscriptions === 'function') await setupScoreSubscriptions(); } catch (e) { dbgLog('post-join setupScoreSubscriptions failed', e); }
    try { if (typeof updatePlayerUI === 'function') updatePlayerUI(); } catch (e) { dbgLog('post-join updatePlayerUI failed', e); }

    // succes: laisser disabled = true (on est maintenant dans la nouvelle partie)
  } catch (e) {
    dbgLog('joinNewGame from modal failed', e);
    // réactiver le bouton pour permettre une nouvelle tentative
    try { joinBtn.disabled = false; } catch (er) { dbgLog('failed to re-enable joinBtn', er); }
    // informer l'utilisateur et proposer de resynchroniser
    try { await pollGameOnce(); } catch (err) { dbgLog('pollGameOnce after failed join also failed', err); }
    alert('Erreur lors du rattachement à la nouvelle manche. Réessaie ou recharge la page.');
  }
});
  ...
  try {
    // Défendre l'affichage en cas de sync initiale incomplète,
    // MAIS marquer la série comme "déjà affichée" pour éviter des ré-entrées répétées.
    const intendedSeriesId = String(info?.series_id ?? state.series_id ?? 'series-shown');

        if (!state.initialSyncDone) {
      dbgLog('suppress series modal until initial sync (queueing for later display)', intendedSeriesId);
      // mettre en file d'attente pour affichage après la sync initiale
    ...
    // Si on a déjà affiché pour cette série (ou marque sentinel), ne rien faire.
    ...
    // identification utile pour ne pas supprimer la série active par erreur
overlay.dataset.seriesId = intendedSeriesId;
    overlay.style.position = 'fixed';
      overlay.style.inset = '0';
...
try {
  ...
  // --- PATCH à appliquer dans showOutcomeModal (rendre la fn async si besoin) ---
async function showOutcomeModal({ winnerId = null, winnerPseudo = null, forGameId = null } = {}) {
  try {
    const targetGameId = forGameId ?? gameId;

    // si on n'a pas fait le sync initial, on marque comme montré et on sort (déjà présent mais on garde)
    if (!state.initialSyncDone) {
    ...
    // Si on a déjà montré une NEW ROUND différente, ne pas ré-afficher l'outcome (évite modal obsolète).
    if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(targetGameId)) {
      dbgLog('suppress outcome modal because a new-round was already shown for another game', targetGameId);
      state.outcomeShownForGameId = String(targetGameId);
      return;
    }

    // cooldown global pour éviter re-affichages rapides par pollers (empêche flash)
    state._modalCooldown = state._modalCooldown || 0;
    if (Date.now() < state._modalCooldown) {
      dbgLog('modal cooldown active, skipping outcome modal for', targetGameId);
      return;
    }

    // MARQUER IMMÉDIATEMENT comme déjà montré pour éviter les courses (important)
    state.outcomeShownForGameId = String(targetGameId);
    // 1.2s de protection contre ré-affichages
    state._modalCooldown = Date.now() + 1200;

    // Avant d'afficher : tenter de resynchroniser l'état autoritatif (le winner a peut-être appelé finalize)
    // -> garantit d'afficher des scores à jour dans la modale
    try {
      await pollGameOnce(); // rafraîchit state.* (board, party_id, series_id, etc)
      // fetch parties/series rows si existants pour obtenir owner/opponent wins (best-effort)
      if (state.party_id) {
        const { data: prow } = await supabase.from('parties').select('owner_wins,opponent_wins').eq('id', state.party_id).maybeSingle();
        if (prow) {
          state.party_owner_wins = Number(prow.owner_wins ?? state.party_owner_wins);
          state.party_opponent_wins = Number(prow.opponent_wins ?? state.party_opponent_wins);
        }
      }
      if (state.series_id) {
        const { data: srow } = await supabase.from('series').select('owner_wins,opponent_wins').eq('id', state.series_id).maybeSingle();
        if (srow) {
          state.series_owner_wins = Number(srow.owner_wins ?? state.series_owner_wins);
          state.series_opponent_wins = Number(srow.opponent_wins ?? state.series_opponent_wins);
        }
      }
    } catch (e) {
      dbgLog('outcome modal pre-fetch failed (ignorable)', e);
    }
...
if (state.match_id) try { joinMatchRPC(state.match_id); } catch(e){ dbgLog('joinMatchRPC startup failed', e); }
  // ensure party poller won't treat the current game as "new"
if (!state.lastPartyKnownGameId) {
  state.lastPartyKnownGameId = String(gameId ?? '');
  dbgLog('startup: initialized lastPartyKnownGameId to', state.lastPartyKnownGameId);
}
} finally {
