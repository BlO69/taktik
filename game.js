...
try {
 ...
 // now show modal (let showNewRoundModal handle marking idempotently)
try {
  showNewRoundModal({
    party_id: state.party_id,
    party_owner_wins: state.party_owner_wins,
...
if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(targetGameId)) {
  ...
  // allow the local winner to still see the outcome modal even if a new-round marker exists
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  if (!amLocalWinner && (Date.now() - when < 15000)) { // 15s window
    ...
    return;
  } else if (!amLocalWinner && (Date.now() - when >= 15000)) {
    // stale marker -> clear and allow outcome modal
    state.newRoundShownForGameId = null;
    state._newRoundShownAt = null;
  } else {
    dbgLog('allow outcome modal for local winner despite new-round marker', state.userId, 'target', targetGameId);
  }
}
