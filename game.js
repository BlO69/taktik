...
export default async function initGame(supabaseClient) {
  ...
  // - if server provided an object map (keys are positions) => convert keys to string ints
    ...
    if (!Array.isArray(serverBoard) && typeof serverBoard === 'object') {
      ...
      try {
        ...
        for (const k of Object.keys(serverBoard)) {
          // keep only defined X/O/null values (but don't coerce away valid values)
    ...
    try {
      ...
      if (Array.isArray(data)) {
        for (const row of data) {
          if (!row) continue;
          if (String(row.id) === String(ownerId)) state.ownerPseudo = row.pseudo;
          if (String(row.id) === String(opponentId)) state.opponentPseudo = row.pseudo;
        }
      }
    } catch (e) {
  ...
  // NOTE: we intentionally DO NOT send p_position to RPC to avoid server-side
  // inconsistency exceptions - the server will recompute pos from p_row/p_col.
  async function placeMoveRPC(pos, row, col, playerChar) {
    ...
    try {
      ...
      // IMPORTANT: omit p_position to avoid "incohérence" errors if server computes pos differently.
      ...
      if (error) {
        ...
        // If server says not allowed because game finished, try to sync state & animate
        await pollGameOnce();
      ...
      if (gameFinished) {
        ...
        if (resp.winner) {
          ...
          const winnerPseudo = (String(resp.winner) === String(state.ownerId)) ? state.ownerPseudo : state.opponentPseudo;
      ...
      chan.on('postgres_changes', {
        ...
        (async () => {
          ...
          if (rec.status === 'finished') {
            ...
            if (rec.winner) {
              state.winnerPseudo = (String(rec.winner) === String(state.ownerId)) ? state.ownerPseudo : state.opponentPseudo;
            }
            // UI text
            updateTurnUI();
            setTimeout(()=>{
  ...
  async function pollGameOnce() {
    ...
    try {
      ...
      if (g.status === 'finished') {
        if (g.winner) {
          state.winnerPseudo = (String(g.winner) === String(state.ownerId)) ? state.ownerPseudo : state.opponentPseudo;
        }
