...
const intendedSeriesId = (state.series_id || (g && g.series_id)) ? String(state.series_id ?? (g && g.series_id)) : null;
const amLocalSeriesWinner = state.userId && state.series_winner && (String(state.userId) === String(state.series_winner));

// if we have a real id and it was dismissed by the user previously, skip for non-winners
if (intendedSeriesId && wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinner) {
  dbgLog('series modal previously dismissed, skipping show', intendedSeriesId);
// if not the local winner and we can't mark it (already marked by another handler), skip
} else if (!amLocalSeriesWinner && intendedSeriesId && !tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
  dbgLog('series already marked, skipping show', intendedSeriesId);
} else {
  // ensure the local series winner still sees the modal; mark it so other clients skip
  if (amLocalSeriesWinner && intendedSeriesId) tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
  try {
    showSeriesModal({
      series_id: state.series_id,
      series_owner_wins: state.series_owner_wins,
...
if (info.new_game_id) {
  ...
  try {
        // compute a robust series id to use for marking.
    // If no real series id is available, DO NOT persist to localStorage (use runtime-only sentinel)
    const intendedSeriesId = (info?.series_id || state.series_id) ? String(info?.series_id ?? state.series_id) : null;

// remplacement du markDismissed local (dans showSeriesModal)
const markDismissed = () => {
  try {
    // if we have a real series id, persist dismissal; otherwise keep a runtime-only sentinel
    const sid = intendedSeriesId ? String(intendedSeriesId) : '__none_runtime__';
    ...
    if (intendedSeriesId) {
      ...
      // runtime-only mark: do not persist (prevents future real-series from being blocked)
      state['seriesFinishedShownForSeriesId'] = sid;
    }
...
btns.appendChild(closeBtn);     
  ...
  async function createNewSeriesFromSeries(parentSeriesId, lastRoundWinner = null) {
    try {
      ...
      // CLEANUP: remove persistent modal keys for the parent series id (avoid stale dismissals)
// This prevents a missing/empty id stored earlier from blocking subsequent series modals.
try {
  const prevSid = parentSeriesId ? String(parentSeriesId) : null;
  if (prevSid) {
    try {
      localStorage.removeItem(`taktik_modal_dismissed_seriesFinishedShownForSeriesId_${prevSid}`);
      localStorage.removeItem(`taktik_modal_shown_seriesFinishedShownForSeriesId_${prevSid}`);
      dbgLog('cleaned up previous series modal keys', prevSid);
    } catch (e) {
      dbgLog('cleanup prev series modal keys failed', e);
    }
  }
} catch (e) {
  dbgLog('cleanup prev series modal outer failed', e);
}
