...
function joinMatchRPC(matchId) {
  ...
  function updatePlayerUI() {
    ...
    setText('seriesOpponentWins', state.series_opponent_wins ?? '');
      // (juste avant la fin de updatePlayerUI)
...
//       La série sera marquée/affichée de façon idempotente ailleurs.
    } catch (e) {
  ...
  // === AVANT D'AFFICHER LA MODALE : on rejoint d'abord la nouvelle partie
try {
  // compute starterChar quickly (prefer server-provided current_turn)
  ...
  // protect against races & make this game known
  state.lastPartyKnownGameId = String(g.id);
  gameId = g.id;

  // join the new game first so losers/spectators get the cleared board & updated scores
  try {
    // Pour s'assurer que tous les clients (perdant / spectateur) peuvent voir la nouvelle modale proprement
state.newRoundShownForGameId = null;
state.outcomeShownForGameId = null;
    await joinNewGame(g.id, starterChar);
  } catch (joinErr) {
    dbgLog('joinNewGame before modal failed (non-fatal)', g.id, joinErr);
    // fallback: do an authoritative poll so UI will update even if join failed
    try { await pollGameOnce(); } catch (pErr) { dbgLog('pollGameOnce after join failure also failed', pErr); }
  }

  // ensure we have authoritative values for the modal (best-effort)
  try { await pollGameOnce(); } catch(e){ dbgLog('pollGameOnce before showing new-round modal failed', e); }

  // Ensure we have authoritative party/series counters BEFORE showing modal
try {
  const partyIdToFetch = g.party_id ?? state.party_id;
  const seriesIdToFetch = g.series_id ?? null;
  await fetchAndApplyPartyAndSeries(partyIdToFetch, seriesIdToFetch);
  dbgLog('fetchAndApplyPartyAndSeries completed before showing new-round modal', partyIdToFetch, seriesIdToFetch);
} catch (e) {
  dbgLog('ensure scores before modal failed', e);
}
// now decide whether to show the modal (idempotent)
// allow the local round-winner to still see the modal even if another client already marked it
const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));

if (!amLocalWinner) {
  // non-winner: attempt to mark first; if already marked, skip showing
  if (!tryMarkModalShown('newRoundShownForGameId', g.id)) {
    dbgLog('newRound already marked, skipping show for non-winner', g.id);
  } else {
    showNewRoundModal({
      party_id: state.party_id,
      party_owner_wins: state.party_owner_wins,
  ...
  // winner: do NOT pre-mark — let showNewRoundModal show and then mark after append
  showNewRoundModal({
    party_id: state.party_id,
    party_owner_wins: state.party_owner_wins,
    party_opponent_wins: state.party_opponent_wins,
  ...
  dbgLog('auto-join + showNewRoundModal block failed', e);
    ...
    if (state.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
      try {
// ancienne ligne: showSeriesModal({ ... });
// remplacer par :

// mark as shown early to prevent concurrent callers from also showing it
...
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerGlobal) {
  ...
  try {
    ...
    // CATCH AJOUTÉ : journaliser toute erreur qui survient en amont
    ...
    try {
      ...
      if (!starterChar) {
        if (state.lastRoundWinnerId) {
          // NOTE: adapt field names if your row uses ownerId/opponentId instead of owner_id/opponent_id
          if (String(state.lastRoundWinnerId) === String(g.owner_id)) starterChar = 'X';
          else if (String(state.lastRoundWinnerId) === String(g.opponent_id)) starterChar = 'O';
        }
      }
      ...
      // (optionnel) if you manage gameId here, switch after marking above
      gameId = g.id;

      // then join — do it inside try/catch to handle failures gracefully
      try {
        await joinNewGame(g.id, starterChar);
      } catch (err) {
        dbgLog('joinNewGame failed for new game', g.id, err);
        // fallback: force a poll so client resynchronises
        try { await pollGameOnce(); } catch (_) { /* ignore */ }
      }
    } catch (e) {
...
try {
  if (_shouldHideLoading) {
    ...
    // Maintenant que l'overlay est retiré et isTransitionLoading=false,
    // forcer un rendu du board et mise à jour des scores pour le perdant.
    try { renderBoard(state.board); } catch(e){ dbgLog('renderBoard after exitTransition failed', e); }
    try { updatePlayerUI(); } catch(e){ dbgLog('updatePlayerUI after exitTransition failed', e); }
  }
} catch(e){ dbgLog('exitTransitionForNonWinners failed', e); }
...
async function pollPartyNewGameOnce() {
  ...
  try {
    ...
    // PATCH 2: forcer un fetch DB pour peupler state.series_winner même si le RPC ne l'a pas retourné
    try {
      await fetchAndApplyPartyAndSeries(state.party_id, state.series_id);
      dbgLog('pre-series-check fetchAndApply completed', state.party_id, state.series_id);
    } catch(e) { dbgLog('pre-patch3 fetchAndApply failed', e); }
...
try {
  ...
  try { state.outcomeShownForGameId = String(prevGameId); } catch(_) {}
      ...
      // mark the new-round shown & last known game id BEFORE switching gameId to avoid races
state.lastPartyKnownGameId = String(newGameId);
      
      // assign the new game id (switch context)
      gameId = newGameId;
      exposeGameStateToWindow();
...
if (newGameId) {
 ...
 async function showNewRoundModal(info) {
  try {
    // --- PATCH : montrer la modale NOUVELLE MANCHE uniquement au gagnant de la manche,
// et ne PAS l'afficher au gagnant si la SERIE est déjà close (la modal série doit s'afficher)
if (!info) info = {};
// 1) si on est pas encore syncé, queue comme avant
if (!state.initialSyncDone) {
  dbgLog('queue new-round modal until initial sync', info?.new_game_id);
  state._pendingNewRoundModal = state._pendingNewRoundModal || [];
  const nid = info && info.new_game_id ? String(info.new_game_id) : null;
  // avoid pushing duplicates for same game id
...
// 2) si la série est terminée ET que l'utilisateur courant est le gagnant de la série,
// afficher immédiatement showSeriesModal au lieu de retourner silencieusement
if (info.series_winner && state.userId && String(info.series_winner) === String(state.userId)) {
  dbgLog('local series winner detected — showing series modal instead of new-round modal', info.series_winner);
  try {
    // pass as much info as we have so showSeriesModal can render immediately
...
// 3) Montrer la new-round modal seulement au gagnant de la manche (pas au perdant/spectateur).
const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
if (!amLocalWinner) {
  dbgLog('suppress new-round modal for local non-winner (spectator/loser)', info?.new_game_id);
  return;
}

// idempotent mark (si on a un new_game_id)
if (info && info.new_game_id) {
  tryMarkModalShown('newRoundShownForGameId', info.new_game_id);
}

    // ensure info has authoritative scores (DB or rpc)
    info = await ensureModalHasAuthoritativeScores(info);
// --- PATCH: ne pas afficher la modale "nouvelle manche" pour le perdant local ---
// Si l'utilisateur est l'un des deux joueurs et qu'il n'est PAS le dernier gagnant,
// on supprime l'affichage de la modale (il a déjà été re-join automatiquement).
if (state.userId && (String(state.userId) === String(state.ownerId) || String(state.userId) === String(state.opponentId))
    && state.lastRoundWinnerId && String(state.userId) !== String(state.lastRoundWinnerId)) {
  dbgLog('suppress new-round modal for local loser', info?.new_game_id);
  return; // ne pas construire/afficher la modale
}
...
if (info.new_game_id) {
  ...
  // LIBELLÉ changé : 'Continuer la partie'
  joinBtn.textContent = 'Continuer la partie';
  ...
  // Comportement : afficher overlay de chargement, puis rejoindre la partie si new_game existe.
  joinBtn.onclick = async () => {
    try {
      // pour le joueur qui clique, on ferme la modale UI immédiatement
      overlay.remove();

      // montrer overlay global de chargement (UI)
      showLoadingOverlay();

      const targetGameId = info.new_game_id || null;
      // compute starterChar as before (optional) — prefer last_round_winner if present
      let starterChar = null;
      ...
      if (targetGameId) {
        try {
          await joinNewGame(targetGameId, starterChar);
        } catch (e) {
          dbgLog('join from new-round modal failed, fallback poll', e);
          try { await pollGameOnce(); } catch (_) {}
        }
      } else {
        try { await pollGameOnce(); } catch (e) { dbgLog('pollGameOnce in joinBtn failed', e); }
      }
    } catch (e) {
      dbgLog('joinBtn onclick failed', e);
    } finally {
      hideLoadingOverlay();
    }
  };
  btns.appendChild(joinBtn);
    ...
    // === post-append idempotent marking so winner still sees the modal exactly once ===
try {
  if (info && info.new_game_id) {
    tryMarkModalShown('newRoundShownForGameId', info.new_game_id);
    state.newRoundShownForGameId = String(info.new_game_id);
    state._newRoundShownAt = Date.now();
    dbgLog('newRound modal post-append marked for new_game_id', info.new_game_id);
  }
} catch (e) { dbgLog('post-append mark newRound failed', e); }
    } catch (e) {
...
try {
  state.seriesFinishedShownForSeriesId = null;
  state.seriesFinishedShownForSeriesIdDismissed = null;
  // Purger aussi les clés localStorage (ancienne série et fallback __none__)
  const _keysToDelete = [
  ...
  for (const _k of _keysToDelete) {
    try {
      ...
      localStorage.removeItem(`taktik_modal_shown_seriesFinishedShownForSeriesId_${_k}`);
    } catch(_e) {}
  }
} catch(e) { 
...
if (state._pendingSeriesModal && state._pendingSeriesModal.length) {
  ...
  for (const item of state._pendingNewRoundModal) {
    try {
      const partyIdToFetch = item.info?.party_id ?? null;
      const seriesIdToFetch = item.info?.series_id ?? null;
      await fetchAndApplyPartyAndSeries(partyIdToFetch, seriesIdToFetch);
      showNewRoundModal(item.info);
    } catch(e) {
      dbgLog('showNewRoundModal from pending failed', e);
    }
  }
