...
state.lastPartyKnownGameId = String(g.id);
  ...
  async function pollPartyNewGameOnce() {
    try {
      ...
      // If we don't know lastPartyKnownGameId, initialize it to the current known gameId
// (so we don't immediately re-handle the already-open current game).
...
try { updateTurnUI(); } catch (e) {}
    ...
    gameId = newGameId;
    exposeGameStateToWindow();
    // --- PATCH: s'assurer qu'on est abonné aux updates party/series du NOUVEAU jeu/série
try {
  // setupScoreSubscriptions va d'abord cleanup puis (re)subscribe aux party/series courants
  await setupScoreSubscriptions();
  // forcer mise à jour UI des scores immédiatement (si les rows existent)
  updatePlayerUI();
  dbgLog('post-join: setupScoreSubscriptions executed');
} catch (e) {
