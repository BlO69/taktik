  ...
  dbgLog('enqueue new-round modal failed', e);
    ...
    if (newGameId) {
      if (!state.initialSyncDone) {
        ...
        // remplacer la ligne tryMarkModalShown('newRoundShownForGameId', newGameId);
if (tryMarkModalShown('newRoundShownForGameId', newGameId)) {
  // si le marquage vient d'être fait par nous, afficher immédiatement la modale pour le finaliseur
  showNewRoundModal({
    party_id: state.party_id,
    party_owner_wins: state.party_owner_wins,
    party_opponent_wins: state.party_opponent_wins,
  ...
  dbgLog('newRound already marked (finalize path) — skipping show', newGameId);
}     
      }
    }
