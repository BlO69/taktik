...
export default async function initGame(supabaseClient) {
  ...
  async function handleFinalizeRoundAndFollowup(p_game_id, options = {}) {
    try {
      ...
      // --- CHANGED: if server created a new game, reset local board/state immediately for THIS client ---
  ...
  async function boot() {
    ...
    if (!gameId) {
      ...
      state.playerChar = computePlayerChar(state.userId);
      // ensure global subscription exists to detect new games in the same party
      await setupGlobalGamesInsertSubscription();
      // subscribe to game realtime/moves
      await setupRealtime();
      // subscribe to party/series scores if present
      await setupScoreSubscriptions();
      // start polling fallback if realtime not active
      startPolling();
    }
  }

  // Start boot without awaiting to not block import; return an interface
  boot().catch(e => dbgLog('boot failed', e));

  return {
    joinNewGame,
    handleFinalizeRoundAndFollowup,
    pollGameOnce,
    pollMovesOnce,
    setupRealtime,
    setupGlobalGamesInsertSubscription
  };
}
