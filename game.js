...
function joinMatchRPC(matchId) {
  ...
  try {
    ...
    // --- PATCH: mark the new game id early to avoid poll/race reading the old game ---
state.lastPartyKnownGameId = String(g.id);
gameId = g.id;
try { exposeGameStateToWindow(); } catch(e){ dbgLog('exposeGameStateToWindow failed after early gameId set', e); }
    ...
    if (prevGameId) {
      state.outcomeShownForGameId = String(prevGameId);
        ...
        try {
      // Queue the new-round modal until we have authoritative scores / initial sync.
      // Avoid calling showNewRoundModal immediately because state.party_id / scores
      // may still reflect the previous game and cause double-flash on the loser.
      state._pendingNewRoundModal = state._pendingNewRoundModal || [];
      const nid = String(g.id);
      if (!state._pendingNewRoundModal.find(it => String(it.info?.new_game_id) === nid)) {
        state._pendingNewRoundModal.push({
          info: {
            party_id: g.party_id ?? state.party_id,
        ...
        dbgLog('queued new-round modal until authoritative sync', nid);
      } else {
        dbgLog('new-round modal already queued', nid);
      }
    } catch (e) {
      dbgLog('queueing newRound modal failed', e);
    }
    ...
    // Show the new-round modal (only once per new game) — mark early to avoid races
try {
  if (!tryMarkModalShown('newRoundShownForGameId', g.id)) {
    dbgLog('newRound already marked, skipping show', g.id);
  } else {
    // pass current state values (these are best-effort; showNewRoundModal will re-fetch if needed)
    showNewRoundModal({
