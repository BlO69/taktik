// game.js (version corrigée : serveur authoritative, normalisation board, no duplicates)
// Basé sur la version fournie par l'utilisateur (corrigée)
...
// Normalize any board representation into an array of length ROWS*COLS with values 'X','O' or null.
// This prevents malformed server payloads from mapping to position 0 etc.
function normalizeBoard(raw) {
  const size = ROWS * COLS;
  const out = new Array(size).fill(null);
  if (raw == null) return out;

  // If it's already a proper array of the right length, copy validated values
  if (Array.isArray(raw)) {
    // If server wrongly sends sparse arrays or different length, be cautious
  ...
  if (typeof raw === 'string') {
    try {
      ...
      dbgLog('normalizeBoard: JSON.parse failed for raw string', e);
      return out;
    }
  }

  // If object mapping position->symbol
  if (typeof raw === 'object') {
    for (const k of Object.keys(raw)) {
      // avoid coercing invalid keys to 0; parse and validate
      const pos = Number(k);
      if (!Number.isFinite(pos)) {
        dbgLog('normalizeBoard: skipping non-numeric board key', k);
        continue;
      }
      if (pos < 0 || pos >= size) {
        dbgLog('normalizeBoard: skipping out-of-range board key', pos);
        continue;
      }
      const v = raw[k];
      if (v === 'X' || v === 'O') out[pos] = v;
      else {
  ...
  // fallback: unknown shape -> empty board
  dbgLog('normalizeBoard: unknown board shape, returning empty board');
  return out;
}
...
function renderGrid() {
  ...
  // Normalize to strict array shape before rendering (prevents odd shapes from producing weird indexing).
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      // remove previous listeners risk by always re-creating node
      cell.addEventListener('click', () => onCellClick(r, c));

      const val = board[i];
      if (val === 'X' || val === 'O') {
        // Only one image appended per cell; we clear cell content by new node creation above
  ...
  const doPoll = async () => {
    ...
    try {
      ...
      for (const m of moves) {
        ...
        if (pos < 0 || pos >= ROWS * COLS) { dbgLog('poll skipping out-of-range pos', pos); continue; }
...
function applyRealtimeMove(rec) {
  try {
    ...
    if (!Number.isFinite(pos)) { dbgLog('applyRealtimeMove: invalid pos', rec); return; }
    if (pos < 0 || pos >= ROWS * COLS) { dbgLog('applyRealtimeMove: out of range', pos); return; }
    ...
    // ensure canonical board shape
    if (!Array.isArray(board)) board = normalizeBoard(board);

    // Only set if different (avoid useless writes)
    if (board[pos] !== player) {
      board[pos] = player;
    } else {
      dbgLog('applyRealtimeMove: board already has same value, skipping write', { pos, player });
    }

    moveIndex = Math.max(moveIndex, Number(rec.move_index ?? moveIndex));
    // Render from canonical board (no extra DOM nodes duplication, single source of truth)
    renderGrid();
    current = (player === 'X') ? 'O' : 'X';
    if (currentTurnEl) currentTurnEl.textContent = current;

    // Refresh canonical state but avoid aggressive re-rendering if not necessary
    ...
    if (gRow?.board) {
      ...
      try {
        ...
        if (!movesErr && Array.isArray(moves) && moves.length) {
          ...
          for (const m of moves) {
            const pos = Number(m.position);
            if (!Number.isFinite(pos) || pos < 0 || pos >= ROWS * COLS) { dbgLog('refreshGameState: skipping bad move pos', m); continue; }
            if (m.player == null) continue;
            newBoard[pos] = m.player;
            const mi = Number(m.move_index ?? NaN);
            if (Number.isFinite(mi)) maxIdx = Math.max(maxIdx, mi);
          }
          board = newBoard;
...
async function placeMove(r, c, playerSymbol, options = {}) {
  ...
  try {
    ...
    board = normalizeBoard(board); // ensure array shape
    ...
    if (supabase && isRemotePlaying && gameId) {
      try {
        ...
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          ...
          // **SERVER AUTHORITATIVE MERGE**: if server returned a board, normalize and replace entirely
          if (rpcRow && rpcRow.board) {
            try {
              board = normalizeBoard(rpcRow.board);
              // update moveIndex if server provided it
              if (rpcRow.move_count != null) {
                const mc = Number(rpcRow.move_count);
                if (Number.isFinite(mc)) moveIndex = Math.max(moveIndex, mc);
              }
              renderGrid(); renderMiniMap();
            } catch (e) {
            ...
            // fallback: fetch last move and apply (but be strict about validity)
            try {
              const { data: lastMove, error: lastErr } = await supabase.from(MOVES_TABLE)
                ...
                .maybeSingle();
              if (!lastErr && lastMove) {
                // Only apply lastMove if it's valid; applyRealtimeMove has safeguards
...
async function joinGame(gId) {
  ...
  try {
    ...
    if (Array.isArray(moves)) {
      for (const m of moves) {
        try {
          ...
          const player = m.player;
          if (!Number.isFinite(pos) || player == null) continue;
          if (pos < 0 || pos >= ROWS * COLS) { dbgLog('joinGame skipping out-of-range pos', pos); continue; }
          board[pos] = player;
    ...
    // If server provided canonical board, prefer it (normalized)
    if (gameRow?.board) {
      board = normalizeBoard(gameRow.board);
    }
