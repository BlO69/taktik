...
async function fetchGameIdFromInvitation(invId) {
  ...
  try {
    // lecture défensive : maybeSingle() pour éviter d'échouer si row absente ou RLS
    ...
    // fallback : si on a series_id mais pas game_id, essayer de résoudre la game
    if (data?.series_id) {
      try {
        // reproduit la logique de resolveGameIdFromInvitation de create_game.js
        ...
        // prefer game où l'utilisateur courant est owner/opponent si possible
        try {
        ...
        // fallback: return latest game id
...
(async function initFromUrl() {
  // --- FIX: assure que supabase est bien disponible ---
  supabase = window.supabase ?? supabase;

  if (!supabase) {
    console.error("initFromUrl: supabase indisponible");
    return;
  }
...
async function joinGame(gId) {
  if (!gId) {
    dbgLog('joinGame called without game id');
    dbgAlert('joinGame', 'appel sans game_id — annulation');
    return;
  }
  if (!supabase) {
    dbgLog('joinGame: supabase client missing');
    dbgAlert('joinGame', 'supabase manquant — passage en local');
    resetLocalGame();
    return;
  }

  dbgLog('joinGame START', gId);
  dbgAlert('joinGame START', `game_id = ${gId}`);

  try {
    // --- 1) load canonical game row ---
    dbgLog('joinGame: loading game row from DB...');
    dbgAlert('joinGame', 'Chargement de la ligne game depuis la DB...');
    ...
    dbgAlert('joinGame: fetched gameRow', JSON.stringify({
      gId,
      gameRow: gameRow ? { id: gameRow.id, owner_id: gameRow.owner_id, opponent_id: gameRow.opponent_id, current_turn: gameRow.current_turn, status: gameRow.status } : null,
      gameErr: gameErr ? (gameErr.message || gameErr) : null
    }, null, 2));

    if (gameErr) {
      dbgLog('joinGame: game fetch error', gameErr);
      dbgAlert('joinGame ERROR', 'Erreur lors du fetch gameRow: ' + (gameErr.message || String(gameErr)));
      throw gameErr;
    }
    if (!gameRow) {
      dbgLog('joinGame: game not found');
      dbgAlert('joinGame ERROR', 'Partie introuvable (gameRow null)');
      throw new Error('Game introuvable');
    }

    // --- 2) load moves ---
    dbgLog('joinGame: loading moves...');
    dbgAlert('joinGame', 'Chargement des coups (moves) ...');
    let moves = null, movesErr = null;
    try {
    ...
    dbgLog('joinGame: moves result', { movesCount: Array.isArray(moves) ? moves.length : 0, movesErr });
    dbgAlert('joinGame: moves', `movesCount=${Array.isArray(moves) ? moves.length : 0}\nmovesErr=${movesErr ? (movesErr.message || String(movesErr)) : 'none'}`);

    // --- reset local state and apply moves ---
    dbgLog('joinGame: resetting local game state');
    dbgAlert('joinGame', 'Réinitialisation du jeu local');
    resetLocalGame();

    if (!movesErr && Array.isArray(moves)) {
      dbgLog('joinGame: applying moves to local board');
      dbgAlert('joinGame', `Application de ${moves.length} coups au board local`);
      for (const m of moves) {
        try {
          const pos = Number(m.position);
          const player = m.player;
          if (!Number.isFinite(pos) || player == null) {
            dbgLog('joinGame: skipping invalid move', m);
            continue;
          }
          board[pos] = player;
          moveIndex = Math.max(moveIndex, Number(m.move_index ?? moveIndex));
        } catch (mvErr) {
          dbgLog('joinGame: exception applying move', mvErr, m);
        }
      }
    } else {
      dbgLog('joinGame: moves fetch failed or empty', movesErr);
      dbgAlert('joinGame', 'Fetch des moves échoué ou vide : ' + (movesErr ? (movesErr.message || String(movesErr)) : 'vide'));
    }

    dbgLog('joinGame: rendering grid and minimap');
    dbgAlert('joinGame', 'Rendu grille & minimap');
    renderGrid();
    renderMiniMap();

    // --- finalize local assignment ---
    gameId = gId;
    dbgLog('joinGame: set local gameId', gameId);
    dbgAlert('joinGame', 'gameId local défini : ' + gameId);

    // ensure fallback polling runs even if realtime subscribe fails
    try {
      startPolling();
      dbgLog('joinGame: startPolling invoked');
      dbgAlert('joinGame', 'Polling fallback démarré');
    } catch (e) {
      dbgLog('joinGame: startPolling failed', e);
      dbgAlert('joinGame', 'startPolling a échoué : ' + String(e));
    }
    ...
    dbgLog('joinGame: ids', { hostId, opponentId, current });
    dbgAlert('joinGame', `owner/opponent/current = ${hostId} / ${opponentId} / ${current}`);

    // --- determine local symbol based on session & roles ---
    try {
      sessionUser = (await supabase.auth.getSession())?.data?.session?.user ?? sessionUser;
      dbgLog('joinGame: sessionUser', sessionUser ? sessionUser.id : null);
      dbgAlert('joinGame', 'session user id: ' + (sessionUser?.id ?? 'null'));
    } catch (sessErr) {
      dbgLog('joinGame: getSession failed', sessErr);
      dbgAlert('joinGame', 'getSession a échoué: ' + String(sessErr));
    }
    ...
    dbgLog('joinGame: local symbol/isHost', { localPlayerSymbol, isHost });
    dbgAlert('joinGame', `localSymbol=${localPlayerSymbol} isHost=${isHost}`);

    if (currentTurnEl) currentTurnEl.textContent = current;

    // --- subscribe to realtime events ---
    dbgLog('joinGame: subscribing to realtime for game', gId);
    dbgAlert('joinGame', 'Abonnement realtime en cours...');
    try {
      await subscribeToGame(gId);
      dbgLog('joinGame: subscribeToGame ok');
      dbgAlert('joinGame', 'subscribeToGame OK');
    } catch (subErr) {
      dbgLog('joinGame: subscribeToGame failed', subErr);
      dbgAlert('joinGame', 'subscribeToGame a échoué : ' + String(subErr));
      // continue — refreshGameState + polling will try to keep sync
    }

    // --- refresh canonical state (party/series counters, etc.) ---
    dbgLog('joinGame: refreshGameState start');
    dbgAlert('joinGame', 'Mise à jour de l\'état canonique (refreshGameState)...');
    try {
      await refreshGameState();
      dbgLog('joinGame: refreshGameState ok');
      dbgAlert('joinGame', 'refreshGameState OK');
    } catch (rfErr) {
      dbgLog('joinGame: refreshGameState failed', rfErr);
      dbgAlert('joinGame', 'refreshGameState a échoué : ' + String(rfErr));
    }

    // --- signal join ---
    try {
      window.dispatchEvent(new Event('taktik:joined'));
      dbgLog('joinGame: dispatched taktik:joined');
      dbgAlert('joinGame', 'Événement taktik:joined dispatché');
    } catch (evErr) {
      dbgLog('joinGame: dispatch event failed', evErr);
      dbgAlert('joinGame', 'dispatch taktik:joined failed: ' + String(evErr));
    }

    dbgLog('joinGame SUCCESS', gId);
    dbgAlert('joinGame SUCCESS', `Partie ${gId} chargée avec succès.`);
  } catch (e) {
    console.warn('joinGame failed', e);
    // verbose debug alerts/logging
    try {
      dbgLog('joinGame CATCH', e);
      try {
        dbgAlert('joinGame FAILURE', `Impossible de charger la partie : ${String(e && (e.message || e))}`);
      } catch (_) {
        /* ignore alert errors */
      }
    } catch (_) {}

    // user-facing short alert (same message as screenshot)
