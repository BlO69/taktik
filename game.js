...
function tryMarkModalShown(stateKey, id) {
  try {
    ...
    // si déjà marqué -> false
    if (state[stateKey] && String(state[stateKey]) === sid) return false;
    // marque immédiatement (atomique côté client)
    state[stateKey] = sid;
    return true;
  } catch (e) {
...
async function fetchAndApplyPartyAndSeries(partyId, seriesId) {
  ...
  try {
    ...
    // n'ignorer que si ON EST DÉJÀ dans la nouvelle partie OU si on a explicitement traité cette new-game.
// cela évite de sauter le traitement sur les clients qui ne l'ont pas encore jointe.
if (String(state.newRoundShownForGameId) === String(g.id) && String(gameId) === String(g.id)) {
  dbgLog('new game already processed (we are already in that game)', g.id);
  return;
}
...
if (finalizeRow.series_winner) {
  try {
    ...
    if (newGameId) {
      ...
      // Marque idempotente : nous avons bien rejoint la nouvelle partie, on peut déclarer la modal "nouvelle manche" montrée.
...
// si on sait déjà qu'une NEW GAME a été annoncée pour la party,
// ne pas afficher l'outcome de l'ancienne partie (empêche flash coté perdant)
if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(targetGameId)) {
  dbgLog('suppress outcome modal because new game already shown for', state.newRoundShownForGameId, 'target', targetGameId);
  // marquer comme "déjà montré" pour bloquer autres handlers
