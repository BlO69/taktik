...
let _boardWasHidden = false;
try {
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  if (!amLocalWinner) {
    // Masquer aussi la board pour éviter le flickering visible sous l'overlay semi-transparent
    try {
      if (boardEl) { boardEl.style.visibility = 'hidden'; _boardWasHidden = true; }
      // Fermer toutes les modales existantes silencieusement avant d'afficher l'overlay
      ['taktik-outcome-modal','taktik-new-round-modal','taktik-series-modal'].forEach(id => {
        try { const el = document.getElementById(id); if (el) el.remove(); } catch(_) {}
      });
    } catch (e) { dbgLog('pre-hide board/modals failed', e); }
    showLoadingOverlay();
    _shouldHideLoading = true;
    dbgLog('showing loading overlay for non-winner during global new-game handling', g.id);
  }
} catch (e) {
...
//       La série sera marquée/affichée de façon idempotente ailleurs.
    ...
    // Ne re-rendre la board et l'UI qu'si l'overlay de chargement n'est PAS affiché (gagnant).
    // Pour le perdant, on attend la fin complète du chargement pour éviter le flickering.
...
try {
  ...
  // now mark & show modale (idempotent)
// allow the local round-winner to still see the modal even if another client already marked it
const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
if (!tryMarkModalShown('newRoundShownForGameId', g.id) && !amLocalWinner) {
  dbgLog('newRound already marked, skipping show', g.id);
} else {
  showNewRoundModal({
    party_id: state.party_id,
    party_owner_wins: state.party_owner_wins,
  ...
  dbgLog('auto-join + showNewRoundModal block failed', e);
    ...
    // Si la série est terminée, n'afficher la modale "Série terminée" QUE au gagnant de la série.
    // Le perdant verra l'overlay chargement jusqu'à ce que le gagnant lance la nouvelle série.
    if (state.series_winner) {
      try {
        ...
        // CORRECTIF 1: la modale série ne s'affiche que pour le gagnant local de la série.
        // Le perdant reste sur l'overlay "Chargement…" jusqu'à ce que la nouvelle série démarre.
        if (!amLocalSeriesWinnerGlobal) {
          dbgLog('series finished but local user is not winner — keeping loading overlay, skip series modal', intendedSeriesId);
          // _shouldHideLoading reste true : on ne cache PAS l'overlay pour le perdant ici.
          // Il sera retiré quand handleGlobalNewGameCreated sera rappelé pour la nouvelle série.
          _shouldHideLoading = false; // ne pas masquer l'overlay maintenant
        } else if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
          dbgLog('series modal previously dismissed, skipping show', intendedSeriesId);
        } else if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    // masquer overlay et révéler la board seulement maintenant que tout est chargé
try {
  if (_shouldHideLoading) {
    hideLoadingOverlay();
    // révéler la board proprement (état vide et à jour)
    try { renderBoard(state.board); } catch (e) { dbgLog('renderBoard reveal after loading failed', e); }
    try { updateTurnUI(); } catch (e) { dbgLog('updateTurnUI reveal after loading failed', e); }
    if (_boardWasHidden && boardEl) { boardEl.style.visibility = ''; }
  }
} catch(e){ dbgLog('hideLoadingOverlay+reveal failed', e); }
...
// ET effacer le localStorage pour permettre la réapparition à chaque nouvelle série
if (state.seriesFinishedShownForSeriesId) {
  try {
    const oldSid = String(state.seriesFinishedShownForSeriesId);
    localStorage.removeItem(`taktik_modal_shown_seriesFinishedShownForSeriesId_${oldSid}`);
    localStorage.removeItem(`taktik_modal_dismissed_seriesFinishedShownForSeriesId_${oldSid}`);
  } catch (e) { dbgLog('clear series localStorage markers failed', e); }
}
...
if (!amUserSeriesWinner) {
  if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    dbgLog('series modal already marked by another handler', intendedSeriesId);
    return;
  }
} else {
  // pour le gagnant de la série, s'assurer que le marqueur est positionné (force le marqueur pour le gagnant)
...
btns.appendChild(closeBtn);     
  ...
  try {
    ...
    // ET effacer le localStorage pour permettre à la prochaine série d'afficher la modale
    if (state.seriesFinishedShownForSeriesId) {
      try {
        const oldSid = String(state.seriesFinishedShownForSeriesId);
        localStorage.removeItem(`taktik_modal_shown_seriesFinishedShownForSeriesId_${oldSid}`);
        localStorage.removeItem(`taktik_modal_dismissed_seriesFinishedShownForSeriesId_${oldSid}`);
      } catch (e) { dbgLog('clear series localStorage on new series btn failed', e); }
    }
