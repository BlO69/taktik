...
// Déterminer si l'utilisateur courant est le gagnant de la série
const amUserSeriesWinner = Boolean(state.userId && String(state.userId) === String(state.series_winner));

// Si la modale a déjà été dismissée localement et que l'utilisateur N'EST PAS le gagnant -> skip
...
// Si l'utilisateur n'est pas le gagnant : on essaie de marquer et on affiche seulement si marquage OK
} else if (!amUserSeriesWinner) {
  if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    try {
      showSeriesModal({
        ...
        series_opponent_wins: state.series_opponent_wins,
        series_winner: state.series_winner,
        series_winner_pseudo: (state.series_winner && (String(state.series_winner) === String(state.owner_id) ? state.ownerProfile?.pseudo : state.opponentProfile?.pseudo)) || null,
        party_owner_wins: state.party_owner_wins,
        party_opponent_wins: state.party_opponent_wins,
...
// Si l'utilisateur EST le gagnant : forcer l'affichage (et marquer pour éviter concur.)
} else {
  ...
  try {
    ...
    dbgLog('showSeriesModal failed (for series winner)', e);
  }
}
} catch (e) {
...
const amUserSeriesWinner = Boolean(state.userId && String(state.userId) === String(finalizeRow.series_winner ?? state.series_winner));
...
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amUserSeriesWinner) {
  ...
  if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    try {
      showSeriesModal({
        ...
        series_winner_pseudo: (finalizeRow.series_winner && (String(finalizeRow.series_winner) === String(state.owner_id) ? state.ownerProfile?.pseudo : state.opponentProfile?.pseudo)) || null,
  ...
  try {
    ...
    dbgLog('showSeriesModal from finalize failed (for series winner)', e);
  }
}
}
...
// --- PATCH A: reset serie modal markers so future series endings will re-show to winner
try {
  const prevSeriesSid = state.seriesFinishedShownForSeriesId || null;
  if (prevSeriesSid) {
    try {
      localStorage.removeItem(`taktik_modal_shown_seriesFinishedShownForSeriesId_${prevSeriesSid}`);
      localStorage.removeItem(`taktik_modal_dismissed_seriesFinishedShownForSeriesId_${prevSeriesSid}`);
    } catch (e) {
      dbgLog('clear series modal storage failed', e);
    }
  }
  state.seriesFinishedShownForSeriesId = null;
  state.seriesFinishedShownForSeriesIdDismissed = null;
} catch (e) {
  dbgLog('reset series modal flags failed', e);
}
...
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amUserSeriesWinner) {
  ...
  if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    try {
      ...
      dbgLog('showSeriesModal failed (subscription)', e);
    }
  }
} else {
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
  try {
    ...
    dbgLog('showSeriesModal failed (subscription, for series winner)', e);
  }
}
            }
...
const amUserSeriesWinner = Boolean(state.userId && String(state.userId) === String(item?.info?.series_winner ?? state.series_winner));
...
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amUserSeriesWinner) {
  ...
  try {
    ...
    dbgLog('showSeriesModal from pending failed (for series winner)', e);
  }
}
