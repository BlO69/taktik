...
function updateCellAt(pos) {
  ...
  const currentOccupant = cell.dataset.occupant || null;
  const boardOccupant = board[pos] || null;
  ...
  cell.innerHTML = '';
  if (boardOccupant) {
    cell.classList.remove('has-x', 'has-o');
...
// Helper : merge serverBoard into local board but preserve existing local occupants
function mergeBoardPreserveLocal(serverBoardRaw) {
  // normaliser serverBoardRaw -> array de length ROWS*COLS
  let sb = null;
  if (!serverBoardRaw) sb = new Array(ROWS * COLS).fill(null);
  else if (typeof serverBoardRaw === 'string') {
    try { sb = JSON.parse(serverBoardRaw); } catch(e) { sb = null; }
  } else sb = serverBoardRaw;

  // si server a donné un objet map { "12":"X", ... } -> normaliser
  if (sb && !Array.isArray(sb) && typeof sb === 'object') {
    const tmp = new Array(ROWS * COLS).fill(null);
    Object.keys(sb).forEach(k => { const p = Number(k); if (Number.isFinite(p) && p >= 0 && p < ROWS*COLS) tmp[p] = sb[k]; });
    sb = tmp;
  }

  if (!Array.isArray(sb)) sb = new Array(ROWS * COLS).fill(null);

  const merged = new Array(ROWS * COLS).fill(null);
  for (let p = 0; p < ROWS * COLS; p++) {
    // preserve local occupant if present; otherwise take server
    merged[p] = board[p] != null ? board[p] : (sb[p] != null ? sb[p] : null);
  }

  board = merged;

  // rebuild appliedMoveIds from canonical board to avoid dup reapplication
  appliedMoveIds.clear();
  for (let p = 0; p < board.length; p++) if (board[p]) appliedMoveIds.add(`pos:${p}`);
}
...
async function refreshGameState() {
  ...
  try {
    ...
    if (gRow?.board) {
      try {
        mergeBoardPreserveLocal(gRow.board);
      } catch (e) { dbgLog('mergeBoardPreserveLocal failed', e); }
    } else {
      ...
      try {
        ...
        if (!movesErr && Array.isArray(moves) && moves.length) {
          ...
          mergeBoardPreserveLocal(newBoard);
          moveIndex = maxIdx;
        } else {
...
function resetLocalGame(stopAll = true) {
  ...
  // Option A (simple / sûr) : re-rendre toute la grille — renderGrid appellera updateCellAt pour chaque cellule.
  if (typeof renderGrid === 'function') {
    renderGrid();
  } else {
    // Option B (defensive) : si renderGrid absent, on vide manuellement toutes les cellules
    for (let pos = 0; pos < ROWS * COLS; pos++) {
      if (typeof updateCellAt === 'function') {
        updateCellAt(pos);
      } else if (boardGrid) {
