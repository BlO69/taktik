...
try {
  ...
  if (sm) {
    ...
    // si on n'a pas d'info, on laisse la modale (prévenir suppressions aveugles)
    if (modalSeriesId && String(state.series_id || '') !== modalSeriesId) {
      sm.remove();
    } // sinon : on conserve la modale active pour éviter le clignotement
  }
} catch (e) {
  dbgLog('modal cleanup failed', e);
    ...
    // --- PATCH D: ensure authoritative resync then show new-round modal to avoid flashing wrong scores ---
...
// small debounce to let score subscriptions / other clients settle (100-300ms)
await new Promise(r => setTimeout(r, 180));

// Show the new-round modal (only once per new game) AFTER resync so numbers are accurate
try {
  showNewRoundModal({
...
try { joinMatchRPC(matchId); } catch(e){ dbgLog('joinMatchRPC setupMatchSubscription failed', e); }
  ...
  async function pollGameOnce() {
    ...
    try {
      ...
      exposeGameStateToWindow();
// --- PATCH A: fetch party & series rows immediately so UI counters are correct on load ---
try {
  if (state.party_id) {
    const { data: prow, error: perr } = await supabase
      .from('parties')
      .select('owner_wins, opponent_wins, winner_id, series_id, match_id')
    ...
    if (!perr && prow) {
      ...
      // if series_id can be propagated from party row, prefer it
      if (prow.series_id) state.series_id = prow.series_id;
      if (prow.match_id && !state.match_id) state.match_id = prow.match_id;
    }
  }
  ...
  if (state.series_id) {
    const { data: srow, error: serr } = await supabase
      .from('series')
      .select('owner_wins, opponent_wins, winner_id, target_parties, match_id')
    ...
    if (!serr && srow) {
      ...
      if (srow.match_id && !state.match_id) state.match_id = srow.match_id;
    }
  }
} catch (e) {
  dbgLog('pollGameOnce: fetch party/series failed', e);
}
// Update UI counters right away
updatePlayerUI();
    ...
    // --- PATCH B: mark initialSyncDone after first successful pollGameOnce so modals behave deterministically ---
    if (!state.initialSyncDone) {
      state.initialSyncDone = true;
      dbgLog('initial sync done');
      // If there were pending series modals queued while sync was off, show them now
      if (Array.isArray(state._pendingSeriesModal) && state._pendingSeriesModal.length) {
        for (const p of state._pendingSeriesModal) {
          try { showSeriesModal(p.info); } catch(e){ dbgLog('showSeriesModal pending failed', e); }
        }
...
try {
  ...
  try {
    // --- PATCH C: if we already know a new round/new game is incoming, suppress outcome modal to avoid flash ---
if (state.newRoundShownForGameId && String(state.newRoundShownForGameId) !== String(forGameId ?? gameId)) {
  dbgLog('suppress outcome modal because newRoundShownForGameId present', state.newRoundShownForGameId);
  // mark as shown for the finished game so later handlers won't try again
  state.outcomeShownForGameId = String(forGameId ?? gameId);
  return;
}
  ...
  async function setupScoreSubscriptions() {
    try {
      ...
      if (state.series_id) {
        ...
        // fetch initial party/series snapshot to seed counters immediately
try {
  if (state.party_id) {
    const { data: p0, error: p0err } = await supabase.from('parties')
      .select('owner_wins, opponent_wins, winner_id')
      .eq('id', state.party_id).maybeSingle();
    if (!p0err && p0) {
      state.party_owner_wins = Number(p0.owner_wins ?? state.party_owner_wins ?? 0);
      state.party_opponent_wins = Number(p0.opponent_wins ?? state.party_opponent_wins ?? 0);
      state.party_winner = p0.winner_id ?? state.party_winner;
    }
  }
  if (state.series_id) {
    const { data: s0, error: s0err } = await supabase.from('series')
      .select('owner_wins, opponent_wins, winner_id')
      .eq('id', state.series_id).maybeSingle();
    if (!s0err && s0) {
      state.series_owner_wins = Number(s0.owner_wins ?? state.series_owner_wins ?? 0);
      state.series_opponent_wins = Number(s0.opponent_wins ?? state.series_opponent_wins ?? 0);
      state.series_winner = s0.winner_id ?? state.series_winner;
    }
  }
  updatePlayerUI();
} catch(e){ 
  dbgLog('setupScoreSubscriptions: initial snapshot failed', e);
}
      }
    } catch (e) {
