...
// Debug flag
let dbg = true;
// --- DBG overlay + android-toast friendly logging ---
// remplace les anciennes fonctions dbgLog / dbgAlert par ce bloc

// configuration rapide
const DBG_OVERLAY_ID = 'taktik-dbg-overlay';
const DBG_BTN_ID = 'taktik-dbg-toggle-btn';
const DBG_MAX_LINES = 500; // garder raisonnable

// crée overlay si nécessaire
function ensureDbgOverlay() {
  if (document.getElementById(DBG_OVERLAY_ID)) return document.getElementById(DBG_OVERLAY_ID);

  // conteneur
  const overlay = document.createElement('div');
  overlay.id = DBG_OVERLAY_ID;
  ...
  overlay.style.background = 'rgba(10,12,16,0.95)';
  overlay.style.color = '#e6eef8';
  overlay.style.fontSize = '12px';
  overlay.style.fontFamily = 'monospace';
  overlay.style.borderRadius = '10px';
  overlay.style.boxShadow = '0 8px 30px rgba(2,6,23,0.6)';
  ...
  clearBtn.onclick = () => { clearDbgPanel(); };
  ...
  copyBtn.onclick = () => {
    try {
      const s = Array.from(overlay.querySelectorAll('.dbg-line')).map(n => n.textContent).join('\n');
      navigator.clipboard?.writeText(s).then(()=>{ dbgToast('Copié'); }).catch(()=>{ dbgToast('Copie impossible'); });
    } catch (e) { dbgToast('Copie échouée'); }
  };
  ...
  list.style.gap = '6px';
  list.style.maxHeight = 'calc(46vh - 44px)';
  ...
  footer.innerHTML = `<span id="${DBG_OVERLAY_ID}-count">0</span><span>logs</span>`;
  overlay.appendChild(footer);

  document.body.appendChild(overlay);

  // floating toggle button
  if (!document.getElementById(DBG_BTN_ID)) {
    const btn = document.createElement('button');
    btn.id = DBG_BTN_ID;
    btn.title = 'Afficher les logs DBG';
    ...
    btn.style.background = 'linear-gradient(180deg,#ff9f1c,#ff6b6b)';
    btn.style.boxShadow = '0 8px 24px rgba(255,107,107,0.18)';
    btn.style.color = '#fff';
    btn.style.fontWeight = '700';
    btn.style.cursor = 'pointer';
    btn.textContent = 'DBG';
    btn.onclick = () => {
      const ov = document.getElementById(DBG_OVERLAY_ID);
      if (!ov) return;
      if (ov.style.display === 'none') showDbgPanel(); else hideDbgPanel();
    };
...
function dbgToast(msg) {
  // try Android bridge flavors
  try {
    if (window.Android && typeof window.Android.showToast === 'function') {
      window.Android.showToast(String(msg));
      return;
    }
    if (window.android && typeof window.android.showToast === 'function') {
      window.android.showToast(String(msg));
    ...
    // generic: try console + small ephemeral element
  } catch (e) { /* ignore bridge errors */ }

  // ephemeral in-page toast
  try {
    const id = 'taktik-dbg-toast';
...
function pushDbgLine(kind, text) {
  try {
    if (!dbg) return;
    const ov = ensureDbgOverlay();
    const list = ov.querySelector('div:nth-child(2)'); // first child=hdr, second=list
    if (!list) return;
    ...
    // style selon type
    if (kind === 'ALERT') {
      line.style.background = 'linear-gradient(90deg,#3b0d0d,#7a1f1f)';
      line.style.border = '1px solid rgba(255,120,120,0.14)';
      left.style.color = '#fff';
    } else if (kind === 'WARN') {
      line.style.background = 'linear-gradient(90deg,#5a3b00,#8a5b00)';
      left.style.color = '#fff';
    } else {
      line.style.background = 'rgba(255,255,255,0.03)';
      left.style.color = '#e6eef8';
    }

    line.appendChild(left);
    line.appendChild(right);

    // add click to copy this line
    line.onclick = () => {
      try {
        navigator.clipboard?.writeText(left.textContent || '').then(()=>{ dbgToast('Copié'); });
      } catch(e){ dbgToast('Copie impossible'); }
    };

    list.appendChild(line);

    // limit number of lines
    const lines = list.querySelectorAll('.dbg-line');
    if (lines.length > DBG_MAX_LINES) {
      lines[0].remove();
    }

    // update counter in footer
    const countEl = document.getElementById(`${DBG_OVERLAY_ID}-count`);
    if (countEl) countEl.textContent = String(list.querySelectorAll('.dbg-line').length);

    // autoscroll to bottom
    list.scrollTop = list.scrollHeight;
  } catch (e) { /* never crash app for debug UI */ }
}

// public small API
function showDbgPanel() { const ov = ensureDbgOverlay(); ov.style.display = 'block'; document.getElementById(DBG_BTN_ID).style.display = 'none'; }
function hideDbgPanel() { const ov = document.getElementById(DBG_OVERLAY_ID); if (ov) ov.style.display = 'none'; const b = document.getElementById(DBG_BTN_ID); if (b) b.style.display = 'block'; }
function clearDbgPanel() { const ov = document.getElementById(DBG_OVERLAY_ID); if (!ov) return; const list = ov.querySelector('div:nth-child(2)'); if (list) list.innerHTML = ''; const countEl = document.getElementById(`${DBG_OVERLAY_ID}-count`); if (countEl) countEl.textContent = '0'; }

// replace old dbgLog/dbgAlert
function dbgLog(...args) {
  try {
    ...
    // console still useful for desktop dev
    try { console.debug('DBG game.js:', ...args); } catch(e){}
    // push to overlay
    pushDbgLine('LOG', text);
    // also try Android bridge quick toast for short messages
    try { if (typeof text === 'string' && text.length < 120) dbgToast(text); } catch(e){}
  } catch(e){}
}
...
function dbgAlert(label, payload) {
  try {
    ...
    const text = (typeof payload === 'string') ? payload : (() => {
      try { return JSON.stringify(payload, (k, v) => { if (Array.isArray(v) && v.length > 50) return `[Array(${v.length}) truncated]`; return v; }, 2); } catch(e) { return String(payload); }
    })();
    const full = `${label} - ${text}`;
    // console for desktop
    try { console.info('[DBG ALERT]', label, payload); } catch(e){}
    // push alert line (high visibility)
    pushDbgLine('ALERT', full);
    // try Android bridge with priority (various bridge method names)
    try {
      if (window.Android && typeof window.Android.showLongToast === 'function') { window.Android.showLongToast(full); return; }
      if (window.Android && typeof window.Android.showToast === 'function') { window.Android.showToast(full); }
      if (window.android && typeof window.android.showToast === 'function') { window.android.showToast(full); }
    } catch(e){ /* ignore bridge errors */ }
    // also show ephemeral in-page toast
    dbgToast(full);
  } catch(e){}
}
...
try {
  window.showDbgPanel = showDbgPanel;
  window.hideDbgPanel = hideDbgPanel;
  window.clearDbgPanel = clearDbgPanel;
  window.dbgLog = dbgLog;
  window.dbgAlert = dbgAlert;
} catch(e){ /* ignore */ }
...
function markAppliedMove(rec) {
  // rec peut contenir id, move_index, position
  if (!rec) return;
  if (rec.id) appliedMoveIds.add(String(rec.id));
  else if (Number.isFinite(Number(rec.position))) appliedMoveIds.add(`pos:${Number(rec.position)}`);
}
...
for (const m of moves) {
        ...
        if (!board[pos]) {
          ...
          // case déjà avait même player (mais marquer applied pour éviter répétitions futures)
...
function applyRealtimeMove(rec) {
  ...
  appliedMoveIds.clear();
    ...
    if (!board[pos]) {
      ...
      if (currentTurnEl) currentTurnEl.textContent = current;
      // refresh minimal metadata if needed
      refreshGameState().catch(e => dbgLog('refreshGameState(from realtime) failed', e));
    } else {
      // If board already has same player, still mark applied to prevent duplicates later
      if (board[pos] === player) {
        if (mid) appliedMoveIds.add(mid);
  ...
  try {
    ...
    if (gRow?.board) {
      ...
      if (!Array.isArray(board) && typeof board === 'object') {
        ...
        board = newBoard;
        // Reconstruire appliedMoveIds à partir du board pour éviter que les realtime/poll
// réappliquent les mêmes positions (marque via token 'pos:N')
appliedMoveIds.clear();
for (let p = 0; p < board.length; p++) {
  if (board[p]) appliedMoveIds.add(`pos:${p}`);
}
      }
    } else {
      ...
      try {
        ...
        if (!movesErr && Array.isArray(moves) && moves.length) {
          ...
          board = newBoard;
          // rebuild appliedMoveIds from canonical board (avoid duplicates from realtime/poll)
appliedMoveIds.clear();
for (let p = 0; p < board.length; p++) {
  if (board[p]) appliedMoveIds.add(`pos:${p}`);
}
...
async function placeMove(r, c, playerSymbol, options = {}) {
  ...
  try {
    ...
    // play click sound (visual placement will come from server/realtime)
    ...
    // clear chosen sounds for next move
    ...
    if (supabase && isRemotePlaying && gameId) {
      try {
        ...
        if (rpcErr) {
          // Server rejected the move — re-sync canonical state from server
        ...
        // If server indicates game finished -> refresh canonical state and animate winner
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          ...
          try {
            // try to animate a winning line for the winner if possible
            // map winner id -> symbol (X/O) when possible
            let winnerSymbol = null;
            if (rpcRow.winner && (rpcRow.winner === hostId)) winnerSymbol = 'X';
            else if (rpcRow.winner && (rpcRow.winner === opponentId)) winnerSymbol = 'O';
            // fallback: if playerSymbol was the one who just played, prefer it
            if (!winnerSymbol && rpcRow.winner == null && playerSymbol) winnerSymbol = playerSymbol;

            if (winnerSymbol) {
              // scan board to find a position that yields a winning sequence
          ...
          // let server-driven logic handle manches/series; trigger post-round handler
            ...
            try {
              ...
              if (!Array.isArray(newBoard) && typeof newBoard === 'object') {
                ...
                Object.keys(newBoard).forEach(k => {
                  const pos = Number(k);
                  const v = newBoard[k];
                  if (Number.isFinite(pos) && pos >= 0 && pos < ROWS * COLS) nb[pos] = v;
                });
                newBoard = nb;
              }

              // ensure board has correct shape
...
for (let p = 0; p < board.length; p++) {
  if (board[p]) appliedMoveIds.add(`pos:${p}`);
            ...
            // fallback: fetch last move and apply via canonical route
            try {
        ...
        // RPC exception — re-sync canonical state and inform user
      ...
      // offline/local mode: do local winner check (we're allowed to modify board here)
      // apply move locally (offline mode only) and perform checks/animation
