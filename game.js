// game.js (version corrigée — fix affichage double des pions)
// Objectif : empêcher que l'apparition d'un pion dans la case choisie crée simultanément
// un autre pion ailleurs (ex: "ordre 0..399"). Solution : utiliser un mapping fiable
// index -> enfant DOM (boardGrid.children[pos]) au lieu de rechercher avec querySelector
// et s'assurer que chaque cellule est complètement nettoyée avant insertion d'une image.
//
// Remarques : ce fichier reprend la logique précédente, avec corrections ciblées sur
// createGridStructureIfNeeded, updateCellAt, et quelques protections supplémentaires
// pour garder le DOM synchronisé avec l'état canonique "board".
...
// Cell DOM cache (pos -> element)
let cellEls = new Array(ROWS * COLS).fill(null);
let current = 'X';
let manches = { X: 0, O: 0 };
let series = { X: 0, O: 0 };
// --- déjà existant en haut ---
let moveIndex = 0;
let isHost = true, isRemotePlaying = false, pendingMove = false;

// valid symbols guard
const VALID_SYMBOLS = new Set(['X', 'O']);
function isValidSymbol(v) { return v === 'X' || v === 'O'; }

// Track applied move ids to ensure idempotence
const appliedMoveIds = new Set();

// Track which board positions are currently occupied (fast incremental updates)
const occupiedPositions = new Set();        // positions currently occupied (numbers)
let lastRenderedOccupied = new Set();      // snapshot of what was rendered last time

function markAppliedMove(rec) {
  if (!rec) return;
  const pos = Number(rec.position);
  if (rec.id) appliedMoveIds.add(String(rec.id));
  if (Number.isFinite(pos)) appliedMoveIds.add(`pos:${pos}`);
}
...
// Update a single cell DOM from the canonical board[pos]
// Important: use boardGrid.children[pos] when possible to ensure strict positional mapping
function updateCellAt(pos) {
  if (!Number.isFinite(pos) || pos < 0 || pos >= ROWS * COLS) return;

  // Prefer direct child access (stable mapping) to avoid querySelector mismatches
  let cell = null;
  try {
    if (boardGrid && boardGrid.children && boardGrid.children.length === ROWS * COLS) {
      cell = boardGrid.children[pos];
    }
  } catch (e) {
    dbgLog('updateCellAt: children access failed', e);
    cell = null;
  }

  // Fallback to cached cellEls if available and matches dataset.i
  if (!cell && cellEls && Number.isFinite(pos)) {
    const cached = cellEls[pos];
    if (cached && String(cached.dataset?.i) === String(pos)) cell = cached;
  }

  // Last fallback to querySelector (shouldn't be needed, but keep it safe)
  if (!cell && boardGrid) {
    cell = boardGrid.querySelector(`[data-i="${pos}"]`);
  }
  if (!cell) return;

  // Normalise occupant: only render a piece if it's strictly 'X' or 'O'
  const boardOccupant = (isValidSymbol(board[pos])) ? board[pos] : null;

  // Avoid needless DOM churn: check current occupant dataset
  const currentOccupant = (cell.dataset && typeof cell.dataset.occupant !== 'undefined') ? cell.dataset.occupant : null;
  if (currentOccupant === boardOccupant) {
    // nothing to change
    return;
  }

  // Clear cell fully to avoid any stray elements (this prevents duplicates)
  // Important: remove all children and attributes related to occupant
  while (cell.firstChild) cell.removeChild(cell.firstChild);
  cell.classList.remove('piece-x', 'piece-o', 'winning');
  delete cell.dataset.occupant;

  if (boardOccupant) {
    cell.classList.add(`piece-${String(boardOccupant).toLowerCase()}`);
    // set occupant dataset for quick check
    cell.dataset.occupant = String(boardOccupant);
    const img = document.createElement('img');
    img.className = 'piece-img';
    img.src = pieceSrc(boardOccupant);
    img.alt = boardOccupant;
    img.loading = 'lazy';
    img.draggable = false;
...
function renderMiniMap() {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      const v = isValidSymbol(board[idx(r, c)]) ? board[idx(r, c)] : null;
      if (!v) continue;
...
function animateWinningPositions(positions) {
  ...
  positions.forEach(i => {
    const el = boardGrid && boardGrid.children && boardGrid.children[i] ? boardGrid.children[i] : (boardGrid && boardGrid.querySelector(`[data-i="${i}"]`));
  ...
  const doPoll = async () => {
    ...
    try {
      ...
      const { data: moves, error } = await supabase
        .from(MOVES_TABLE)
        .select('id,position,player,move_index')
        .eq('game_id', gameId)
        .order('move_index', { ascending: true });

      if (error) {
        dbgLog('polling moves error', error);
        return;
      }
      ...
      for (const m of moves) {
        ...
        const mid = m.id ? String(m.id) : (Number.isFinite(pos) ? `pos:${pos}` : null);

        if (!Number.isFinite(pos) || !isValidSymbol(player)) continue;

        if (mid && appliedMoveIds.has(mid)) {
          if (Number.isFinite(mi)) maxIdx = Math.max(maxIdx, mi);
          continue;
        }

        // ne jamais écraser une case occupée
        if (!board[pos]) {
          board[pos] = player;
          ...
          changed = true;
          lastAppliedMove = m;
          dbgLog('poll applied move', { id: mid, pos, player, mi });
        } else {
          // if the board already has same player, still mark applied to avoid reapplying later
          if (board[pos] === player && mid) appliedMoveIds.add(mid);
          else dbgLog('poll conflict ignored', {
            pos,
            local: board[pos],
            server: player
          });
        ...
        if (Number.isFinite(mi)) {
          maxIdx = Math.max(maxIdx, mi);
        }
      }

      if (changed) {
        // only update changed positions: prefer to center on last applied
        renderGrid(); // renderGrid now updates only diffs
        const lastMove = lastAppliedMove || (moves.length ? moves[moves.length - 1] : null);
        if (lastMove && lastMove.player) {
          try {
            ...
            dbgLog('poll centering failed', e);
          }

          current = (lastMove.player === 'X') ? 'O' : 'X';
...
function applyRealtimeMove(rec) {
  try {
    ...
    const mid = rec.id ? String(rec.id) : (Number.isFinite(pos) ? `pos:${pos}` : null);

    if (!Number.isFinite(pos) || pos < 0 || pos >= ROWS * COLS) {
      dbgLog('applyRealtimeMove: invalid pos', rec);
      return;
    }
    if (!isValidSymbol(player)) { dbgLog('applyRealtimeMove: invalid player symbol', rec); return; }

    if (mid && appliedMoveIds.has(mid)) {
      dbgLog('applyRealtimeMove: skip already applied move', { id: mid, pos, player, moveIndex });
      if (Number.isFinite(mi)) moveIndex = Math.max(moveIndex, mi);
      return;
    }

    if (Number.isFinite(mi) && mi <= moveIndex) {
      dbgLog('applyRealtimeMove: skipping old/duplicate move by move_index', { rec, moveIndex });
      if (mid) appliedMoveIds.add(mid);
      return;
    }

    if (!Array.isArray(board) || board.length !== ROWS * COLS) {
      board = new Array(ROWS * COLS).fill(null);
      appliedMoveIds.clear();
      occupiedPositions.clear();
    }

    // If the target cell is empty, apply it. If not, mark applied if same player.
    if (!board[pos]) {
      board[pos] = player;
      occupiedPositions.add(pos);
      markAppliedMove(rec);
      dbgLog('applyRealtimeMove: applied', { id: mid, pos, player, mi });
      ...
      if (board[pos] === player) {
        if (mid) appliedMoveIds.add(mid);
        dbgLog('applyRealtimeMove: board already had same player - marking applied', { id: mid, pos, player });
      } else {
        dbgLog('applyRealtimeMove conflict ignored', { pos, local: board[pos], server: player });
      }
    }
...
// --- Create grid DOM structure once (now ensures strict children ordering) ---
function createGridStructureIfNeeded() {
  if (!boardGrid) return;

  // Quick check: if children count matches and dataset.i matches index, assume OK
  if (boardGrid.childElementCount === ROWS * COLS && cellEls.every((el, i) => {
    if (!el) return false;
    return String(el.dataset?.i) === String(i) && boardGrid.children[i] === el;
  })) {
    return;
  }

  // Otherwise recreate grid structure deterministically (children order = index)
  boardGrid.innerHTML = '';
  cellEls = new Array(ROWS * COLS).fill(null);
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      const i = idx(r, c);
      ...
      cell.dataset.c = c;
      cell.dataset.i = i;
      // attach single click handler per cell
      cell.addEventListener('click', () => onCellClick(r, c));
      cell.innerHTML = '';
      boardGrid.appendChild(cell);
      // cache direct mapping pos -> element
      cellEls[i] = cell;
    }
  }

  // Sanity: ensure children length equals expected
  if (boardGrid.childElementCount !== ROWS * COLS) {
    dbgLog('createGridStructureIfNeeded: unexpected child count after create', boardGrid.childElementCount);
  }
}

// --- New incremental renderGrid: update only added/removed positions ---
function renderGrid() {
  if (!boardGrid) return;

  // ensure structure exists
  createGridStructureIfNeeded();

  // Save scroll to restore
  const prevScroll = (boardViewport) ? { left: boardViewport.scrollLeft, top: boardViewport.scrollTop } : { left: 0, top: 0 };

  // Compute sets of added / removed positions
  const added = [];
  const removed = [];

  // added: positions present in occupiedPositions but not in lastRenderedOccupied
  for (const p of occupiedPositions) {
    if (!lastRenderedOccupied.has(p)) added.push(p);
  }
  // removed: positions that were rendered but are no longer occupied
  for (const p of lastRenderedOccupied) {
    if (!occupiedPositions.has(p)) removed.push(p);
  }

  // First clear removed cells
  for (const pos of removed) {
    try {
      // board[pos] should be null here, but updateCellAt handles both cases
      updateCellAt(pos);
    } catch (e) {
      dbgLog('renderGrid: clearing removed pos failed', pos, e);
    }
  }

  // Then render added cells
  for (const pos of added) {
    try {
      updateCellAt(pos);
    } catch (e) {
      dbgLog('renderGrid: updating added pos failed', pos, e);
    }
  }

  // finally, update snapshot
  lastRenderedOccupied = new Set(occupiedPositions);

  // restore scroll and update minimap
  if (boardViewport) {
...
// --- DB reads / refresh canonical state (modified to update occupiedPositions and render diffs) ---
async function refreshGameState() {
  ...
  try {
    ...
    if (gRow?.board) {
      ...
      if (!Array.isArray(board) && typeof board === 'object') {
        ...
        Object.keys(board).forEach(k => {
          const pos = Number(k);
          const v = board[k];
          if (Number.isFinite(pos) && isValidSymbol(v)) newBoard[pos] = v;
        });
        board = newBoard;
      } else if (Array.isArray(board)) {
        // sanitize array board: only keep valid symbols, convert others to null
        const nb = new Array(ROWS * COLS).fill(null);
        for (let i = 0; i < Math.min(board.length, ROWS * COLS); i++) {
          nb[i] = isValidSymbol(board[i]) ? board[i] : null;
        }
        board = nb;
      }

      // rebuild occupiedPositions from canonical board (fast)
      occupiedPositions.clear();
      for (let p = 0; p < board.length; p++) {
        if (isValidSymbol(board[p])) occupiedPositions.add(p);
      }
      // rebuild appliedMoveIds tokens based on board to avoid re-applying same positions
      appliedMoveIds.clear();
      for (let p = 0; p < board.length; p++) {
        if (isValidSymbol(board[p])) appliedMoveIds.add(`pos:${p}`);
      }
    } else {
      ...
      try {
        ...
        if (!movesErr && Array.isArray(moves) && moves.length) {
          ...
          for (const m of moves) {
            const pos = Number(m.position);
            if (Number.isFinite(pos) && isValidSymbol(m.player)) newBoard[pos] = m.player;
            const mi = Number(m.move_index ?? NaN);
            if (Number.isFinite(mi)) maxIdx = Math.max(maxIdx, mi);
          }
          board = newBoard;
          // rebuild occupiedPositions and appliedMoveIds
          occupiedPositions.clear();
          appliedMoveIds.clear();
          for (let p = 0; p < board.length; p++) {
            if (isValidSymbol(board[p])) {
              occupiedPositions.add(p);
              appliedMoveIds.add(`pos:${p}`);
            }
          }
          moveIndex = maxIdx;
        } else {
    ...
    // render only diffs (renderGrid handles incremental updates)
    renderGrid();

    // if finished -> try to animate a winning line
...
async function placeMove(r, c, playerSymbol, options = {}) {
  ...
  try {
    ...
    if (!Number.isFinite(i) || i < 0 || i >= ROWS * COLS) {
      dbgLog('placeMove: invalid index', { r, c, i });
      return false;
    }

    if (!isValidSymbol(playerSymbol)) {
      dbgLog('placeMove: invalid symbol attempted', playerSymbol);
      return false;
    }
    ...
    const selPunch = document.getElementById('selPunchline');
    const selClickE = document.getElementById('selClick');
    if (selPunch) selPunch.value = '';
    if (selClickE) selClickE.value = '';
    ...
    if (supabase && isRemotePlaying && gameId) {
      try {
        ...
        if (rpcErr) {
          dbgLog('PLACE_MOVE rejected by server', rpcErr);
          try { await refreshGameState(); } catch (e) { dbgLog('refresh after rpcErr failed', e); }
        ...
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          try {
            ...
            dbgLog('refreshGameState after rpc finish failed', e);
          }

          try {
            let winnerSymbol = null;
            if (rpcRow.winner && (rpcRow.winner === hostId)) winnerSymbol = 'X';
            else if (rpcRow.winner && (rpcRow.winner === opponentId)) winnerSymbol = 'O';
            if (!winnerSymbol && rpcRow.winner == null && playerSymbol) winnerSymbol = playerSymbol;

            if (winnerSymbol) {
              outer: for (let posIdx = 0; posIdx < board.length; posIdx++) {
                if (board[posIdx] === winnerSymbol) {
                  const seq = findWinningLine(posIdx, winnerSymbol);
                  if (seq && seq.length >= WIN) { animateWinningPositions(seq); break outer; }
                }
              }
            }
          } catch (e) {
            dbgLog('post-finish animation failed', e);
          }

          try { await handleRoundEnd(rpcRow.winner ?? 'draw'); } catch (e) { dbgLog('handleRoundEnd failed', e); }

        } else {
          if (rpcRow && rpcRow.board) {
            try {
              let newBoard = null;
              if (typeof rpcRow.board === 'string') newBoard = JSON.parse(rpcRow.board);
              else newBoard = rpcRow.board;
              ...
              if (!Array.isArray(newBoard) && typeof newBoard === 'object') {
                ...
                Object.keys(newBoard).forEach(k => {
                  const pos = Number(k);
                  const v = newBoard[k];
                  if (Number.isFinite(pos) && pos >= 0 && pos < ROWS * COLS && isValidSymbol(v)) nb[pos] = v;
                });
                newBoard = nb;
              }

              if (!Array.isArray(newBoard) || newBoard.length !== ROWS * COLS) {
                const nb = new Array(ROWS * COLS).fill(null);
                if (Array.isArray(newBoard)) {
                  for (let j = 0; j < Math.min(newBoard.length, ROWS * COLS); j++) {
                    nb[j] = isValidSymbol(newBoard[j]) ? newBoard[j] : null;
                  }
                }
                newBoard = nb;
              } else {
                for (let j = 0; j < newBoard.length; j++) {
                  newBoard[j] = isValidSymbol(newBoard[j]) ? newBoard[j] : null;
                }
              }

              // Merge: replace canonical board and rebuild occupiedPositions
              board = newBoard;
              occupiedPositions.clear();
              appliedMoveIds.clear();
              for (let p = 0; p < board.length; p++) {
        ...
        try { await refreshGameState(); } catch (err) { dbgLog('refresh after rpc exception failed', err); }
      ...
      if (currentTurnEl) currentTurnEl.textContent = current;
      // update only the changed cell (no full grid sweep)
      updateCellAt(i);
      renderMiniMap();
    ...
    if (Array.isArray(moves)) {
      for (const m of moves) {
        try {
          ...
          if (m.id) appliedMoveIds.add(String(m.id));
          else appliedMoveIds.add(`pos:${pos}`);
...
function resetLocalGame(stopAll = true) {
  ...
  // 1) réinitialise l'état canonique
  board = new Array(ROWS * COLS).fill(null);
  appliedMoveIds.clear();
  occupiedPositions.clear();
  lastRenderedOccupied.clear();
  current = 'X';
  localPlayerSymbol = 'X';
  manches = { X: 0, O: 0 };
  series = { X: 0, O: 0 };
  ...
  // 2) assure que la structure DOM existe (créée une seule fois)
  if (typeof createGridStructureIfNeeded === 'function') {
    createGridStructureIfNeeded();
  }

  // 3) vide l'affichage des pions.
  if (boardGrid) {
    // We rely on children order to clear cells reliably
    for (let pos = 0; pos < ROWS * COLS; pos++) {
      const cell = boardGrid.children && boardGrid.children[pos] ? boardGrid.children[pos] : boardGrid.querySelector(`[data-i="${pos}"]`);
      if (cell) {
        while (cell.firstChild) cell.removeChild(cell.firstChild);
        delete cell.dataset.occupant;
        cell.classList.remove('piece-x', 'piece-o', 'winning');
    ...
    // refresh cached cellEls mapping
    for (let i = 0; i < ROWS * COLS; i++) {
      cellEls[i] = boardGrid.children[i] || null;
    }
  } else {
    if (typeof renderGrid === 'function') renderGrid();
  }

  // 4) met à jour l'UI (tour, scores, etc.)
  if (currentTurnEl) currentTurnEl.textContent = current;
  if (scoreX) scoreX.textContent = '0';
  if (scoreO) scoreO.textContent = '0';
  if (seriesX) seriesX.textContent = '0';
  if (seriesO) seriesO.textContent = '0';

  // 5) rafraîchir mini map si nécessaire
