...
if (!tryMarkModalShown('newRoundShownForGameId', g.id) && !amLocalWinner) {
  ...
  try {
    // --- show series modal (respecter priorité pour le gagnant local) ---
    const intendedSeriesId = state.series_id ? String(state.series_id) : (g && g.series_id ? String(g.series_id) : '');
    // detect local series winner (if any)
    const amLocalSeriesWinner = state.userId && state.series_winner && (String(state.userId) === String(state.series_winner));

    // If local user is series winner, ensure they see the modal (do not let earlier pre-marking suppress it).
    if (amLocalSeriesWinner) {
      dbgLog('local series winner detected — show series modal and avoid skipping due to prior mark', intendedSeriesId);
      // ensure runtime marker so other clients won't duplicate the modal afterwards
      ...
      try {
        ...
        dbgLog('series modal show failed (local-winner branch)', e);
      }
    } else {
      // existing behaviour for non-winners: respect wasModalDismissed / tryMarkModalShown to avoid duplicates
...
// prefer showing modal to local series winner even if previously marked
const amLocalSeriesWinner = state.userId && state.series_winner && (String(state.userId) === String(state.series_winner));
if (amLocalSeriesWinner) {
  dbgLog('local series winner detected in poll -> show series modal (skip skip-mark path)', intendedSeriesId);
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
  showSeriesModal({
    series_id: state.series_id,
    series_owner_wins: state.series_owner_wins,
  ...
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    dbgLog('series already marked, skipping show (finalize)', intendedSeriesId);
  } else {
    showSeriesModal({
      series_id: state.series_id,
      series_owner_wins: state.series_owner_wins,
...
btns.appendChild(closeBtn);     
  ...
  // Do NOT remove the overlay immediately: wait for the create RPC to succeed/fail
  try {
    await createNewSeriesFromSeries(info.series_id, info.last_round_winner ?? state.lastRoundWinnerId);
    // success: remove the modal overlay now
    if (overlay && typeof overlay.remove === 'function') overlay.remove();
  } catch (e) {
    dbgLog('createNewSeriesFromSeries failed', e);
    // keep UX consistent: remove modal and resync so the client is not stuck
    if (overlay && typeof overlay.remove === 'function') overlay.remove();
    await pollGameOnce();
  }
};
...
const amLocalSeriesWinner = state.userId && state.series_winner && (String(state.userId) === String(state.series_winner));
if (amLocalSeriesWinner) {
  dbgLog('local series winner detected in series subscription — show modal', intendedSeriesId);
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
  showSeriesModal({
    series_id: state.series_id,
    series_owner_wins: state.series_owner_wins,
