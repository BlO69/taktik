// game.js (corrigé + affichage avatars/pseudos + RPC fallback + finalize round handling)
...
export default async function initGame(supabaseClient) {
  ...
  async function handleFinalizeRoundAndFollowup(p_game_id) {
    try {
  ...
  async function joinNewGame(newGameId) {
    try {
      ...
      // small notification
      setTimeout(()=>alert('Nouvelle manche démarrée.'), 150);
    } catch (e) {
  ...
  window.taktikGame.finalizeRound = async (gId) => {
    return handleFinalizeRoundAndFollowup(gId || gameId);
  };
