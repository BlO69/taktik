...
if (finalizeRow.series_winner) {
 ...
 async function showNewRoundModal(info) {
  try {
    // wait for initial sync — but if queued with new_game_id, mark to avoid duplicate later
    if (!state.initialSyncDone) {
      dbgLog('suppress new-round modal until initial sync');
      if (info && info.new_game_id) tryMarkModalShown('newRoundShownForGameId', info.new_game_id);
      return;
    }
