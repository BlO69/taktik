...
function joinMatchRPC(matchId) {
  ...
  function updatePlayerUI() {
    ...
    updateTurnUI();
        ...
        // Preserve existing displayed value when state value is temporarily missing
     setText('scoreX', (state.party_owner_wins != null) ? state.party_owner_wins : (document.getElementById('scoreX')?.textContent || '—'));
     setText('scoreO', (state.party_opponent_wins != null) ? state.party_opponent_wins : (document.getElementById('scoreO')?.textContent || '—'));
     setText('seriesX', (state.series_owner_wins != null) ? state.series_owner_wins : (document.getElementById('seriesX')?.textContent || '—'));
     setText('seriesO', (state.series_opponent_wins != null) ? state.series_opponent_wins : (document.getElementById('seriesO')?.textContent || '—'));
  ...
  dbgLog('enqueue new-round modal failed', e);
    ...
    if (newGameId) {
      if (!state.initialSyncDone) {
       ...
       tryMarkModalShown('newRoundShownForGameId', newGameId);     
      }
    }
...
try {
  if (finalizeRow && finalizeRow.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
    // pass authoritative info from finalizeRow to the modal
    showSeriesModal({
      series_id: finalizeRow.series_id ?? state.series_id,
      series_owner_wins: Number(finalizeRow.series_owner_wins ?? state.series_owner_wins ?? 0),
      series_opponent_wins: Number(finalizeRow.series_opponent_wins ?? state.series_opponent_wins ?? 0),
...
} catch (e) { dbgLog('post-join forced showSeriesModal failed', e); }
