...
function applyRealtimeMove(rec) {
  try {
    ...
    // if server sent whole board, prefer it
    if (rec.board) {
      try {
        const nboard = normalizeBoard(rec.board);
        board = nboard;
    ...
    if (serverWinnerUid || serverGameFinished) {
      ...
      // If winnerSymbol is still uuid-like, try to infer from board (last move)
      if (winnerSymbol) {
        handleRoundEnd(winnerSymbol).catch(e => dbgLog('handleRoundEnd (realtime) failed', e));
      } else {
...
async function refreshGameState() {
  ...
  try {
    ...
    // board normalization: prefer gRow.board
    if (gRow?.board) {
      try {
    ...
    if (supabase && isRemotePlaying && gameId) {
      try {
        ...
        if (rpcRow && (rpcRow.board !== undefined && rpcRow.board !== null)) {
          ...
          // No canonical board — fallback: fetch last move and apply
          try {
