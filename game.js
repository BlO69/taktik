...
function applyRealtimeMove(rec) {
  try {
    ...
    renderGrid(); // renderMiniMap called from renderGrid
    // --- center on the received move (makes UI show the correct area) ---
    try {
      let rr = null, cc = null;
      ...
      if (Number.isFinite(rr) && Number.isFinite(cc)) centerViewportOn(rr, cc);
    } catch (e) { dbgLog('applyRealtimeMove centering failed', e); }
        ...
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          ...
          if (rpcRow && rpcRow.board) {
            try {
              ...
              renderGrid(); renderMiniMap();
              // center on server canonical last move if RPC returned it
              try {
                const rpcPos = (rpcRow && rpcRow.position != null) ? Number(rpcRow.position) : null;
                if (Number.isFinite(rpcPos)) centerViewportOn(Math.floor(rpcPos / COLS), rpcPos % COLS);
                else centerViewportOn(r, c); // fallback to the local clicked cell
              } catch(e) { dbgLog('post-RPC centering failed', e); }
            } catch (e) {
...
async function joinGame(gId) {
  ...
  try {
    ...
    // center viewport on last move if any
    try {
      const lastApplied = Array.isArray(moves) && moves.length ? moves[moves.length - 1] : null;
      if (lastApplied && lastApplied.position != null) {
        const pos = Number(lastApplied.position);
        if (Number.isFinite(pos)) centerViewportOn(Math.floor(pos / COLS), pos % COLS);
      }
    } catch(e) { dbgLog('joinGame centering failed', e); }
