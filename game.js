...
try {
  ...
  dbgLog('ensure scores before modal failed', e);
    ...
    // allow the local round-winner to still see the "new round" modal even if a mark exists
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  if (!amLocalWinner) {
    ...
    // ensure marker present (idempotent) but still show the modal to the local winner
    tryMarkModalShown('newRoundShownForGameId', g.id);
    showNewRoundModal({
      party_id: state.party_id,
      party_owner_wins: state.party_owner_wins,
      party_opponent_wins: state.party_opponent_wins,
 ...
 async function showNewRoundModal(info) {
  try {
    ...
    if (info && info.new_game_id) {
      const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
      if (!amLocalWinner) {
        // non-winner clients must not re-show if already marked
        ...
        // for the local winner: ensure the mark exists but still allow the modal to be shown
