...
if (!amUserSeriesWinner) {
  ...
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
    ...
    // --- Détection du rôle de l'utilisateur pour la série ---
    const amOwnerSM = !!(state.userId && state.ownerId && String(state.userId) === String(state.ownerId));
    const amOpponentSM = !!(state.userId && state.opponentId && String(state.userId) === String(state.opponentId));
    const amPlayerSM = amOwnerSM || amOpponentSM;
    const amUserSeriesLoser = amPlayerSM && info.series_winner && state.userId && String(state.userId) !== String(info.series_winner);

    // --- Perdant de la série : modale de défaite dédiée + defeat.wav (une seule fois) ---
    if (amUserSeriesLoser) {
      try { if (typeof playSound === 'function') playSound(DEFEAT_SOUND); } catch(e) { dbgLog('defeat sound for series loser failed', e); }

      const prevLoser = document.getElementById('taktik-series-modal');
      if (prevLoser) prevLoser.remove();

      const loserOverlay = document.createElement('div');
      loserOverlay.id = 'taktik-series-modal';
      loserOverlay.style.position = 'fixed';
      loserOverlay.style.inset = '0';
      loserOverlay.style.background = 'rgba(0,0,0,0.65)';
      loserOverlay.style.display = 'flex';
      loserOverlay.style.alignItems = 'center';
      loserOverlay.style.justifyContent = 'center';
      loserOverlay.style.zIndex = '10001';
      loserOverlay.style.padding = '16px';

      const loserBox = document.createElement('div');
      loserBox.style.width = 'min(520px, 96%)';
      loserBox.style.maxWidth = '520px';
      loserBox.style.background = '#fff';
      loserBox.style.borderRadius = '12px';
      loserBox.style.boxShadow = '0 20px 40px rgba(0,0,0,0.35)';
      loserBox.style.padding = '20px';
      loserBox.style.textAlign = 'center';
      loserOverlay.appendChild(loserBox);

      const loserTitle = document.createElement('h3');
      loserTitle.style.margin = '0 0 8px 0';
      loserTitle.style.fontSize = '20px';
      loserTitle.textContent = 'Série terminée';
      loserBox.appendChild(loserTitle);

      const winnerDisplayName = info.series_winner_pseudo ||
        (info.series_winner && String(info.series_winner) === String(state.ownerId) ? state.ownerProfile?.pseudo : null) ||
        (info.series_winner && String(info.series_winner) === String(state.opponentId) ? state.opponentProfile?.pseudo : null) ||
        (info.series_winner ? String(info.series_winner).slice(0, 8) : 'votre adversaire');

      const loserMsg = document.createElement('div');
      loserMsg.style.margin = '6px 0 12px';
      loserMsg.style.fontSize = '15px';
      loserMsg.innerHTML = `Vous avez perdu la série.<br><span style="color:#888">La série est remportée par <strong>${winnerDisplayName}</strong>.</span>`;
      loserBox.appendChild(loserMsg);

      const loserDetails = document.createElement('div');
      loserDetails.style.fontSize = '14px';
      loserDetails.style.color = '#333';
      loserDetails.style.marginBottom = '14px';
      loserDetails.innerHTML = `
        <div>Score de la série — Joueur 1 : <strong>${info.series_owner_wins ?? '0'}</strong> — Joueur 2 : <strong>${info.series_opponent_wins ?? '0'}</strong></div>
        <div style="margin-top:6px">Manches — Joueur 1 : <strong>${info.party_owner_wins ?? '0'}</strong> — Joueur 2 : <strong>${info.party_opponent_wins ?? '0'}</strong></div>
      `;
      loserBox.appendChild(loserDetails);

      const loserBtns = document.createElement('div');
      loserBtns.style.display = 'flex';
      loserBtns.style.justifyContent = 'center';
      const loserCloseBtn = document.createElement('button');
      loserCloseBtn.textContent = 'Fermer';
      loserCloseBtn.style.padding = '8px 16px';
      loserCloseBtn.style.borderRadius = '6px';
      loserCloseBtn.style.border = '1px solid #ccc';
      loserCloseBtn.style.cursor = 'pointer';
      loserCloseBtn.onclick = () => { markDismissed(); loserOverlay.remove(); };
      loserBtns.appendChild(loserCloseBtn);
      loserBox.appendChild(loserBtns);
      document.body.appendChild(loserOverlay);
      return; // ne pas afficher la modale gagnant au perdant de la série
    }
...
// retirer une éventuelle modale précédente (nettoyage défensif)
const prevModal = document.getElementById('taktik-outcome-modal');
if (prevModal) prevModal.remove();

    // --- résolution des rôles ---
    const amOwnerOM = !!(state.userId && state.ownerId && String(state.userId) === String(state.ownerId));
    const amOpponentOM = !!(state.userId && state.opponentId && String(state.userId) === String(state.opponentId));
    const amPlayerOM = amOwnerOM || amOpponentOM;
    const actualWinnerIdOM = winnerId ?? state.winnerId ?? null;
    const amLocalWinnerOM = amPlayerOM && actualWinnerIdOM && state.userId && String(state.userId) === String(actualWinnerIdOM);

    // --- Gagnant de la manche : victory.wav immédiat + auto-finalise sans modale ---
    // La modale "nouvelle manche" (non-clôture série) ou "série terminée" (clôture série)
    // apparaîtra automatiquement après la finalisation.
    if (amLocalWinnerOM) {
      try { if (typeof playSound === 'function') playSound(VICTORY_SOUND); } catch(e) { dbgLog('victory sound failed', e); }
      try {
        handleFinalizeRoundAndFollowup(String(targetGameId), { starterChar: null })
          .catch(e => {
            dbgLog('auto-finalize for winner failed', e);
            try { pollGameOnce().catch(()=>{}); } catch(_) {}
          });
      } catch(e) { dbgLog('auto-finalize call exception', e); }
      return; // pas de modale outcome pour le gagnant
    }

    // --- Perdant ou spectateur : pas de modale de résultat de manche ---
    // Le perdant de la SÉRIE recevra la modale de défaite dédiée via showSeriesModal.
    return;
  } catch (e) {
