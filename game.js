...
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerGlobal) {
  ...
  try {
    ...
    try {
      // === PATCH: Guard auto-join if local user is the series winner ===
      ...
      // --- determine if local user is the winner of the SERIES (best-effort) ---
      let seriesWinnerId = state.series_winner || null;
      if (!seriesWinnerId && g && g.series_id) {
        try {
          const { data: srow, error: serr } = await supabase
            ...
            .eq('id', g.series_id)
            .maybeSingle();
          if (!serr && srow) seriesWinnerId = srow.winner_id ?? null;
        } catch (e) {
          dbgLog('fetch series winner fallback failed', e);
        }
      }

      const amSeriesWinner = state.userId && seriesWinnerId && (String(state.userId) === String(seriesWinnerId));
      if (amSeriesWinner) {
        dbgLog('Local user is series winner — skipping auto-join so they can interact with series modal', g.id);

        // pre-mark the series modal shown key for concurrency and ensure we still show it to the winner
        const intendedSeriesId = g.series_id ? String(g.series_id) : (state.series_id ? String(state.series_id) : '');
        tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);

        // refresh scores/state then show the series modal immediately (best-effort)
        try {
          await fetchAndApplyPartyAndSeries(g.party_id ?? state.party_id, g.series_id ?? state.series_id);
        } catch (e) { dbgLog('fetchAndApplyPartyAndSeries before showSeriesModal failed', e); }

        try {
          showSeriesModal({
            series_id: intendedSeriesId,
            series_owner_wins: state.series_owner_wins,
            series_opponent_wins: state.series_opponent_wins,
            series_winner: seriesWinnerId ?? state.series_winner,
            party_owner_wins: state.party_owner_wins,
            party_opponent_wins: state.party_opponent_wins,
            new_game_id: g.id,
            last_round_winner: state.lastRoundWinnerId
          ...
          dbgLog('showSeriesModal for local series winner failed', e);
        }
      } else {
        // not the series winner -> behave as before (join so losers/spectators get the cleared board)
      ...
      dbgLog('series-winner guard in handleGlobalNewGameCreated failed', e);
      // safe fallback: try to join (preserve existing behaviour)
      try { await joinNewGame(g.id, starterChar); } catch(e2){ dbgLog('fallback join failed', e2); }
    }
...
// ne PAS afficher la modale "nouvelle manche" : la modal de série (showSeriesModal) gère le flow.
if (info.series_winner && state.userId && String(info.series_winner) === String(state.userId)) {
  dbgLog('suppress new-round modal for local series winner (series modal will be shown instead)', info.series_winner);
  return;
}
...
if (!amUserSeriesWinner) {
  ...
  // pour le gagnant de la série, s'assurer que le marqueur est positionné
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
}
