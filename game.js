...
try {
  ...
  try {
    ...
    if (!state.initialSyncDone) {
      ...
      // éviter répétition plus tard
      state.outcomeShownForGameId = gameId;
      return;
    }

    // éviter duplicate pour même partie
    if (state.outcomeShownForGameId === gameId) {
      dbgLog('outcome already shown for', gameId);
      return;
    }
    ...
    if (state.userId && actualWinnerId && String(state.userId) === String(actualWinnerId)) {
      ...
      continueBtn.onclick = async () => {
        ...
        try {
          // l'appel serveur/logic: finalize round and followup (tu as cette fn)
          await handleFinalizeRoundAndFollowup(gameId, { starterChar: null });
        } catch (err) {
    ...
    // marquer comme montré (empêcher répétition)
    state.outcomeShownForGameId = gameId;
  } catch (e) {
