...
try {
  ...
  dbgLog('overlay show guard failed', e);
    ...
    try {
      ...
      // Ne pas appeler updatePlayerUI ici : les scores seront mis à jour une seule fois
      // à la fin (après joinNewGame / exitTransitionForNonWinners) pour éviter le flickering.
  ...
  try {
    ...
    try {
      ...
      if (String(gameId) === String(g.id)) {
        dbgLog('handleGlobalNewGameCreated: already joined this game (winner path), nothing to do', g.id);
        // joinNewGame a déjà rendu le board et mis à jour l'UI — ne pas re-rendre pour éviter le flicker
      } else {
...
try {
  if (_shouldHideLoading) {
    ...
    // L'overlay est retiré, isTransitionLoading=false :
    // forcer un rendu final du board et mise à jour des scores pour le perdant/spectateur.
    // (pendant joinNewGame, isTransitionLoading était true donc renderBoard était supprimé)
...
btns.appendChild(closeBtn);     
  ...
  try {
    ...
    if (info.new_game_id) {
      ...
      // joinNewGame fait déjà fetchAndApplyPartyAndSeries + pollGameOnce + renderBoard + updatePlayerUI en interne.
    ...
    // Pas de fetchAndApplyPartyAndSeries ni pollGameOnce ici : déjà fait dans joinNewGame/createNewSeriesFromSeries.
    // Un appel supplémentaire re-triggerait renderBoard + updatePlayerUI et provoquerait le flickering.
  } catch (e) {
...
state.series_winner = null;
// fetchAndApplyPartyAndSeries sera fait dans joinNewGame — ne pas doubler ici pour éviter le flickering.
...
// reset local board and UI for immediate join — joinNewGame gère fetchAndApplyPartyAndSeries + pollGameOnce en interne
await joinNewGame(newGameId, starterChar);
return resp;
    } catch (e) {
...
// Render empty board immediately pour le gagnant (réinitialisation visuelle instantanée).
// Pour le perdant/spectateur, l'overlay "Chargement..." masque déjà le board : on n'affiche rien.
if (!state.isTransitionLoading) {
...
try {
  ...
  dbgLog('update URL on joinNewGame failed', e);
    ...
    // pollGameOnce a déjà chargé party_id / series_id. On ne refait qu'un seul appel
    // fetchAndApplyPartyAndSeries pour mettre les compteurs à jour, sans re-fetcher manuellement
    // les rows parties/series (évite les updatePlayerUI en cascade qui causent le flickering).
    try {
      // si pollGameOnce n'a pas rempli party_id (cas limite), tenter depuis games row uniquement
      ...
      dbgLog('joinNewGame: best-effort fetch games row failed', e);
    }
    // Un seul appel fetchAndApplyPartyAndSeries pour charger scores/series de façon autoritaire
    try {
      await fetchAndApplyPartyAndSeries(state.party_id, state.series_id);
    } catch (e) {
