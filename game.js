...
// --- NEW: join match (rpc_join_match) to set role (owner/opponent/animator/spectator)
async function joinMatchRpc() {
  if (!state.match_id) return;
  try {
    const { data, error } = await supabase.rpc('rpc_join_match', { p_match_id: state.match_id });
    if (error) {
      dbgLog('rpc_join_match error', error);
      return;
    }
    const row = Array.isArray(data) ? data[0] : data;
    if (row) {
      state.userRole = row.role ?? 'spectator';
      dbgLog('joined match via rpc_join_match, role=', state.userRole, row);
      // optional: reflect role in a DOM element with id="my_role" if present
      const roleEl = document.getElementById('my_role');
      if (roleEl) roleEl.textContent = state.userRole;
    }
  } catch (e) {
    dbgLog('joinMatchRpc exception', e);
  }
}
...
joinBtn.onclick = async () => {
  ...
  // NE PAS marquer la série comme montrée ici : on doit marquer seulement *après* affichage réelle
  return;
}
...
if (!state.initialSyncDone) {
  ...
  return;
      ...
      // --- Replacement inside showOutcomeModal ---
// determine roles
const isOwner = state.userId && String(state.userId) === String(state.ownerId);
const isOpponent = state.userId && String(state.userId) === String(state.opponentId);
const isSpectator = state.userRole === 'spectator' || (!isOwner && !isOpponent && !!state.userId);

const winnerIsOwner = winnerId && String(winnerId) === String(state.ownerId);
const winnerIsOpponent = winnerId && String(winnerId) === String(state.opponentId);
const amWinner = state.userId && winnerId && String(state.userId) === String(winnerId);
const amLoser = (isOwner || isOpponent) && !amWinner;
...
// Winner message for the winner and for spectators (owner-based mirroring)
if (amWinner) {
  // you are the winner
  who.textContent = winnerPseudo ? `Félicitations — vous avez gagné !` : 'Félicitations — vous avez gagné !';
} else if (isSpectator) {
  // spectators see who won
  who.textContent = winnerPseudo ? `${winnerPseudo} a gagné.` : 'La partie est terminée.';
} else if (amLoser) {
  // loser sees a short defeat message (no repeated pseudo)
  who.textContent = 'Vous avez perdu.';
} else {
  // fallback
  who.textContent = winnerPseudo ? `${winnerPseudo} a gagné.` : 'La partie est terminée.';
}
...
// play sounds:
// - winner: play victory.wav
// - loser (owner/opponent): play defeat.wav
// - spectators: mirror owner result (if owner won -> victory for spect; if owner lost -> defeat for spect)
try {
  // Ensure we only play one sound and only once per finished game (outcomeShownForGameId prevents repeats)
  if (amWinner) {
    playSound(VICTORY_SOUND);
    launchEmojis();
  } else if (isSpectator) {
    if (winnerIsOwner) playSound(VICTORY_SOUND); 
    launchEmojis(); 
    else playSound(DEFEAT_SOUND);
  } else if (amLoser) {
    playSound(DEFEAT_SOUND);
  }
} catch (e) { dbgLog('play sound failed', e); }
...
try {
  ...
  hideLoadingOverlay();
      ...
      // after initial sync, join match participants (rpc) so server knows our role
if (state.match_id && state.userId) {
  try { await joinMatchRpc(); } catch (e) { dbgLog('joinMatchRpc failed', e); }
}
      if (state.match_id) {
