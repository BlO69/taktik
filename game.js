// game.js (corrigé : appelle rpc_finalize_round après rpc_place_move lorsque nécessaire)
...
export default async function initGame(supabaseClient) {
  ...
  let state = {
    ...
    userId: null,
    winnerPseudo: null,
    // ---- new flags & score containers ----
    partyOwnerWins: 0,
    partyOpponentWins: 0,
    seriesOwnerWins: 0,
    seriesOpponentWins: 0,
    roundFinalizedForGame: {}, // map gameId -> boolean
  };
  ...
  function updatePlayerUI() {
    ...
    setText('partyOwnerWins', state.partyOwnerWins ?? '');
    setText('partyOpponentWins', state.partyOpponentWins ?? '');
    setText('seriesOwnerWins', state.seriesOwnerWins ?? '');
    setText('seriesOpponentWins', state.seriesOpponentWins ?? '');
  ...
  async function placeMoveRPC(pos, row, col, playerChar) {
    ...
    try {
      ...
      if (gameFinished) {
        ...
        // ---- NEW: call rpc_finalize_round immediately if server returned winner OR move_id ----
        try {
          const shouldFinalize = (resp.winner || resp.move_id);
          if (shouldFinalize) {
            // Avoid double finalization for same game
            if (!state.roundFinalizedForGame[gameId]) {
              state.roundFinalizedForGame[gameId] = true; // optimistic mark
              const finalizeRes = await finalizeRoundRPC(gameId);
              // finalizeRoundRPC handles UI updates and possibly joining new game
              dbgLog('finalizeRoundRPC result', finalizeRes);
            } else {
              dbgLog('round already finalized (client flag)', gameId);
            }
          } else {
            dbgLog('rpc_place_move indicated finished but no winner/move_id — skipping finalize');
          }
        } catch (e) {
          dbgLog('finalizeRoundRPC failed', e);
          // reset flag to allow retry later (e.g., via realtime or manual)
          state.roundFinalizedForGame[gameId] = false;
        }
      } else {
  ...
  // NEW: finalizeRoundRPC - call server rpc_finalize_round and handle response
  async function finalizeRoundRPC(gId) {
    if (!gId) return null;
    try {
      dbgLog('Calling rpc_finalize_round for game', gId);
      const { data, error } = await supabase.rpc('rpc_finalize_round', { p_game_id: gId });
      if (error) {
        dbgLog('rpc_finalize_round error', error);
        // try to resync game state so UI is consistent
        await pollGameOnce();
        // rethrow to let caller decide
        throw error;
      }
      const resp = Array.isArray(data) ? data[0] : data;
      if (!resp) {
        dbgLog('rpc_finalize_round returned empty', data);
        return null;
      }

      // Update local scores / series info from RPC result
      state.partyOwnerWins = Number(resp.party_owner_wins ?? state.partyOwnerWins);
      state.partyOpponentWins = Number(resp.party_opponent_wins ?? state.partyOpponentWins);
      state.seriesOwnerWins = Number(resp.series_owner_wins ?? state.seriesOwnerWins);
      state.seriesOpponentWins = Number(resp.series_opponent_wins ?? state.seriesOpponentWins);

      // if RPC returned party_winner / series_winner, we can compute winnerPseudo if profile known
      if (resp.party_winner) {
        if (String(resp.party_winner) === String(state.ownerId)) state.partyWinnerPseudo = state.ownerProfile?.pseudo;
        else if (String(resp.party_winner) === String(state.opponentId)) state.partyWinnerPseudo = state.opponentProfile?.pseudo;
      }
      if (resp.series_winner) {
        if (String(resp.series_winner) === String(state.ownerId)) state.seriesWinnerPseudo = state.ownerProfile?.pseudo;
        else if (String(resp.series_winner) === String(state.opponentId)) state.seriesWinnerPseudo = state.opponentProfile?.pseudo;
      }

      // update UI badges/texts
      updatePlayerUI();

      // show modal / toast about new round & scores
      showRoundFinalizedModal({
        party_owner_wins: state.partyOwnerWins,
        party_opponent_wins: state.partyOpponentWins,
        series_owner_wins: state.seriesOwnerWins,
        series_opponent_wins: state.seriesOpponentWins,
        new_game_id: resp.new_game_id ?? null,
        party_id: resp.party_id ?? null
      });

      // If server created a new game for the next match, automatically switch/join it
      if (resp.new_game_id) {
        try {
          // small delay to let modal show
          setTimeout(() => {
            joinNewGame(String(resp.new_game_id));
          }, 800);
        } catch (e) {
      ...
      dbgLog('finalizeRoundRPC exception', e);
      // ensure we clear the client-side finalized flag so retry is possible
      state.roundFinalizedForGame[gId] = false;
      return null;
    }
  }

  // Modal UI to show after finalizeRound
  function showRoundFinalizedModal({ party_owner_wins, party_opponent_wins, series_owner_wins, series_opponent_wins, new_game_id, party_id }) {
    try {
      // Remove previous modal if any
      const prev = document.getElementById('roundFinalizedModal');
      if (prev) prev.remove();

      const overlay = document.createElement('div');
      overlay.id = 'roundFinalizedModal';
      ...
      title.textContent = 'Nouvelle manche / scores';
      title.style.marginTop = '0';
      card.appendChild(title);

      const p = document.createElement('div');
      p.innerHTML = `
        <div>Manches — Joueur 1: <strong>${party_owner_wins ?? 0}</strong> vs Joueur 2: <strong>${party_opponent_wins ?? 0}</strong></div>
        <div style="margin-top:6px">Séries — Joueur 1: <strong>${series_owner_wins ?? 0}</strong> vs Joueur 2: <strong>${series_opponent_wins ?? 0}</strong></div>
      `;
      card.appendChild(p);
      ...
      if (new_game_id) {
        ...
        joinBtn.onclick = () => {
          overlay.remove();
          joinNewGame(String(new_game_id));
        };
      ...
      // auto-remove after 8s if user doesn't close
      setTimeout(() => {
        try { overlay.remove(); } catch(_) {}
      }, 8000);
    } catch (e) {
      dbgLog('showRoundFinalizedModal failed', e);
    }
  }

  // Join a new game created by server (reset local board & subscribe)
  async function joinNewGame(newGameId) {
    if (!newGameId) return;
    try {
      dbgLog('Joining new game', newGameId);

      // teardown existing realtime/poll
      stopPolling();
      try {
      ...
      } catch (e) { dbgLog('cleanup channels fail', e); }
      channels = [];

      // reset some client state
      gameId = newGameId;
      state.board = {};
      state.moveCount = 0;
      ...
      // fetch authoritative row then subscribe
      await pollGameOnce();
      await setupRealtime();
      startPolling();

      dbgLog('joined new game done', newGameId);
    } catch (e) {
      dbgLog('joinNewGame exception', e);
    }
  }
      ...
      chan.on('postgres_changes', {
        ...
        (async () => {
          ...
          if (rec.status === 'finished') {
            ...
            // optionally call finalize here if not yet done (server idempotent)
            try {
              if (!state.roundFinalizedForGame[gameId]) {
                state.roundFinalizedForGame[gameId] = true;
                await finalizeRoundRPC(gameId);
              }
            } catch (e) {
              dbgLog('finalizeRoundRPC (from realtime) failed', e);
              state.roundFinalizedForGame[gameId] = false;
            }
          } else {
            updateTurnUI();
