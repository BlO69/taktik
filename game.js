// game.js (version modifiée — idempotence renforcée par token par position)
// Basé sur la version fournie par l'utilisateur ; principaux changements :
// - introduction de `positionToToken` (Map) pour associer à chaque position le token
//   réellement appliqué (id de move ou `pos:<n>`), ce qui rend l'affichage strictement idempotent
// - updateCellAt vérifie maintenant occupant+token avant toute modification DOM
...
// Map position -> token (string). Token = rec.id (string) when available, otherwise "pos:<n>".
// This map allows strict idempotence per-cell when updating DOM.
const positionToToken = new Map();

function markAppliedMove(rec) {
  if (!rec) return;
  const pos = Number(rec.position);
  const tokenFromId = rec.id ? String(rec.id) : null;
  if (rec.id) appliedMoveIds.add(String(rec.id));
  if (Number.isFinite(pos)) appliedMoveIds.add(`pos:${pos}`);
  // set mapping pos -> token (prefer real id if available)
  if (Number.isFinite(pos)) {
    const token = tokenFromId || `pos:${pos}`;
    positionToToken.set(pos, token);
  }
}
...
// Strict idempotence: require both occupant and token equality to skip DOM changes.
function updateCellAt(pos) {
  if (!Number.isFinite(pos) || pos < 0 || pos >= ROWS * COLS) return;
  const cell = cellEls[pos] || (boardGrid && boardGrid.querySelector(`[data-i="${pos}"]`));
  if (!cell) return;

  // Normalise occupant: only render a piece if it's strictly 'X' or 'O'
  const boardOccupant = (isValidSymbol(board[pos])) ? board[pos] : null;

  // expected token for this pos
  const expectedToken = positionToToken.has(pos) ? String(positionToToken.get(pos)) : null;

  // Avoid needless DOM churn: check current occupant + current token
  const currentOccupant = (cell.dataset && typeof cell.dataset.occupant !== 'undefined') ? cell.dataset.occupant : null;
  const currentToken = (cell.dataset && typeof cell.dataset.moveId !== 'undefined') ? cell.dataset.moveId : null;

  // If both occupant and token match expected, nothing to do
  if (currentOccupant === boardOccupant && currentToken === expectedToken) {
    return;
  }

  // Otherwise, clear content and set according to boardOccupant
  try {
    while (cell.firstChild) cell.removeChild(cell.firstChild);
  } catch (e) {
    // ignore DOM removal errors, continue to set dataset safely
    dbgLog('updateCellAt: clearing children failed', e);
  }
  ...
  if (boardOccupant) {
    ...
    // set token dataset
    if (expectedToken) cell.dataset.moveId = expectedToken;
    else delete cell.dataset.moveId;
    ...
    // cell empty — remove any piece-related classes and dataset
    cell.classList.remove('piece-x', 'piece-o', 'winning');
    delete cell.dataset.occupant;
    delete cell.dataset.moveId;
  }
}
    ...
    try {
      ...
      for (const m of moves) {
        ...
        if (!board[pos]) {
          ...
          // mark applied and map pos->token
          if (m) markAppliedMove(m);
          else if (mid) {
            appliedMoveIds.add(mid);
            positionToToken.set(pos, mid);
          }
          changed = true;
          lastAppliedMove = m;
          dbgLog('poll applied move', { id: mid, pos, player, mi });
        } else if (board[pos] !== player) {
          dbgLog('poll conflict ignored', {
            pos,
            local: board[pos],
            server: player
          });
          ...
          if (!positionToToken.has(pos) && mid) positionToToken.set(pos, mid);
        }
...
function applyRealtimeMove(rec) {
  try {
    ...
    if (!board[pos]) {
      ...
      occupiedPositions.add(pos);
      markAppliedMove(rec);
      // ensure the position token is set
      if (mid && !positionToToken.has(pos)) positionToToken.set(pos, mid);
...
async function placeMove(r, c, playerSymbol, options = {}) {
  ...
  try {
    ...
    if (supabase && isRemotePlaying && gameId) {
        ...
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          ...
          if (rpcRow && rpcRow.board) {
            try {
              ...
              // Merge: replace canonical board and rebuild occupiedPositions & tokens
              board = newBoard;
              occupiedPositions.clear();
              appliedMoveIds.clear();
              positionToToken.clear();
      ...
      positionToToken.set(i, `pos:${i}`);
      appliedMoveIds.add(`pos:${i}`);
      moveIndex++;
    ...
    if (Array.isArray(moves)) {
      for (const m of moves) {
        try {
          ...
          if (m.id) {
            appliedMoveIds.add(String(m.id));
            positionToToken.set(pos, String(m.id));
          } else {
            appliedMoveIds.add(`pos:${pos}`);
            positionToToken.set(pos, `pos:${pos}`);
          }
...
function resetLocalGame(stopAll = true) {
  ...
  if (boardGrid) {
    for (let pos = 0; pos < ROWS * COLS; pos++) {
      const cell = boardGrid.querySelector(`[data-i="${pos}"]`);
      if (cell) {
        while (cell.firstChild) cell.removeChild(cell.firstChild);
        delete cell.dataset.occupant;
        delete cell.dataset.moveId;
