...
// utilitaire pour synchroniser l'état game -> window.gameState
  ...
  try {
    ...
    // mirror series_id and party_id if present (new)
    if (state.series_id) {
      window.gameState.series_id = state.series_id;
      window.gameState.seriesId = state.series_id;
    } else {
      window.gameState.series_id = window.gameState.series_id ?? null;
      window.gameState.seriesId = window.gameState.seriesId ?? window.gameState.series_id;
    }

    if (state.party_id) {
      window.gameState.party_id = state.party_id;
      window.gameState.partyId = state.party_id;
    } else {
      window.gameState.party_id = window.gameState.party_id ?? null;
      window.gameState.partyId = window.gameState.partyId ?? window.gameState.party_id;
    }
    ...
    if (state.ownerId) {
      ...
      window.gameState.owner_id = window.gameState.owner_id ?? null;
      window.gameState.ownerId = window.gameState.ownerId ?? window.gameState.owner_id;
    }
    ...
    if (state.opponentId) {
      ...
      window.gameState.opponent_id = window.gameState.opponent_id ?? null;
      window.gameState.opponentId = window.gameState.opponentId ?? window.gameState.opponent_id;
    }

    // ✅ mirror match_id if present
    if (state.match_id) {
      window.gameState.match_id = state.match_id;
      window.gameState.matchId = state.match_id;
    } else {
      window.gameState.match_id = window.gameState.match_id ?? null;
      window.gameState.matchId = window.gameState.matchId ?? window.gameState.match_id;
    }
    ...
    // ensure URL contains current game_id (so refresh keeps the same game)
    try {
      if (gameId) {
        ...
        if (state.series_id) params.set('series_id', String(state.series_id));
        if (state.party_id) params.set('party_id', String(state.party_id));
...
// ensure everyone subscribes to the new series/party immediately
try {
  await setupScoreSubscriptions();
} catch(e) {
  dbgLog('setupScoreSubscriptions after createNewSeriesFromSeries failed', e);
}
// push a quick authoritative resync & update UI so counters update immediately
try { await pollGameOnce(); } catch(e){ dbgLog('pollGameOnce after createNewSeriesFromSeries failed', e); }
updatePlayerUI();
...
try {
  ...
  dbgLog('update URL on joinNewGame failed', e);
    ...
    try {
      ...
      // --- ensure we have party/series info even if pollGameOnce didn't fill it ---
if (!state.series_id && state.party_id) {
  try {
    const { data: prow, error: perr } = await supabase
      .from('parties')
      .select('id, series_id, owner_wins, opponent_wins, winner_id, match_id')
      .eq('id', state.party_id)
      .maybeSingle();
    if (!perr && prow) {
      state.series_id = prow.series_id ?? state.series_id;
  ...
  } catch(e) { dbgLog('joinNewGame: fetch party failed', e); }
}

// If we have a series_id now, fetch series row to populate series scores (so UI updates immediately)
if (state.series_id) {
  try {
    const { data: srow, error: serr } = await supabase
      .from('series')
      ...
      .maybeSingle();
    if (!serr && srow) {
      // don't override values coming from rpc_finalize_round unless they were empty
      if (!state.series_owner_wins) state.series_owner_wins = Number(srow.owner_wins ?? state.series_owner_wins ?? 0);
      if (!state.series_opponent_wins) state.series_opponent_wins = Number(srow.opponent_wins ?? state.series_opponent_wins ?? 0);
  ...
  } catch(e) { dbgLog('joinNewGame: fetch series failed', e); }
}

// Ensure we subscribe to series updates and refresh UI counters immediately
try {
  await setupScoreSubscriptions();
} catch(e){ dbgLog('joinNewGame: setupScoreSubscriptions failed', e); }
updatePlayerUI();
    } catch(e) {
  ...
  function showOutcomeModal({ winnerId = null, winnerPseudo = null } = {}) {
    try {
      // Allow the winner to see the outcome modal immediately even if initialSyncDone isn't true yet
const amWinner = state.userId && winnerId && String(state.userId) === String(winnerId);
if (!state.initialSyncDone && !amWinner) {
  dbgLog('suppress outcome modal until initial sync (not winner)');
  return; // ne pas marquer outcomeShown ici
}
      // remove existing
      ...
      if (amLoser) {
  // Message pour le perdant : on n'adresse PAS le gagnant au perdant.
  if (winnerPseudo) who.textContent = `Vous avez perdu — gagnant : ${winnerPseudo}.`;
  else who.textContent = 'Vous avez perdu.';
} else if (amWinner) {
  // Message clair pour le gagnant (utilise son pseudo si disponible)
  const mePseudo = (state.userId && ((String(state.userId) === String(state.ownerId)) ? state.ownerProfile?.pseudo : state.opponentProfile?.pseudo)) || '';
  if (mePseudo) who.textContent = `Félicitations ${mePseudo} — vous avez gagné !`;
  else if (winnerPseudo) who.textContent = `Vous avez gagné — ${winnerPseudo}.`;
  else who.textContent = 'Vous avez gagné !';
} else {
  // Spectateur
  if (winnerPseudo) who.textContent = `${winnerPseudo} a gagné.`;
      ...
      if (amWinner) {
  // tentative immédiate
  try { playSound(VICTORY_SOUND); } catch (e) { /* ignore */ }
      }
