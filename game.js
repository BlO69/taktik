...
const markDismissed = () => {
  try {
    ...
    // --- PERDANT DE SÉRIE : ne voit pas la modale "Série terminée" ---
    // Le perdant a déjà vu la modale defeat (showOutcomeModal).
    // Seuls le gagnant de la série et les spectateurs voient cette modale.
    const _isSeriesPlayer = state.userId && (
      String(state.userId) === String(state.ownerId) ||
      String(state.userId) === String(state.opponentId)
    );
    if (_isSeriesPlayer && info.series_winner && String(state.userId) !== String(info.series_winner)) {
      dbgLog('suppress series modal for series loser (they see defeat modal instead)', state.userId);
      // marquer quand même pour éviter que d'autres handlers re-tentent l'affichage
...
try {
  ...
  try {
    ...
    // s'assurer que lastRoundWinnerId est à jour pour les détections amLocalWinner en aval
    if (winnerId) {
      try { state.lastRoundWinnerId = winnerId; } catch(e){ dbgLog('set lastRoundWinnerId failed', e); }
    }
    // supprimer la modale outcome pour les non-gagnants si une new-round modal a été affichée récemment
    ...
    // suspendre jusqu'à la sync initiale
    if (!state.initialSyncDone) {
    ...
    // marquage immédiat pour éviter les races
    state.outcomeShownForGameId = String(targetGameId);
    // effacer sentinelle new-round périmée
    state.newRoundShownForGameId = null;
    state._newRoundShownAt = null;
    ...
    // --- résolution des rôles et pseudos ---
    ...
    const amWinner = amPlayer && actualWinnerId && state.userId && String(state.userId) === String(actualWinnerId);

    // ====== GAGNANT DE MANCHE — fast-path ======
    // Le gagnant ne voit PAS la modale outcome classique.
    // On joue victory.wav, on auto-finalise, puis on affiche :
    //   • "Nouvelle manche" si la série n'est pas encore close (manche 1 ou 2)
    //   • "Série terminée"  si c'est la 3ᵉ victoire (manche closeuse de série, best-of-5)
    if (amWinner) {
        ...
        .then(finalizeRow => {
          try {
            if (finalizeRow && finalizeRow.series_winner) {
              // Série close → showSeriesModal déjà appelé à l'intérieur de handleFinalizeRoundAndFollowup
              dbgLog('winner fast-path: série close, serie modal géré dans handleFinalizeRoundAndFollowup');
            } else {
              // Manche non-décisive pour la série → afficher "Nouvelle manche" au gagnant
              showNewRoundModal({
                party_id: state.party_id,
                party_owner_wins: state.party_owner_wins,
                party_opponent_wins: state.party_opponent_wins,
      ...
      return; // le gagnant ne voit pas la modale outcome
    }

    // ====== PERDANT / SPECTATEUR — modale defeat ======
    const overlay = document.createElement('div');
    ...
    // container pour emojis/confetti (non-interactif, conservé pour usage futur)
    ...
    if (amLoser) {
      msg.textContent = amOwner
        ? `Dommage, ${ownerPseudo} — vous avez perdu cette manche.`
        : `Dommage, ${opponentPseudo} — vous avez perdu cette manche.`;
    } else {
      // spectateur
    ...
    infoEl.style.color = '#444';
    infoEl.style.marginBottom = '14px';
    infoEl.textContent = 'Le vainqueur peut lancer la manche suivante.';
    ...
    // --- son defeat joué une seule fois pour le perdant (à l'affichage de la modale) ---
    if (amLoser) {
