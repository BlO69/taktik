...
// 1337..1344 replacement
const intendedSeriesId = state.series_id ? String(state.series_id) : (g && g.series_id ? String(g.series_id) : '');
// si l'utilisateur courant est le gagnant de la série, ne PAS pré-marquer : on laisse
// showSeriesModal gérer l'idempotence côté gagnant (afin qu'il puisse voir/interagir).
...
if (amSeriesWinner) {
  ...
  }); } catch(e){ dbgLog('showSeriesModal (winner path) failed', e); }
} else {
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    try { showSeriesModal({
      ...
      new_game_id: (typeof g !== 'undefined' && g) ? g.id ?? null : null,
      last_round_winner: state.lastRoundWinnerId
    }); } catch(e){ dbgLog('showSeriesModal failed', e); }
  }
}
} catch (e) {
        ...
        if (state.outcomeShownForGameId !== gameId) {
          ...
          try {
            ...
            if (state.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
             ...
             // 1656..1662 replacement
const intendedSeriesId = state.series_id ? String(state.series_id) : '';
const amSeriesWinner = state.userId && state.series_winner && (String(state.userId) === String(state.series_winner));
if (amSeriesWinner) {
  // laisser showSeriesModal gérer son propre marquage/idempotence pour le gagnant
  showSeriesModal({
    series_id: state.series_id,
    series_owner_wins: state.series_owner_wins,
    series_opponent_wins: state.series_opponent_wins,
...
if (finalizeRow.series_winner) {
  // 2058..2064 replacement
const intendedSeriesId = String(finalizeRow.series_id ?? state.series_id ?? '');
const amSeriesWinner = state.userId && (String(state.userId) === String(finalizeRow.series_winner ?? state.series_winner ?? ''));
if (amSeriesWinner) {
  try {
    showSeriesModal({
  ...
  } catch(e){ dbgLog('showSeriesModal (finalize, winner path) failed', e); }
} else {
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
...
} // end if (finalizeRow.series_winner)
...
try {
  ...
  state._newRoundShownAt = null;
    ...
    try {
      ...
      // only play once per game session (pour la partie ciblée)
      if (String(state.outcomeShownForGameId) !== String(targetGameId)) {
        // if viewer is the actual winner -> victory
        if ((viewerRole === 'owner' && ownerWon) || (viewerRole === 'opponent' && opponentWon)) {
          ...
          // spectator hears owner's outcome
          if (ownerWon) {
            if (typeof playSound === 'function' && typeof VICTORY_SOUND !== 'undefined') playSound(VICTORY_SOUND);
          } else if (opponentWon) {
            if (typeof playSound === 'function' && typeof DEFEAT_SOUND !== 'undefined') playSound(DEFEAT_SOUND);
...
if (amSeriesWinner) {
  ...
  } catch(e){ dbgLog('showSeriesModal (subscription winner path) failed', e); }
} else {
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
...
if (state._pendingSeriesModal && state._pendingSeriesModal.length) {
  for (const item of state._pendingSeriesModal) {
    try {
  // 3276..3281 replacement
const intendedSeriesId = item?.info?.series_id ? String(item.info.series_id) : (state.series_id ? String(state.series_id) : '');
const amSeriesWinner = state.userId && (String(state.userId) === String(item?.info?.series_winner ?? state.series_winner ?? ''));
if (amSeriesWinner) {
  try { showSeriesModal(item.info); } catch(e){ dbgLog('showSeriesModal (pending winner) failed', e); }
} else {
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
