...
joinBtn.onclick = async () => {
  ...
  try {
    ...
    // --- Idempotence: mark & early-exit if series modal already shown ---
try {
  if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    dbgLog('series modal already handled (idempotent guard)', intendedSeriesId);
    return;
  }
  // keep a state-level marker for other codepaths that inspect it
  state.seriesFinishedShownForSeriesId = intendedSeriesId;
} catch(e) {
  dbgLog('series modal pre-mark failed', e);
  // Not blocking: continue to attempt showing the modal, but we avoided race in normal case.
}
    if (!state.initialSyncDone) {
