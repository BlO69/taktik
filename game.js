...
// Sources originales : game - Copie.js & game.js (fichiers fournis). See file citations in chat.
// - Gère : création/join via RPC, rpc_place_move, realtime moves, polling fallback,
//   rendering 20x20 grid + minimap, sounds, controls, optimistic UI & rollback,
//   invitation -> game_id resolution, reconnect/backoff, and exposes window.taktikGame.
//
// Usage:
//   import initGame from './game.js';
//   initGame(window.supabase);
// or include as module <script type="module">import initGame from './game.js'; initGame(window.supabase);</script>
//
// Based on: :contentReference[oaicite:2]{index=2} :contentReference[oaicite:3]{index=3}
...
function dbgAlert(label, payload) {
  ...
  try {
    ...
    // Use console.log instead of alert in many contexts to avoid blocking
    console.info(`[DBG ALERT] ${label}:\n`, text);
  } catch (e) {
    try { console.info(`[DBG ALERT] ${label}: (unable to stringify payload)`); } catch (_) {}
  }
}
// expose debug helper globally so inline scripts can call it safely
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      cell.addEventListener('click', () => onCellClick(r, c));
            const val = board[i];
      if (val) {
        // add CSS class so background rules apply
...
// Normalise le format renvoyé par le serveur (jsonb) en Array length ROWS*COLS remplie de null|'X'|'O'
function normalizeBoard(b) {
  const out = new Array(ROWS * COLS).fill(null);
  if (b == null) return out;

  // si c'est une string JSONifiée, parser d'abord
  if (typeof b === 'string') {
  ...
  if (typeof b === 'object') {
    // objet sparse { "0":"X", "45":"O", ... }
    for (const k of Object.keys(b)) {
      const idx = Number(k);
      if (!Number.isNaN(idx) && idx >= 0 && idx < out.length) out[idx] = (b[k] == null ? null : b[k]);
    }
    return out;
  }
...
function applyRealtimeMove(rec) {
  try {
    if (!rec) return;
    // canonical record may include: position, player, move_index OR move_count, board, current_turn
    const pos = Number(rec.position);
    const player = rec.player;
    if (!Number.isFinite(pos) && !rec.board) return;

    // prefer server-provided move index fields
    const mi = Number(rec.move_index ?? rec.move_count ?? NaN);
    if (Number.isFinite(mi) && mi <= (moveIndex || 0)) {
      dbgLog('applyRealtimeMove: skipping old/duplicate move', { rec, moveIndex });
      // Even if duplicate, if server provided full board, merge it
      if (rec.board) {
        board = normalizeBoard(rec.board);
        renderGrid(); renderMiniMap();
      }
      return;
    }

    // If server provided full board, prefer it (strong canonical source)
    if (rec.board) {
      const newBoard = normalizeBoard(rec.board);
      board = newBoard;
      // set moveIndex if provided
      if (Number.isFinite(mi)) moveIndex = Math.max(moveIndex || 0, mi);
      else {
        // attempt to recalc from board (count non-null)
        const filled = board.reduce((s, v) => s + (v ? 1 : 0), 0);
        moveIndex = Math.max(moveIndex || 0, filled);
      }
      renderGrid(); renderMiniMap();
    } else {
      // no full board: apply single move if position valid
    ...
    // update turn from server if given
    current = rec.current_turn ?? current;
    if (currentTurnEl) currentTurnEl.textContent = current;

    // trigger canonical refresh if server claims finished or a winner present
    if (rec.status === 'finished' || rec.game_finished === true || rec.winner) {
      refreshGameState().catch(e => dbgLog('refreshGameState(from realtime finished) failed', e));
    }
  } catch (e) {
...
async function refreshGameState() {
  ...
  try {
    ...
    // board may be stored as array or object or absent
        // normalise board stocké côté serveur
    if (gRow?.board) {
      try {
        board = normalizeBoard(gRow.board);
        // try to set moveIndex from server row if present (some RPCs return move_count)
        // but the selected query above doesn't fetch move_count — fallback to counting non-null
        moveIndex = board.reduce((s, v) => s + (v ? 1 : 0), 0);
      } catch (e) {
        dbgLog('board normalization failed', e);
      }
    } else {
      // reconstruct from moves if necessary (and compute reliable moveIndex)
      try {
        ...
        if (!movesErr && Array.isArray(moves) && moves.length) {
          ...
          board = newBoard;
          moveIndex = Math.max(moveIndex || 0, maxIdx, board.reduce((s, v) => s + (v ? 1 : 0), 0));
        } else {
...
async function placeMove(r, c, playerSymbol, options = {}) {
  ...
  try {
    ...
    // cellule occupée
    if (board[i]) {
      try { alert('Cellule déjà occupée (client)'); } catch (_) {}
      return false;
    }
    ...
    // optimistic: save snapshot for rollback
    const prevBoardVal = board[i];
    const prevMoveIndex = moveIndex || 0;
    const prevCurrent = current;

    // play punchline first (local sound) - keep existing behaviour
    ...
    if (supabase && isRemotePlaying && gameId) {
      try {
        ...
        if (rpcErr) {
          ...
          // friendly message
          const msg = rpcErr?.message || rpcErr?.details || (rpcErr?.hint ? `${rpcErr.hint}` : null) || String(rpcErr);
          try { alert("Le serveur a rejeté le coup : " + (msg || 'erreur inconnue')); } catch(_) {}

          return false;
        }

        // normalize rpcRow (RPC might return array or single row)
        let rpcRow = null;
        if (Array.isArray(rpcData) && rpcData.length) rpcRow = rpcData[0];
        else rpcRow = rpcData;

        // If server provided canonical board, merge it (preferred)
        if (rpcRow && (rpcRow.board !== undefined && rpcRow.board !== null)) {
          try {
            // prefer a normalizeBoard helper if present, otherwise attempt safe parsing/assignment
            if (typeof normalizeBoard === 'function') {
              board = normalizeBoard(rpcRow.board);
            } else {
              if (typeof rpcRow.board === 'string') {
                try { board = JSON.parse(rpcRow.board); } catch (_) { board = rpcRow.board; }
              } else {
                board = rpcRow.board;
              }
            }

            // set moveIndex from server if present, else compute from board
            const mi = Number(rpcRow.move_count ?? rpcRow.move_index ?? NaN);
            if (Number.isFinite(mi)) moveIndex = mi;
            else moveIndex = (Array.isArray(board) ? board.reduce((s, v) => s + (v ? 1 : 0), 0) : moveIndex);
          ...
          // No canonical board — fallback to fetch last move and apply
          try {
        ...
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          // Try to map winner uuid to 'X'|'O' using owner/opponent info if available
          let winnerSymbol = null;
          try {
            const winnerUid = rpcRow.winner;
            if (winnerUid) {
              if (winnerUid === hostId) winnerSymbol = 'X';
              else if (winnerUid === opponentId) winnerSymbol = 'O';
              else if (sessionUser?.id && winnerUid === sessionUser.id) {
                winnerSymbol = (sessionUser.id === hostId) ? 'X' : (sessionUser.id === opponentId ? 'O' : null);
              }
            }
          } catch (e) { dbgLog('winner symbol mapping failed', e); }

          // ensure canonical state then call round end with symbol (if known)
          await refreshGameState();
          if (!winnerSymbol && rpcRow.aligned_count && Number(rpcRow.aligned_count) >= WIN) {
            // best-effort: assume the player who just played (playerSymbol) is the winner
          ...
          if (winnerSymbol) {
            ...
            // if we still don't know symbol, try to read games table winner uuid and map again
            try {
              const { data: g, error: gErr } = await supabase.from(GAME_TABLE)
                .select('winner,owner_id,opponent_id,board,current_turn')
                .eq('id', gameId)
                .maybeSingle();
              if (!g || gErr) {
                dbgLog('post-RPC canonical read failed or empty', gErr);
              } else if (g.winner) {
                if (g.winner === g.owner_id) await handleRoundEnd('X');
                else if (g.winner === g.opponent_id) await handleRoundEnd('O');
              }
            } catch (e) { dbgLog('post-RPC canonical read exception', e); }
          }
        ...
        // network / unexpected exception: rollback optimistic UI
        board[i] = prevBoardVal;
        moveIndex = prevMoveIndex;
        current = prevCurrent;
        if (currentTurnEl) currentTurnEl.textContent = current;
      ...
      // offline/local mode — run local winner detection
...
async function waitForGameId(invId, attempts = 12, intervalMs = 2000) {
  // helper: use dbgAlert if present, otherwise console.log
  ...
  try {
    ...
    if (menuGames) menuGames.addEventListener('click', async () => {
      if (window.taktikCreateGame && typeof window.taktikCreateGame.searchProfiles === 'function') {
        ...
        // try dynamic import of create_game.js if present
        try {
