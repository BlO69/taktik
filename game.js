...
export default async function initGame(supabaseClient) {
  ...
  async function handleGlobalNewGameCreated(g) {
    try {
      ...
      if (state.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
        try {
                    state.seriesFinishedShownForSeriesId = state.series_id;
  ...
  async function pollPartyNewGameOnce() {
    try {
      ...
      // query latest game for this party
      const { data: rows, error } = await supabase
        .from('games')
        .select('id, owner_id, opponent_id, party_id, created_at')
        .eq('party_id', state.party_id)
        .order('created_at', { ascending: false })
        .limit(1);

      if (error) {
        dbgLog('pollPartyNewGameOnce error', error);
        return;
      }
      ...
      // If we don't know lastPartyKnownGameId, initialize it to current gameId (so we don't immediately re-handle current)
      ...
      if (String(latest.id) !== String(gameId) && String(latest.id) !== String(state.lastPartyKnownGameId)) {
        ...
        const { data: g, error: gerr } = await supabase
          ...
          .maybeSingle();
        if (g && g.id) {
          // mark it known immediately to avoid races
          state.lastPartyKnownGameId = g.id;
  ...
  try {
    // Récupère la ligne du jeu (verrou logique côté client)
    const { data: g, error: gerr } = await supabase
      .from('games')
    ...
    if (gerr) {
      dbgLog('fetch game row error before finalize', gerr);
      throw gerr;
    }
    if (!g) {
      const err = new Error(`Game not found: ${p_game_id}`);
      dbgLog(err.message);
      throw err;
    }

    // --- Normalisation / détermination de winner en UUID si nécessaire ---
    try {
      let winnerToSet = null;

      // If there is no winner stored on the row: try to infer (state, then board)
      if (!g.winner) {
        if (state.winnerId && (String(state.winnerId) === String(g.owner_id) || String(state.winnerId) === String(g.opponent_id))) {
          ...
          // infer by aligned sequences on the client board
          let winnerChar = null;
          if (typeof findWinningSequences === 'function' && findWinningSequences(state.board, 'X').length) winnerChar = 'X';
          else if (typeof findWinningSequences === 'function' && findWinningSequences(state.board, 'O').length) winnerChar = 'O';
          if (winnerChar === 'X' && g.owner_id) winnerToSet = g.owner_id;
          if (winnerChar === 'O' && g.opponent_id) winnerToSet = g.opponent_id;
        }
      } else {
        // g.winner present: accept UUIDs or map 'X'/'O' -> UUIDs
        const gw = String(g.winner);
        if (gw === 'X' && g.owner_id) {
          winnerToSet = g.owner_id;
        } else if (gw === 'O' && g.opponent_id) {
          winnerToSet = g.opponent_id;
        } else if ((g.owner_id && String(g.owner_id) === gw) || (g.opponent_id && String(g.opponent_id) === gw)) {
          // already a participant UUID (keep it)
          winnerToSet = g.winner;
        } else {
          // last resort: if state has a plausible UUID, use it
          if (state.winnerId && (String(state.winnerId) === String(g.owner_id) || String(state.winnerId) === String(g.opponent_id))) {
            winnerToSet = state.winnerId;
          }
        }
      ...
      // write back only if we have a candidate and it differs from the stored value
      if (winnerToSet && String(winnerToSet) !== String(g.winner)) {
        try {
          ...
          if (uerr) {
            dbgLog('failed to update games.winner before finalize', uerr);
          } else {
            dbgLog('updated games.winner before finalize', winnerToSet);
            // keep local object in sync so following logic uses the UUID
            g.winner = winnerToSet;
            // also update state.winnerId if you use it elsewhere as canonical
            state.winnerId = winnerToSet;
          }
        } catch (e) {
      ...
      // on poursuit malgré l'échec de normalisation : rpc_finalize_round gérera l'idempotence/erreurs serveur
    }
    ...
    try {
      ...
      if (info.new_game_id) {
        ...
        joinBtn.onclick = async () => {
          overlay.remove();
          // When user clicks join, we try to join the new game immediately.
          // This is the moment the winner clicked finalize and created the new game; joinNewGame resets board & UI for this client.
  ...
  // Remplace : function showSeriesModal(info = {}) {
async function showSeriesModal(info = {}) {
  try {
    // ensure we know the current user (avoid race where modal shown before startup finished)
    if (!state.userId) {
      try {
        ...
        dbgLog('showSeriesModal: getUser failed', e);
      }
    }
    ...
    try {
      ...
      if (lastRoundWinner) {
        ...
        if (String(state.lastRoundWinnerId) === String(ownerId)) starterChar = 'X';
            ...
            // Attempt #1: let rpc_finalize_round on server create the child series by finalizing a
      // representative finished game from the parent series (server will create the new series/party/game).
      let candidateGameId = null;
      try {
        // find up to 10 most recent parties for that series (best-effort)
        const { data: parties, error: perr } = await supabase
          .from('parties')
          .select('id')
          .eq('series_id', parentSeriesId)
          .order('created_at', { ascending: false })
          .limit(10);

        if (!perr && Array.isArray(parties) && parties.length) {
          const partyIds = parties.map(p => p.id);
          // find the latest game among those parties
          const { data: latestGames, error: lgerr } = await supabase
            ...
            .order('created_at', { ascending: false })
            .limit(1);
          if (!lgerr && Array.isArray(latestGames) && latestGames.length) {
            candidateGameId = latestGames[0].id;
          }
        }
      } catch (e) {
        dbgLog('find candidate game for finalize failed', e);
      }

      if (candidateGameId) {
        try {
          const { data: finData, error: finErr } = await supabase.rpc('rpc_finalize_round', { p_game_id: candidateGameId });
          if (!finErr && finData) {
            const finRow = Array.isArray(finData) ? finData[0] : finData;
            if (finRow && finRow.new_game_id) {
              // server created the new series and returned the new game id: join it
              state.series_id = finRow.new_series_id ?? state.series_id;
              state.party_id = finRow.new_party_id ?? state.party_id;
              await joinNewGame(finRow.new_game_id, starterChar);
              return finRow;
            }
          }
        } catch (e) {
          dbgLog('rpc_finalize_round for new series failed', e);
        }
      }

      // Fallback: if above failed, create via rpc_new_game (existing behaviour)
  ...
  async function joinNewGame(newGameId, starterChar = null) {
    try {
      ...
      // set lastPartyKnownGameId so party poller doesn't re-handle this
