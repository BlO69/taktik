// game.js
// Init function exported par défaut.
...
export default async function initGame(supabaseClient) {
  ...
  function renderCell(pos, player, options={ aligned:false }) {
    ...
    if (options.aligned) {
      ...
      // remove aligned class after animation to avoid permanent state
  ...
  function renderBoard(boardObj) {
    ...
    // update DOM for changed keys (and leave others intact)
    // We'll render all cells for simplicity
  ...
  async function placeMoveRPC(pos, row, col, playerChar) {
    ...
    try {
      ...
      // RPC returns move state (as in function you shared)
      dbgLog('rpc_place_move success', data);
      // defensive: data may be array or object
      ...
      if (aligned >= WINLEN && resp.move_id) {
        // last move pos is pos — highlight it
  ...
  async function handleIncomingMoveRow(row) {
    ...
    try {
      ...
      const moveIndex = Number(row.move_index ?? row.move_index ?? 0);
      ...
      // optionally fetch game row to get authoritative current_turn/status
      await syncGameRow(); // lightweight sync
      updateTurnUI();
    } catch (e) {
  ...
  async function boot() {
    ...
    if (!gameId) {
      // maybe page created by create_game.js will set window.taktikGame later; wait short
