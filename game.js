// game.js (version corrigée — empêche l'affichage accidentel d'un pion en (0,0))
// Basé sur la version fournie par l'utilisateur ; principaux changements :
// - validation stricte des symboles acceptés ('X' / 'O')
// - ne rend un pion que si la valeur canonique est exactement 'X' ou 'O'
// - protection supplémentaire lors de la reconstruction du board à partir du serveur/moves
...
function renderGrid() {
  ...
  // assure la structure DOM (créée une seule fois)
  createGridStructureIfNeeded();

  // Mettre à jour chaque cellule (ne recrée pas les éléments parent)
  for (let pos = 0; pos < ROWS * COLS; pos++) {
    try { updateCellAt(pos); } catch (e) { dbgLog('updateCellAt failed', e, pos); }
  }
...
// Create grid DOM structure once (or when grid needs rebuild)
function createGridStructureIfNeeded() {
  if (!boardGrid) return;
  // check whether we already have the right number of cells
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      cell.dataset.c = c;
      cell.dataset.i = i;
      // keep click handler on cell — onCellClick uses r,c
      cell.addEventListener('click', () => onCellClick(r, c));
      // append empty placeholder (content managed by updateCell)
      cell.innerHTML = '';
...
function updateCellAt(pos) {
  ...
  // Avoid needless DOM churn: check current occupant
  ...
  // reset content but keep cell element
  // ensure we fully remove any leftover image nodes to avoid stray icons (safety)
...
function applyRealtimeMove(rec) {
  try {
    ...
    if (!board[pos]) {
      ...
      // mieux : update only the changed cell and minimap + UI turn update
      updateCellAt(pos);
      renderMiniMap();
        ...
        if (rpcRow && (rpcRow.game_finished === true || rpcRow.winner)) {
          ...
          if (rpcRow && rpcRow.board) {
            try {
              ...
              // ensure board has correct shape and sanitize values
              if (!Array.isArray(newBoard) || newBoard.length !== ROWS * COLS) {
                ...
                // sanitize values if already correct length
...
function resetLocalGame(stopAll = true) {
  ...
  // createGridStructureIfNeeded() renverra immédiatement si déjà construite.
  if (typeof createGridStructureIfNeeded === 'function') {
    createGridStructureIfNeeded();
  }

  // 3) vide l'affichage des pions.
  // Nous enlevons explicitement tout <img> et dataset.occupant sur CHAQUE cellule
  if (boardGrid) {
    for (let pos = 0; pos < ROWS * COLS; pos++) {
      const cell = boardGrid.querySelector(`[data-i="${pos}"]`);
      if (cell) {
        // remove children explicitly
        while (cell.firstChild) cell.removeChild(cell.firstChild);
        delete cell.dataset.occupant;
        cell.classList.remove('piece-x', 'piece-o', 'winning');
      }
    ...
    // fallback to renderGrid which will call updateCellAt
