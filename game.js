// game.js (corrigé : finalization controlée par le vainqueur, scores realtime, sons joués une seule fois,
// + synchronisation complète quand rpc_finalize_round crée une nouvelle party OU une nouvelle series)
export default async function initGame(supabaseClient) {
  ...
  let state = {
    ...
    winnerPseudo: null,
    winnerId: null,
    // party/series info filled by rpc_finalize_round or poll
    party_id: null,
    party_owner_wins: 0,
      ...
      gch.on('postgres_changes', {
        ...
        (async () => {
          try {
            ...
            // If we have a party_id and the new game belongs to it -> join (loser included)
            if (state.party_id && g.party_id && String(g.party_id) === String(state.party_id)) {
              ...
              if (String(g.id) === String(gameId)) {
                dbgLog('global insert: same gameId, skipping join');
                return;
              }
              // determine starterChar: if lastRoundWinnerId known and matches owner/opponent
              ...
              dbgLog('global insert: auto-joining new game for same party', { newGameId: g.id, starterChar });
              await joinNewGame(g.id, starterChar).catch(e => dbgLog('auto-joinNewGame failed', e));

              // fetch up-to-date party & series info to show modal to this client too
              try {
                const { data: partyRow } = await supabase.from('parties').select('id, owner_wins, opponent_wins, winner_id, series_id').eq('id', g.party_id).maybeSingle();
                let seriesRow = null;
                if (partyRow && partyRow.series_id) {
                  const res = await supabase.from('series').select('id, owner_wins, opponent_wins, winner_id').eq('id', partyRow.series_id).maybeSingle();
                  seriesRow = res.data ?? null;
                }
                showNewRoundModal({
                  party_id: partyRow?.id ?? null,
                  party_owner_wins: partyRow?.owner_wins ?? state.party_owner_wins ?? 0,
                  party_opponent_wins: partyRow?.opponent_wins ?? state.party_opponent_wins ?? 0,
                  party_winner: partyRow?.winner_id ?? state.party_winner,
                  series_id: seriesRow?.id ?? state.series_id,
                  series_owner_wins: seriesRow?.owner_wins ?? state.series_owner_wins ?? 0,
                  series_opponent_wins: seriesRow?.opponent_wins ?? state.series_opponent_wins ?? 0,
                  series_winner: seriesRow?.winner_id ?? state.series_winner,
                  new_game_id: g.id
                });
              } catch (e) { dbgLog('fetch party/series after global insert failed', e); }
              return;
            }

            // Otherwise, if the new game's players match our current pair, auto-join as well
            ...
            if (ourOwner && ourOpponent) {
              ...
              if (samePair) {
                ...
                dbgLog('global insert: auto-joining new game for matching pair', { newGameId: g.id, starterChar });
                await joinNewGame(g.id, starterChar).catch(e => dbgLog('auto-joinNewGame failed', e));

                // fetch party/series that contain this new game (if available) and show modal
                try {
                  const { data: partyRow } = await supabase.from('parties').select('id, owner_wins, opponent_wins, winner_id, series_id').eq('id', g.party_id).maybeSingle();
                  let seriesRow = null;
                  if (partyRow && partyRow.series_id) {
  ...
  // --- UPDATED: also fetch party -> series to populate state.series_id and scores for all clients ---
  async function pollGameOnce() {
    if (!gameId) return;
    try {
      const { data: g, error } = await supabase
        .from('games')
        .select('id,board,move_count,current_turn,status,owner_id,opponent_id,winner,party_id')
        .eq('id', gameId)
        .maybeSingle();
      ...
      // if we have a party_id, fetch its series id + current party counters (ensures sync after series end/new)
      if (state.party_id) {
        try {
          ...
          if (!perr && partyRow) {
            ...
            // set series_id from party
            state.series_id = partyRow.series_id ?? state.series_id;
          }
        } catch (e) { dbgLog('pollGameOnce party read failed', e); }
      }

      // if we have series_id, fetch series counters (keeps series counters in sync for all clients)
      if (state.series_id) {
        try {
        ...
        } catch (e) { dbgLog('pollGameOnce series read failed', e); }
      }
      ...
      // ensure player UI reflects latest profiles & scores
      updatePlayerUI();
  ...
  async function handleFinalizeRoundAndFollowup(p_game_id, options = {}) {
    try {
      ...
      // --- NEW: if server created a new game, reset local board/state immediately for ALL clients ---
      // This ensures the loser (et tous les autres clients connectés) voit la board réinitialisée
      // and scores updated right away, even if they don't (yet) auto-join the new game.
      ...
      if (newGameId) {
        ...
        // outcome modal for the finished game already shown; reset it for the new game lifecycle
        state.winnerId = null;
        state.winnerPseudo = null;
        state.outcomeShownForGameId = null;

        // re-render an empty board for this client (so loser sees cleared board immediately)
        renderBoard(state.board);
        updateTurnUI();
      }
    ...
    try {
      ...
      try {
        for (const ch of channels) {
          // do NOT remove globalGamesChannel here (we only remove channel objects that are per-game)
      ...
      // refresh authoritative state (this will also populate party_id and series_id via the updated pollGameOnce)
      await pollGameOnce();
      ...
      if (isWinner) {
        ...
        continueBtn.onclick = async () => {
          try {
            ...
            // NOTE: do not play sound here — the sound was already played once when modal opened
        ...
        // play victory sound on modal open (only once)
        ...
        // play defeat sound on modal open (only once)
  ...
  // Ensure we have realtime listeners on the current party and series so scores get updated for everyone.
      ...
      if (state.party_id) {
        ...
        pchan.on('postgres_changes', {
          ...
          try {
            const rec = payload?.new;
            if (!rec) return;
            dbgLog('party UPDATED realtime', rec);
            // update local party scores/winner
            state.party_owner_wins = Number(rec.owner_wins ?? state.party_owner_wins ?? 0);
            state.party_opponent_wins = Number(rec.opponent_wins ?? state.party_opponent_wins ?? 0);
            state.party_winner = rec.winner_id ?? state.party_winner;
            // reflect on UI immediately for everyone
            updatePlayerUI();
          } catch (e) { dbgLog('party update handler failed', e); }
        });

        try { await pchan.subscribe(); } catch(e) { dbgLog('pchan subscribe fail', e); }
      }
      ...
      if (state.series_id) {
        ...
        schan.on('postgres_changes', {
          ...
          try {
            const rec = payload?.new;
            if (!rec) return;
            dbgLog('series UPDATED realtime', rec);
            // update local series scores/winner
            state.series_owner_wins = Number(rec.owner_wins ?? state.series_owner_wins ?? 0);
            state.series_opponent_wins = Number(rec.opponent_wins ?? state.series_opponent_wins ?? 0);
            state.series_winner = rec.winner_id ?? state.series_winner;
            // reflect on UI immediately for everyone
            updatePlayerUI();

            // If this series was just finished (rec.status maybe), other clients may expect a new series creation.
            // They will get the global INSERT for the new game's creation and join automatically (handled by global insert).
          } catch (e) { dbgLog('series update handler failed', e); }
        });

        try { await schan.subscribe(); } catch(e) { dbgLog('schan subscribe fail', e); }
      }
    } catch (e) {
      dbgLog('setupScoreSubscriptions failed', e);
    }
  }
  ...
  async function boot() {
    ...
    // ensure global games subscription to detect new games (for losers)
    await setupGlobalGamesInsertSubscription();

    // also setup score subscriptions if we already have party/series
    await setupScoreSubscriptions();
