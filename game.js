...
// NORMALISATION : on convertit les id null/'' en '__none__' pour éviter
// réapparitions dues à des id équivalentes mais représentées différemment.
...
// Utilitaire complémentaire : tester si la série/modal a été volontairement fermée par l'utilisateur
function wasModalDismissed(stateKey, id) {
  try {
    const sid = (id == null || id === '') ? '__none__' : String(id);
    return state[`${stateKey}Dismissed`] && String(state[`${stateKey}Dismissed`]) === sid;
  } catch (e) {
    return false;
  }
}
...
// markDismissed : appeler quand l'utilisateur ferme volontairement la modale (ou clique Nouvelle série)
const markDismissed = () => {
  try {
    ...
    info = await ensureModalHasAuthoritativeScores(info);
    // --- NEW: only show the full "Série terminée" modal to the series winner ---
// For all other clients: mark the series as shown, update local scores/UI and return.
if (info.series_winner && state.userId && String(state.userId) !== String(info.series_winner)) {
  dbgLog('series finished — skipping modal UI for non-winner', intendedSeriesId);
  // mark as shown so other handlers won't try to display it again
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
  // update local authoritative counters and UI
  state.series_owner_wins = Number(info.series_owner_wins ?? state.series_owner_wins ?? 0);
  state.series_opponent_wins = Number(info.series_opponent_wins ?? state.series_opponent_wins ?? 0);
  state.series_winner = info.series_winner ?? state.series_winner;
  updatePlayerUI();
  return; // do NOT render the series modal for non-winner clients
}
      ...
      try {
        ...
        if (userIsSeriesWinner && info.series_id) {
          ...
          newSeriesBtn.onclick = async () => {
            markDismissed();
            // ensure nobody else will re-show this series modal while we create/join the new series
try {
  tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
  state.seriesFinishedShownForSeriesId = intendedSeriesId;
  state.seriesFinishedShownForSeriesIdDismissed = intendedSeriesId;
} catch(e) { 
  dbgLog('mark series shown before createNewSeries failed', e); }
