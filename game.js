...
// --- PATCH: dispatch event so game.html sees that we joined ---
// Replace the simple dispatch you added earlier with this combined block.
try {
  // broadcast for any listeners
  window.dispatchEvent(new CustomEvent('taktik:joined', {
    detail: { gameId: gameId, hostId: hostId, opponentId: opponentId }
  }));

  // if the page exposes updatePlayerStatsOnce on window, call it now
  if (typeof window.updatePlayerStatsOnce === 'function') {
    window.updatePlayerStatsOnce().catch(e =>
      dbgLog('forced updatePlayerStatsOnce failed', e)
    );
  }

  dbgLog('post-join: dispatched taktik:joined and attempted UI update', { gameId, hostId, opponentId });
} catch (e) {
  dbgLog('post-join update error', e);
}
return gameId;
  } catch (e) {
