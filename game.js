  ...
  const doPoll = async () => {
    ...
    try {
      ...
      for (const m of moves) {
        ...
        // ✅ RÈGLE ABSOLUE : ne jamais écraser une case occupée
        if (!board[pos]) {
          board[pos] = player;
          changed = true;
          lastAppliedMove = m;
        } else if (board[pos] !== player) {
          // ⚠️ conflit — on log mais on ne touche pas
...
function applyRealtimeMove(rec) {
  try {
    ...
    if (!Array.isArray(board) || board.length !== ROWS * COLS) board = new Array(ROWS * COLS).fill(null);
    ...
    if (!board[pos]) {
      ...
      // Conflit détecté — log, mais on n'écrase pas le board local
    ...
    if (changed) {
      renderGrid(); // renderMiniMap est appelé depuis renderGrid
      current = (player === 'X') ? 'O' : 'X';
      if (currentTurnEl) currentTurnEl.textContent = current;
      // refresh metadata (winner, parties, etc.) depuis le serveur si besoin
