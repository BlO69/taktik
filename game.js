...
const markDismissed = () => {
  try {
   ...
   // try to mark now; if another handler already did, skip — BUT always show to the local series winner
...
if (amUserSeriesWinner) {
  // Gagnant de la série : effacer les flags persistants pour cette série afin qu'il voit
  // systématiquement la modale "Série terminée" / bouton "Nouvelle série", même si elle avait
  // déjà été marquée (autre onglet, reload, série précédente avec même id fallback).
  try {
    const _sid = intendedSeriesId || '__none__';
    ...
    state.seriesFinishedShownForSeriesId = null;
    state.seriesFinishedShownForSeriesIdDismissed = null;
  } catch (e) { dbgLog('clear series modal flags for winner failed', e); }
  // Marquer après reset (idempotent)
...
// traitement amélioré des modales de série en attente (remplace l'ancien bloc)
if (state._pendingSeriesModal && state._pendingSeriesModal.length) {
  for (const item of state._pendingSeriesModal) {
    try {
      ...
      // déterminer si l'utilisateur local est le gagnant de la série.
      // Note : item.info peut ne pas contenir series_winner, donc on essaie d'abord les sources locales,
      // puis on fait un fetch best-effort sur la table series si nécessaire.
      let amLocalSeriesWinnerPending = false;
      try {
        const candidateWinner = item?.info?.series_winner ?? state.series_winner ?? null;
        if (candidateWinner && state.userId && String(candidateWinner) === String(state.userId)) {
          amLocalSeriesWinnerPending = true;
        } else if (!candidateWinner && intendedSeriesId) {
          // best-effort fetch pour combler l'info manquante
          try {
            const { data: srow, error: serr } = await supabase
              .from('series')
              .select('winner_id')
              .eq('id', intendedSeriesId)
              .maybeSingle();
            if (!serr && srow && srow.winner_id) {
              // conserver l'info dans la queue pour usage ultérieur
              item.info = item.info || {};
              item.info.series_winner = srow.winner_id;
              if (state.userId && String(srow.winner_id) === String(state.userId)) {
                amLocalSeriesWinnerPending = true;
              }
            }
          } catch (fe) {
            dbgLog('fetch series winner for pending modal failed', fe);
          }
        }
      } catch (e) {
        dbgLog('determine series winner for pending modal failed', e);
      }

      // Décision : si la modale a été *dismissed* ou *marquée* par un autre client, on skip — 
      // sauf si on est le gagnant local (exception).
      if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerPending) {
        ...
        // si on est gagnant local, garantir le marquage idempotent avant d'afficher
        if (amLocalSeriesWinnerPending) tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId);
