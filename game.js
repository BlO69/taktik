...
// --- PATCH: if we have a match_id prefer match counters for series scores ---
try {
  const matchIdToRead = state.match_id ?? (partyRow && partyRow.match_id) ?? null;
  if (matchIdToRead) {
    const { data: matchRow, error: merr } = await supabase
      ...
      .maybeSingle();
    if (!merr && matchRow) {
      state.series_owner_wins = Number(matchRow.owner_series_wins ?? state.series_owner_wins ?? 0);
      state.series_opponent_wins = Number(matchRow.opponent_series_wins ?? state.series_opponent_wins ?? 0);
      dbgLog('Applied series scores from matches row', matchIdToRead, state.series_owner_wins, state.series_opponent_wins);
    }
  }
} catch (e) {
  dbgLog('fetchAndApplyPartyAndSeries: fetch match row failed', e);
}
...
try {
  await setupScoreSubscriptions();
  // --- PATCH: read match row immediately after finalize so series counters update instantly ---
try {
  const mid = state.match_id ?? (typeof finalizeRow !== 'undefined' ? finalizeRow?.match_id : null) ?? null;
  if (mid) {
    const { data: matchRow, error: merr } = await supabase
      .from('matches')
      .select('id, owner_series_wins, opponent_series_wins')
    ...
    if (!merr && matchRow) {
      state.series_owner_wins = Number(matchRow.owner_series_wins ?? state.series_owner_wins ?? 0);
      state.series_opponent_wins = Number(matchRow.opponent_series_wins ?? state.series_opponent_wins ?? 0);
      dbgLog('Applied series scores from matches after finalize', mid, state.series_owner_wins, state.series_opponent_wins);
      updatePlayerUI();
    }
  }
} catch (e) {
  dbgLog('post-finalize fetch match row failed', e);
}
...
// --- PATCH: subscribe to match row updates so series counters follow matches.owner_series_wins ---
try {
  if (state.match_id && !channels.some(ch => ch && ch._topic === `match:${state.match_id}`)) {
    const mchan = supabase.channel(`match:${state.match_id}`);
    ...
    mchan.on('postgres_changes', {
      ...
      if (!row) return;
      try {
        // Prefer match-level series counters when available
        if (row.owner_series_wins != null || row.opponent_series_wins != null) {
          ...
          dbgLog('Match subscription: applied owner_series_wins/opponent_series_wins', row.id, state.series_owner_wins, state.series_opponent_wins);
          updatePlayerUI();
        }
      } catch (e) { dbgLog('match subscription handler error', e); }
    });

    try { await mchan.subscribe(); } catch(e){ dbgLog('subscribe match channel failed', e); }
  }
} catch (e) {
  dbgLog('setupScoreSubscriptions match-subscribe failed', e);
}
      }
    } catch (e) {
