...
if (newGameId) {
 ...
 async function showNewRoundModal(info) {
  // PATCH: modale "Nouvelle manche" désactivée — la modale de victoire/série gère déjà la suite.
  try {
    return;
...
try {
  state.seriesFinishedShownForSeriesId = null;
  state.seriesFinishedShownForSeriesIdDismissed = null;
  // Purger uniquement la clé "dismissed" (pas "shown") pour l'ancienne série et __none__.
  // On conserve intentionnellement la clé "shown" : si elle est effacée, un poll tardif sur
  // l'ancienne game 'finished' déclencherait à nouveau showSeriesModal en boucle.
  const _keysToDelete = [
  ...
  for (const _k of _keysToDelete) {
    try {
      ...
      // NOTE: localStorage "shown" key intentionnellement conservée pour bloquer les réapparitions
    } catch(_e) {}
  }
} catch(e) { 
...
// PATCH: si la série est terminée, on NE réinitialise PAS outcomeShownForGameId —
// cela évite qu'un poll tardif sur l'ancienne game 'finished' re-déclenche
// showOutcomeModal (et donc l'auto-finalize) en boucle côté gagnant/perdant.
if (!state.series_winner) {
  state.outcomeShownForGameId = null;
} else {
  dbgLog('joinNewGame: série terminée — outcomeShownForGameId conservé pour bloquer les replays');
}
state.newRoundShownForGameId = null;
...
try {
  ...
  // PATCH: modales "Partie terminée" et "Nouvelle manche" désactivées.
  // Le gagnant déclenche la finalisation automatiquement (sans popup).
  // La modale de série gère l'affichage de fin de série.
  try {
    const targetGameId = forGameId ?? gameId;

    // Toujours mettre à jour lastRoundWinnerId si fourni
    if (winnerId) {
      try { state.lastRoundWinnerId = winnerId; } catch(e){ dbgLog('set lastRoundWinnerId failed', e); }
    }

    // Suppression des marqueurs new-round (sécurité)
    try {
      state.newRoundShownForGameId = null;
      state._newRoundShownAt = null;
    } catch(e){ dbgLog('clear newRound markers failed', e); }

    // Bloquer les appels redondants sur le même gameId
    if (String(state.outcomeShownForGameId) === String(targetGameId)) {
      dbgLog('showOutcomeModal: déjà traité pour', targetGameId);
      return;
    }

    // Attendre la sync initiale avant d'agir
    if (!state.initialSyncDone) {
      dbgLog('showOutcomeModal: sync initiale non terminée, marquage silencieux', targetGameId);
      state.outcomeShownForGameId = String(targetGameId);
      return;
    }

    // Marquage immédiat (anti-race)
    state.outcomeShownForGameId = String(targetGameId);

    // Retirer toute modale résiduelle (sécurité)
    try {
      const prev = document.getElementById('taktik-outcome-modal');
      if (prev) prev.remove();
    } catch(e){}

    // Si la série est déjà terminée, on ne finalise pas ici :
    // showSeriesModal sera appelé par le flux normal (finalize / polling).
    if (state.series_winner) {
      dbgLog('showOutcomeModal: série terminée, pas d\'auto-finalize (showSeriesModal prendra le relai)');
      return;
    }

    // Gagnant de la manche → finalisation automatique silencieuse
    const amLocalWinner = winnerId && state.userId && String(state.userId) === String(winnerId);
    if (amLocalWinner) {
      dbgLog('showOutcomeModal: gagnant local détecté — auto-finalize dans 400 ms', targetGameId);
      setTimeout(() => {
        handleFinalizeRoundAndFollowup(String(targetGameId), { starterChar: null })
          .catch(e => {
            dbgLog('auto-finalize failed, fallback poll', e);
            pollGameOnce().catch(() => {});
          });
      }, 400);
    } else {
      dbgLog('showOutcomeModal: non-gagnant — attente de l\'événement new-game', targetGameId);
    }
  } catch (e) {
...
if (state._pendingSeriesModal && state._pendingSeriesModal.length) {
  ...
  // PATCH: modale "Nouvelle manche" désactivée — vider la queue sans l'afficher.
