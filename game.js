...
// === Guard: if we're already joining this new game (winner path), skip heavy handling ===
if (state._joiningGameId && String(state._joiningGameId) === String(g.id)) {
  dbgLog('handleGlobalNewGameCreated: winner already joining this new game, skip heavy reset', g.id);
  // release handling lock and return — joinNewGame will do the authoritative reset + poll.
...
// NOTE : on laisse volontairement la modale 'taktik-series-modal' en place.
   ...
   // NOTE: Do NOT reset state.board here — defer authoritative reset to joinNewGame
// which is the single place that must perform the canonical UI reset + poll.
// We only remember that a new game is pending so joinNewGame (or the winner path)
// will perform the reset once and for all.
state._pendingNewGameId = String(g.id);
dbgLog('handleGlobalNewGameCreated: recorded pending new game id; waiting for joinNewGame to perform authoritative reset', g.id);   
  ...
  try {
    ...
    try {
      ...
      if (String(gameId) === String(g.id)) {
        ...
        // Render board once cleanly for this client
        try { renderBoard(state.board); } catch(e){ dbgLog('renderBoard winner-already-joined failed', e); }
        try { updatePlayerUI(); } catch(e){ dbgLog('updatePlayerUI winner-already-joined failed', e); }
      } else {
