  ...
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    ...
    dbgLog('series already marked in finalize handler, skipping show', intendedSeriesId);
      ...
      showSeriesModal({
        ...
        series_winner_pseudo: (finalizeRow.series_winner && (state.ownerProfile?.pseudo || state.opponentProfile?.pseudo)) || null,
...
btns.appendChild(closeBtn);     
  ...
  // Afficher un overlay pendant toute l'opération (gagnant aussi)
  try { 
    showLoadingOverlay(); 
  } catch (e) { 
    dbgLog('showLoadingOverlay failed', e); 
  }

  try {
    // créer la nouvelle série (RPC + join géré à l'intérieur)
    await createNewSeriesFromSeries(
      info.series_id, 
      info.last_round_winner ?? state.lastRoundWinnerId
    );

    // forcer un refresh local des compteurs / subscriptions pour éviter tout décalage d'UI
    try { 
      await fetchAndApplyPartyAndSeries(state.party_id, state.series_id); 
    } catch (e) { 
      dbgLog('refresh after create failed', e); 
    }

    try { 
      await pollGameOnce(); 
    } catch (e) { 
      dbgLog('pollGameOnce after create failed', e); 
    }
    ...
    try { 
      ...
      dbgLog('pollGameOnce failed', e2); 
    }

  } finally {
    // toujours masquer l'overlay et retirer la modale
    try { 
      hideLoadingOverlay(); 
    } catch (e) { 
      dbgLog('hideLoadingOverlay failed', e); 
    }
