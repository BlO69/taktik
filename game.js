...
// --- PATCH: si ce new-game suit la fin d'une série et que le dernier gagnant est aussi le vainqueur de la série,
//           indique au modal de ne PAS proposer le bouton "Rejoindre" au series-winner local.
try {
  if (state.series_winner && state.lastRoundWinnerId && String(state.series_winner) === String(state.lastRoundWinnerId)) {
    // drapeauter la requête pour que showNewRoundModal évite d'afficher le bouton "Rejoindre" au series winner local
    queuedInfo.info.skipJoinForSeriesWinner = true;
    dbgLog('queuedInfo flagged to skip join button for series winner', queuedInfo.info.new_game_id);
  }
} catch (e) { dbgLog('set skipJoinForSeriesWinner failed', e); }
  // avoid duplicates
...
// --- PATCH: pour les non-gagnants, afficher un overlay "Chargement..." pendant qu'on rejoint la nouvelle partie
try {
  const amLocalWinner = state.lastRoundWinnerId && state.userId && (String(state.lastRoundWinnerId) === String(state.userId));
  if (!amLocalWinner) {
    // bloque l'UI localement pour éviter le flicker entre modales
    showLoadingOverlay();
    dbgLog('show loading overlay for non-winner before joinNewGame', g.id);
  }
} catch (e) { dbgLog('showLoadingOverlay guard failed', e); }
...
if (!tryMarkModalShown('newRoundShownForGameId', g.id) && !amLocalWinner) {
  ...
  // --- PATCH: on cache l'overlay avant d'afficher la modale (si présente)
try { hideLoadingOverlay(); dbgLog('hide loading overlay before showing new-round modal'); } catch(e){ dbgLog('hideLoadingOverlay failed', e); }
  showNewRoundModal({
    party_id: state.party_id,
    party_owner_wins: state.party_owner_wins,
    party_opponent_wins: state.party_opponent_wins,
...
if (newGameId) {
 ...
 async function showNewRoundModal(info) {
  try {
    // If we are not yet initial-synced, queue the new-round modal request (do NOT mark sentinel yet)
    if (!state.initialSyncDone) {
    ...
    // If caller didn't mark earlier, attempt to mark now (idempotent)
    if (info && info.new_game_id) {
  // tenter de marquer, mais ne PAS aborter l'affichage si un caller avait déjà marqué.
  // L'appelant (ex: handleGlobalNewGameCreated) peut pré-marquer pour sérialiser le flux;
  // showNewRoundModal doit quand même construire/afficher la modale.
...
// --- PATCH: si request flagged skipJoinForSeriesWinner ET l'utilisateur local est le series_winner,
//           ne pas construire la modale new-round (le gagnant a déjà l'outcome-modal pour "Continuer").
try {
  if (info && info.skipJoinForSeriesWinner && state.userId && info.series_winner && String(state.userId) === String(info.series_winner)) {
    dbgLog('Suppressing new-round modal for series winner (skipJoinForSeriesWinner flag present)', info.new_game_id);
    return;
  }
} catch(e){ dbgLog('suppress new-round modal guard failed', e); }
...
if (state.userId && (String(state.userId) === String(state.ownerId) || String(state.userId) === String(state.opponentId))
  ...
  return; // ne pas construire/afficher la modale
    ...
    if (prev) prev.remove();
      ...
      // Determine rapidement si ce client est le gagnant local de la manche
const amLocalWinner = state.userId && state.lastRoundWinnerId && (String(state.userId) === String(state.lastRoundWinnerId));

// Si une new_game existe côté serveur, propose le bouton "Rejoindre" uniquement aux non-gagnants locaux.
// (Les gagnants locaux contrôlent la transition via l'outcome-modal "Continuer de jouer".)
if (info.new_game_id && !amLocalWinner) {
  ...
  joinBtn.onclick = async () => {
    try {
      // fermer visuel avant join
      try { if (overlay && typeof overlay.remove === 'function') overlay.remove(); } catch(_) {}
      // compute starterChar: prefer last_round_winner or server current_turn fallback
      let starterChar = null;
      if (info && info.last_round_winner) {
      ...
      try {
        ...
        dbgLog('joinNewGame from new-round modal failed', joinErr);
        try { await pollGameOnce(); } catch(_) {}
      }
      try { await pollGameOnce(); } catch(_) {}
      try { await setupScoreSubscriptions(); } catch(_) {}
    } catch (e) {
      dbgLog('joinBtn handler failed', e);
    }
  };
  btns.appendChild(joinBtn);
} else {
  // si amLocalWinner === true, on n'affiche PAS le bouton "Rejoindre" :
  // le gagnant doit utiliser l'outcome modal (Continuer de jouer) pour finaliser/continuer.
  dbgLog('skipping join button in new-round modal for local winner (they use outcome modal continue)', info?.new_game_id);
}
