...
// Decide whether to pre-mark the series modal.
// If the local user is the series winner, DO NOT pre-mark here — the winner must be allowed
// to see/click the series modal to start the next series. Otherwise pre-mark to avoid races.
const intendedSeriesId = state.series_id ? String(state.series_id) : (g && g.series_id ? String(g.series_id) : '');
const amLocalSeriesWinner = state.series_winner && state.userId && (String(state.series_winner) === String(state.userId));
if (!amLocalSeriesWinner) {
  ...
  dbgLog('skip pre-mark for series (local series winner needs to see modal)', intendedSeriesId);
}
  try {
    showSeriesModal({
      series_id: state.series_id,
...
if (finalizeRow.series_winner) {
  ...
  // If this client is the series winner, do NOT pre-mark here so the winner can see the modal.
// Otherwise pre-mark to avoid duplicate shows across clients.
const amLocalSeriesWinner = finalizeRow.series_winner && state.userId && (String(finalizeRow.series_winner) === String(state.userId));
if (!amLocalSeriesWinner) {
  if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId)) {
  ...
  dbgLog('skip pre-mark in finalize (local series winner needs to see modal)', intendedSeriesId);
}
try {
  showSeriesModal({
