...
try {
  ...
  if (state.outcomeShownForGameId || !amLocalWinner) {
    state._preMarkedOutcomeForPrevGameId = String(prevGameId);  
  } else {
  ...
  try { state._preMarkedOutcomeForPrevGameId = String(prevGameId); } catch(_) {}
...
if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerGlobal) {
  ...
  try {
    ...
    // masquer overlay si on l'avait affiché
try { if (_shouldHideLoading) exitTransitionForNonWinners(); } catch(e){ dbgLog('exitTransitionForNonWinners failed', e); }
...
try { joinMatchRPC(matchId); } catch(e){ dbgLog('joinMatchRPC setupMatchSubscription failed', e); }
  ...
  // ensure we have authoritative party/series counters before evaluating series modal
  try {
    if (state.party_id) {
      await fetchAndApplyPartyAndSeries(state.party_id, null);
    }
  } catch (e) {
    dbgLog('fetchAndApplyPartyAndSeries after pollGameOnce failed', e);
  }
}
...
async function pollPartyNewGameOnce() {
  ...
  try {
    ...
    // PATCH 3: si le serveur a clos la série et que l'utilisateur courant est le gagnant,
    // afficher immédiatement showSeriesModal pour éviter les races asynchrones
    if (state.series_winner && state.userId && String(state.userId) === String(state.series_winner)) {
      const sid = state.series_id ? String(state.series_id) : '';
      // si on ne l'a pas encore montré pour cette série, afficher
      if (String(state.seriesFinishedShownForSeriesId) !== sid) {
        try {
          dbgLog('Local series winner detected after finalize → forcing showSeriesModal', sid);
        ...
        } catch(e) { dbgLog('forced showSeriesModal after finalize failed', e); }
      }
    }
...
state.newRoundShownForGameId = null;
state.outcomeShownForGameId = null;
// Si une nouvelle série commence, aussi réinitialiser les flags liés à la modale série
...
state.seriesFinishedShownForSeriesIdDismissed = null;
  ...
  dbgLog('enqueue new-round modal failed', e);
    ...
    // If finalizeRow signals a finished series -> delegate to showSeriesModal (which will queue if needed)
// Pré-marquer la série pour éviter que plusieurs handlers la montrent en même temps
if (finalizeRow.series_winner) {
  const intendedSeriesId = String(finalizeRow.series_id ?? state.series_id ?? '');
  const amLocalSeriesWinnerFinalize = state.userId && (finalizeRow.series_winner ?? state.series_winner) &&
    String(state.userId) === String(finalizeRow.series_winner ?? state.series_winner);
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerFinalize) {   
    ...
    dbgLog('series already marked in finalize handler, skipping show', intendedSeriesId);
      ...
      showSeriesModal({
        ...
        series_winner: finalizeRow.series_winner ?? state.series_winner,
        series_winner_pseudo: (finalizeRow.series_winner &&
          (String(finalizeRow.series_winner) === String(state.ownerId)
...
if (!amUserSeriesWinner) {
  ...
  // Winner: DO NOT pre-mark here — we will mark AFTER we append the modal DOM so the winner
  // is guaranteed to see it at least once (avoids races where another client pre-marks).
  dbgLog('series winner detected — will show modal and mark after append', intendedSeriesId);
}
...
if (winnerId) {
  try { state.lastRoundWinnerId = winnerId; } catch(e){ dbgLog('set lastRoundWinnerId failed', e); }
    ...
    // (insérer à la ligne 3006 — juste après la mise à jour lastRoundWinnerId)
try {
  // assure que owner/opponent pseudo / profile sont chargés avant d'afficher le modal
  await fetchPseudosIfNeeded(state.ownerId, state.opponentId);
} catch (e) {
  dbgLog('fetchPseudosIfNeeded in showOutcomeModal failed', e);
}
...
if (prev) prev.remove();
    ...
    if (amPlayer) {
      // vue joueur
      if (amOwner && actualWinnerId && String(actualWinnerId) === String(state.ownerId)) {
        msg.textContent = `Félicitations, ${ownerPseudo} — vous avez gagné !`;
      } else if (amOwner) {
        msg.textContent = `Dommage, ${ownerPseudo} — vous avez perdu cette manche.`;
      } else if (amOpponent && actualWinnerId && String(actualWinnerId) === String(state.opponentId)) {
        msg.textContent = `Félicitations, ${opponentPseudo} — vous avez gagné !`;
      } else if (amOpponent) {
        msg.textContent = `Dommage, ${opponentPseudo} — vous avez perdu cette manche.`;
      }
    } else {
        ...
        schan.on('postgres_changes', {
          ...
          try {
            ...
            // show series modal if winner is decided and we haven't shown it
             if (state.series_winner && String(state.seriesFinishedShownForSeriesId) !== String(state.series_id)) {
  const intendedSeriesId = state.series_id ? String(state.series_id) : '';
  const amLocalSeriesWinnerSub = state.userId && state.series_winner &&
    String(state.userId) === String(state.series_winner);
  if (wasModalDismissed('seriesFinishedShownForSeriesId', intendedSeriesId)) {
    dbgLog('series modal previously dismissed, skipping show (subscription)', intendedSeriesId);
  } else if (!tryMarkModalShown('seriesFinishedShownForSeriesId', intendedSeriesId) && !amLocalSeriesWinnerSub) {
    dbgLog('series already marked, skipping show (subscription)', intendedSeriesId);
  } else {   
    showSeriesModal({
                  series_id: state.series_id,
                  series_owner_wins: state.series_owner_wins,
