...
async function pollPartyNewGameOnce() {
  ...
  function showNewRoundModal(info) {
    try {
      // Prevent repeated flashing: if we've already shown the modal for this new_game_id, skip.
// Also mark it as shown immediately to avoid races between poll/realtime/finalize.
const newGameId = info?.new_game_id ?? null;
if (newGameId && String(state.newRoundShownForGameId) === String(newGameId)) {
  dbgLog('new-round modal already shown for', newGameId);
  return;
}
if (newGameId) state.newRoundShownForGameId = String(newGameId);
